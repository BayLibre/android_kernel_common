...
use crate::{
    ...
    seq_file::SeqFile,
    str::CStr,
    types::{AsBytes, ForeignOwnable, FromBytes, Opaque},
};
use core::{
...
impl IovIter {
    ...
    pub fn copy_from_iter<T: FromBytes>(&mut self) -> Result<T> {
        let len = size_of::<T>();
        let mut out: MaybeUninit<T> = MaybeUninit::uninit();
        // SAFETY: The local variable `out` is valid for writing `size_of::<T>()` bytes.
        let res = unsafe {
            bindings::_copy_from_iter(out.as_mut_ptr().cast::<c_void>(), len, self.inner.get())
        };
        if res == len {
            // SAFETY: The above call to `_copy_from_iter` initialized `out`, and since `T` is
            // `FromBytes`, any bit-pattern is valid for the type.
