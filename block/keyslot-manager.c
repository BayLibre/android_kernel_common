	...
	void *ll_priv_data)
{
	...
	ksm = kvzalloc(sizeof(*ksm) + sizeof(ksm->slots[0]) * num_slots,
		       GFP_KERNEL);
	if (!ksm)
		return NULL;
...
void keyslot_manager_put_slot(struct keyslot_manager *ksm, unsigned int slot)
{
	...
	/*
	 * Since there isn't an atomic_dec_and_lock_irqsave function
	 * available in this kernel version, use this workaround.
	 */
	if (atomic_add_unless(&ksm->slots[slot].slot_refs, -1, 1))
		return;
...
void keyslot_manager_destroy(struct keyslot_manager *ksm)
{
	if (ksm) {
		...
		memzero_explicit(ksm, sizeof(*ksm) +
					sizeof(ksm->slots[0]) * ksm->num_slots);
		kvfree(ksm);
	}
}
