...
pipe_write(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (chars && !was_empty) {
		...
		if ((buf->flags & PIPE_BUF_FLAG_CAN_MERGE) &&
			 ...
			 * on new data being available in the pipe using epoll_wait() with EPOLLIN | EPOLLET
			 *
			 * Note that, application that expected epoll to be notified even when pipe isn't
			 * empty are buggy and need to be fixed. This only restores the behavior for small
			 * writes (single byte) on fifos because they are currently used by soome application libraries
			 * to serve serve notification / looper threads
			 *
