...
static inline void fuse_bpf_set_in_ends(struct bpf_fuse_args *fa)
{
	...
	for (i = 0; i < 3; i++)
		fa->ctx.in_args[i].end_offset = (void *)
			((char *)fa->ctx.in_args[i].value
			+ fa->ctx.in_args[i].size);
}
...
static inline void fuse_bpf_set_in_immutable(struct bpf_fuse_args *fa)
{
	...
	for (i = 0; i < 3; i++)
		fa->in_flags[i] |= BPF_FUSE_IMMUTABLE;
}
...
static inline void fuse_bpf_set_out_ends(struct bpf_fuse_args *fa)
{
	...
	for (i = 0; i < 2; i++)
		fa->ctx.out_args[i].end_offset = (void *)
			((char *)fa->ctx.out_args[i].value
			+ fa->ctx.out_args[i].size);
}
...
static inline void fuse_bpf_free_alloced(struct bpf_fuse_args *fa)
{
	...
	for (i = 0; i < 3; i++)
		if (fa->in_flags[i] & BPF_FUSE_ALLOCATED)
			kfree(fa->ctx.in_args[i].value);
	for (i = 0; i < 2; i++)
		if (fa->out_flags[i] & BPF_FUSE_ALLOCATED)
			kfree(fa->ctx.out_args[i].value);
}
...
({									\
	...
	do {								\
		...
		if (IS_ERR(fer.result))					\
			fa.ctx.error_in = PTR_ERR(fer.result);		\
		if (!(ext_flags & FUSE_BPF_POST_FILTER))		\
			break;						\
									\
		fa.ctx.opcode |= FUSE_POSTFILTER;			\
