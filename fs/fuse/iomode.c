...
int fuse_inode_uncached_io_start(struct fuse_inode *fi, struct fuse_backing *fb)
{
	...
	printk("DROSEN: uncached io start. fb=%p after open, passctr=%d, cachectr=%d", fb, fi->iopassctr, fi->iocachectr);
...
void fuse_inode_uncached_io_end(struct fuse_inode *fi)
{
	...
	printk("DROSEN: end uncahced io.passctr=%d, cachectr=%d", fi->iopassctr, fi->iocachectr);
	if (!fi->iocachectr) {
...
static void fuse_inode_passthrough_io_end(struct fuse_inode *fi)
{
	...
	printk("DROSEN: end passthrough io.passctr=%d, cachectr=%d", fi->iopassctr, fi->iocachectr);
	if (!fi->iopassctr) {
		oldfb = fuse_inode_backing_set(fi, NULL);
	}
	spin_unlock(&fi->lock);
