...
void fuse_open_initialize(struct fuse_args *fa, struct fuse_open_io *foio,
...
void fuse_create_open_initialize(
...
void fuse_release_initialize(struct fuse_args *fa, struct fuse_release_in *fri,
...
void fuse_releasedir_initialize(struct fuse_args *fa,
...
void fuse_flush_initialize(struct fuse_args *fa, struct fuse_flush_in *ffi,
...
void fuse_fsync_initialize(struct fuse_args *fa, struct fuse_fsync_in *ffi,
...
void fuse_dir_fsync_initialize(struct fuse_args *fa, struct fuse_fsync_in *ffi,
		   struct file *file, loff_t start, loff_t end, int datasync)
{
	struct fuse_file *fuse_file = file->private_data;
...
void fuse_getxattr_initialize(struct fuse_args *fa,
		...
		size_t size)
{
	if (fa->out_argvar)
		...
		struct fuse_getxattr_out *fgo = fa->out_args[0].value;

		return ERR_PTR(fgo->size);
	}
}

void fuse_listxattr_initialize(struct fuse_args *fa,
			       struct fuse_getxattr_io *fgio,
...
void fuse_setxattr_initialize(struct fuse_args *fa,
...
void fuse_file_read_iter_initialize(
...
void fuse_file_write_iter_initialize(
...
void fuse_file_fallocate_initialize(struct fuse_args *fa,
...
void fuse_lookup_initialize(struct fuse_args *fa, struct fuse_lookup_io *fli,
    ...
    struct inode *dir, struct dentry *entry, umode_t mode, dev_t rdev)
{
	...
	err = vfs_mknod(backing_inode, backing_path.dentry, fmi->mode & ~fmi->umask, new_decode_dev(fmi->rdev));
	...
	if (d_really_is_negative(backing_path.dentry) ||
		...
		err = -EINVAL;
		//TODO overlayfs responds to this situation with a lookupOneLen. Should we do that too?
		goto out;
	}
	inode = fuse_iget_backing(dir->i_sb, backing_inode);
	if (IS_ERR(inode)) {
...
void fuse_rename2_initialize(struct fuse_args *fa, struct fuse_rename2_in *fri,
			      struct inode *olddir, struct dentry *oldent,
			      struct inode *newdir, struct dentry *newent,
			      unsigned int flags)
{
...
void fuse_rename_initialize(struct fuse_args *fa, struct fuse_rename_in *fri,
			     struct inode *olddir, struct dentry *oldent,
			     struct inode *newdir, struct dentry *newent)
{
...
void fuse_link_initialize(struct fuse_args *fa, struct fuse_link_in *fli,
...
void fuse_getattr_initialize(struct fuse_args *fa, struct fuse_getattr_io *fgio,
...
/* TODO reconcile with version in dir.c */
static void iattr_to_fattr(const struct iattr *iattr,
			   struct fuse_setattr_in *arg)
{
	...
	if (ivalid & ATTR_UID)
		arg->valid |= FATTR_UID,   arg->uid = __kuid_val(iattr->ia_uid);
	if (ivalid & ATTR_GID)
		arg->valid |= FATTR_GID,   arg->gid = __kgid_val(iattr->ia_gid);
	if (ivalid & ATTR_SIZE)
...
void fuse_setattr_initialize(struct fuse_args *fa, struct fuse_setattr_io *fsio,
		struct dentry *dentry, struct iattr *attr, struct file *file)
{
	*fsio = (struct fuse_setattr_io) {0};
	iattr_to_fattr(attr, &fsio->fsi);
...
void fuse_get_link_initialize(struct fuse_args *fa, struct fuse_dummy_io *unused,
		struct inode *inode, struct dentry *dentry, struct delayed_call *callback, const char **out)
{
	/* TODO
		...
		struct inode *inode, struct dentry *dentry, struct delayed_call *callback, const char **out)
{
	...
	/* TODO: If we want to do our own thing, copy the data and then call the callback */
...
void fuse_readdir_initialize(struct fuse_args *fa, struct fuse_read_io *frio,
			     ...
			     bool *force_again, bool *allow_force)
{
	...
	if (!page) {
		/* Proceed with empty buffer for now ... */
	}
...
void fuse_access_initialize(struct fuse_args *fa, struct fuse_access_in *fai,
