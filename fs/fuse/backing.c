		      ...
		      struct inode *inode, struct file *file, bool isdir)
{
	...
	retval = inode_permission(mnt_user_ns(fd->backing_path.mnt),
		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	err = vfs_create(mnt_user_ns(get_fuse_dentry(entry)->backing_path.mnt),
...
int fuse_getxattr_backing(struct fuse_args *fa,
		...
		size_t size)
{
	ssize_t ret = vfs_getxattr(
		mnt_user_ns(get_fuse_dentry(dentry)->backing_path.mnt),
			  ...
			  int flags)
{
	return vfs_setxattr(
		mnt_user_ns(get_fuse_dentry(dentry)->backing_path.mnt),
		get_fuse_dentry(dentry)->backing_path.dentry, name, value, size,
		flags);
}
			     ...
			     struct dentry *dentry, const char *name)
{
	...
	return vfs_removexattr(
		mnt_user_ns(get_fuse_dentry(dentry)->backing_path.mnt),
		backing_path->dentry, name);
}
			   ...
			   struct dentry *entry, unsigned int flags)
{
	...
	case FUSE_ACTION_REPLACE: {
		...
		close_task_fd(fc->task, febo->backing_fd);
		if (!backing_file)
			return ERR_PTR(-EIO);
...
int fuse_canonical_path_initialize(
		struct fuse_args *fa, struct fuse_dummy_io *fdi,
		const struct path *path, struct path *canonical_path)
{
...
void *fuse_canonical_path_finalize(
		struct fuse_args *fa,
		const struct path *path, struct path *canonical_path)
{
	return NULL;
		...
		struct inode *dir, struct dentry *entry, umode_t mode, dev_t rdev)
{
	...
	if (!IS_POSIXACL(backing_inode))
		mode &= ~fmi->umask;
	err = vfs_mknod(mnt_user_ns(backing_path.mnt), backing_inode,
			backing_path.dentry, mode, new_decode_dev(fmi->rdev));
		...
		struct inode *dir, struct dentry *entry, umode_t mode)
{
	...
	if (!IS_POSIXACL(backing_inode))
		mode &= ~fmi->umask;
	err = vfs_mkdir(mnt_user_ns(backing_path.mnt), backing_inode,
		...
		struct inode *dir, struct dentry *entry)
{
	...
	inode_lock_nested(backing_inode, I_MUTEX_PARENT);
	err = vfs_rmdir(mnt_user_ns(backing_path.mnt), backing_inode,
			backing_path.dentry);
	inode_unlock(backing_inode);
			 ...
			 unsigned int flags)
{
	...
	rd = (struct renamedata) {
		.old_mnt_userns = mnt_user_ns(
			get_fuse_dentry(old_backing_dentry)->backing_path.mnt),
		.old_dir = d_inode(old_backing_dir_dentry),
		.old_dentry = old_backing_dentry,
		.new_mnt_userns = mnt_user_ns(
			get_fuse_dentry(new_backing_dentry)->backing_path.mnt),
		...
		struct inode *dir, struct dentry *entry)
{
	...
	inode_lock_nested(backing_inode, I_MUTEX_PARENT);
	err = vfs_unlink(mnt_user_ns(backing_path.mnt), backing_inode,
			 backing_path.dentry, NULL);
	inode_unlock(backing_inode);
		      ...
		      struct inode *dir, struct dentry *newent)
{
	...
	err = vfs_link(
		backing_old_path.dentry,
		mnt_user_ns(
			get_fuse_dentry(backing_dir_dentry)->backing_path.mnt),
		...
		struct dentry *dentry, struct iattr *attr, struct file *file)
{
	...
	res = notify_change(mnt_user_ns(backing_path->mnt), backing_path->dentry,
			    &new_attr, NULL);
		...
		struct inode *dir, struct dentry *entry, const char *link, int len)
{
	...
	inode_lock_nested(backing_inode, I_MUTEX_PARENT);
	err = vfs_symlink(mnt_user_ns(backing_path.mnt), backing_inode,
			  backing_path.dentry, link);
	inode_unlock(backing_inode);
	if (err)
		goto out;
	if (d_really_is_negative(backing_path.dentry) ||
...
int fuse_access_backing(struct fuse_args *fa, struct inode *inode, int mask)
{
	...
	return inode_permission(mnt_user_ns(fi->backing_path.mnt),
