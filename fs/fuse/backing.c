			 ...
			 struct inode *inode, struct file *file, bool isdir)
{
	...
	fa->ctx = (struct bpf_fuse_context) {
		.nodeid = get_fuse_inode(inode)->nodeid,
		.opcode = isdir ? FUSE_OPENDIR : FUSE_OPEN,
		.in_numargs = 1,
		.in_args[0] = (struct fuse_bpf_arg) {
			 ...
			 struct inode *inode, struct file *file, bool isdir)
{
	...
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0] = (struct fuse_bpf_arg) {
		      ...
		      struct inode *inode, struct file *file, bool isdir)
{
	...
	const struct fuse_open_in *foi = fa->ctx.in_args[0].value;
	struct fuse_file *ff;
	int retval;
	int mask;
		       ...
		       struct inode *inode, struct file *file, bool isdir)
{
	...
	struct fuse_open_out *foo = fa->ctx.out_args[0].value;
		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	fa->ctx = (struct bpf_fuse_context) {
		...
		.in_args[1] = (struct fuse_bpf_arg) {
			.size = entry->d_name.len + 1,
			.max_size = NAME_MAX + 1,
			.value =  (void *) entry->d_name.name,
		},
	};
	fa->in_flags[1] = BPF_FUSE_VARIABLE_SIZE | BPF_FUSE_MUST_ALLOCATE;
		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	fa->ctx.out_numargs = 2;
	fa->ctx.out_args[0] = (struct fuse_bpf_arg) {
		.size = sizeof(fcoio->feo),
		.value = &fcoio->feo,
	};
	fa->ctx.out_args[1] = (struct fuse_bpf_arg) {
		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	int err = 0;
	const struct fuse_create_in *fci = fa->ctx.in_args[0].value;

	if (!dir_fuse_inode || !dir_fuse_dentry)
		return -EIO;

	inode_lock_nested(dir_fuse_inode->backing_inode, I_MUTEX_PARENT);
	backing_dentry = lookup_one_len(fa->ctx.in_args[1].value,
					dir_fuse_dentry->backing_path.dentry,
					strlen(fa->ctx.in_args[1].value));
		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	struct fuse_entry_out *feo = fa->ctx.out_args[0].value;
	struct fuse_open_out *foo = fa->ctx.out_args[1].value;
...
{
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(flio->flo);
	fa->ctx.out_args[0].value = &flio->flo;
...
{
	const struct fuse_lseek_in *fli = fa->ctx.in_args[0].value;
	struct fuse_lseek_out *flo = fa->ctx.out_args[0].value;
	struct fuse_file *fuse_file = file->private_data;
	struct file *backing_file = fuse_file->backing_file;
	loff_t ret;
...
{
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(fcf->fwo);
	fa->ctx.out_args[0].value = &fcf->fwo;
...
int fuse_copy_file_range_backing(struct bpf_fuse_args *fa, struct file *file_in, loff_t pos_in,
				 ...
				 unsigned int flags)
{
	const struct fuse_copy_file_range_in *fci = fa->ctx.in_args[0].value;
	struct fuse_file *fuse_file_in = file_in->private_data;
		   ...
		   struct file *file, loff_t start, loff_t end, int datasync)
{
	struct fuse_file *fuse_file = file->private_data;
	struct file *backing_file = fuse_file->backing_file;
	const struct fuse_fsync_in *ffi = fa->ctx.in_args[0].value;
		...
		size_t size)
{
	*fgio = (struct fuse_getxattr_io) {
	...
	fa->in_flags[1] = BPF_FUSE_MUST_ALLOCATE | BPF_FUSE_VARIABLE_SIZE;
...
int fuse_getxattr_initialize_out(struct bpf_fuse_args *fa,
		...
		size_t size)
{
	fa->ctx.flags = size ? FUSE_BPF_OUT_ARGVAR : 0;
	fa->ctx.out_numargs = 1;
	if (size) {
		fa->ctx.out_args[0].size = size;
		fa->ctx.out_args[0].max_size = size;
		fa->ctx.out_args[0].value = value;
		fa->out_flags[0] = BPF_FUSE_VARIABLE_SIZE;
	} else {
		fa->ctx.out_args[0].size = sizeof(fgio->fgo);
		fa->ctx.out_args[0].value = &fgio->fgo;
	}
	return 0;
}
		...
		size_t size)
{
	ssize_t ret = vfs_getxattr(get_fuse_dentry(dentry)->backing_path.dentry,
				   fa->ctx.in_args[1].value, value, size);

	if (fa->ctx.flags & FUSE_BPF_OUT_ARGVAR)
		fa->ctx.out_args[0].size = ret;
	else
		((struct fuse_getxattr_out *)fa->ctx.out_args[0].value)->size = ret;
		...
		size_t size)
{
	...
	if (fa->ctx.flags & FUSE_BPF_OUT_ARGVAR)
		return ERR_PTR(fa->ctx.out_args[0].size);

	fgo = fa->ctx.out_args[0].value;
			      ...
			      struct dentry *dentry, char *list, size_t size)
{
	...
	if (size) {
		fa->ctx.flags = FUSE_BPF_OUT_ARGVAR;
		fa->ctx.out_args[0].size = size;
		fa->ctx.out_args[0].max_size = size;
		fa->ctx.out_args[0].value = (void *)list;
		fa->out_flags[0] = BPF_FUSE_VARIABLE_SIZE;
	} else {
		fa->ctx.out_args[0].size = sizeof(fgio->fgo);
		fa->ctx.out_args[0].value = &fgio->fgo;
			     ...
			     const void *value, size_t size, int flags)
{
	*fsxi = (struct fuse_setxattr_in) {
	...
	fa->in_flags[1] = BPF_FUSE_VARIABLE_SIZE | BPF_FUSE_MUST_ALLOCATE;
	fa->in_flags[2] = BPF_FUSE_VARIABLE_SIZE | BPF_FUSE_MUST_ALLOCATE;
				...
				struct dentry *dentry, const char *name)
{
	...
	fa->in_flags[0] = BPF_FUSE_VARIABLE_SIZE | BPF_FUSE_MUST_ALLOCATE;
		...
		struct kiocb *iocb, struct iov_iter *to)
{
	...
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(fri->frio);
	fa->ctx.out_args[0].value = &fri->frio;
...
int fuse_file_read_iter_backing(struct bpf_fuse_args *fa,
		struct kiocb *iocb, struct iov_iter *to)
{
	struct fuse_read_iter_out *frio = fa->ctx.out_args[0].value;
	struct file *file = iocb->ki_filp;
	struct fuse_file *ff = file->private_data;
	ssize_t ret;
		...
		struct kiocb *iocb, struct iov_iter *from)
{
	/* TODO we can't assume 'from' is a kvec */
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(fwio->fwio);
	fa->ctx.out_args[0].value = &fwio->fwio;
		...
		struct kiocb *iocb, struct iov_iter *from)
{
	struct file *file = iocb->ki_filp;
	struct fuse_file *ff = file->private_data;
	struct fuse_write_iter_out *fwio = fa->ctx.out_args[0].value;
	ssize_t ret;
...
int fuse_file_fallocate_backing(struct bpf_fuse_args *fa,
		struct file *file, int mode, loff_t offset, loff_t length)
{
	const struct fuse_fallocate_in *ffi = fa->ctx.in_args[0].value;
	struct fuse_file *ff = file->private_data;
			   ...
			   struct dentry *entry, unsigned int flags)
{
	...
	struct fuse_entry_out *feo = fa->ctx.out_args[0].value;
	struct fuse_entry_bpf_out *febo = fa->ctx.out_args[1].value;
...
{
	fa->ctx.opcode = FUSE_CANONICAL_PATH;
	return 0;
}
...
{
	int err = 0;
	const struct fuse_mknod_in *fmi = fa->ctx.in_args[0].value;
...
{
	int err = 0;
	const struct fuse_mkdir_in *fmi = fa->ctx.in_args[0].value;
	struct inode *backing_inode = get_fuse_inode(dir)->backing_inode;
	struct path backing_path = {};
	struct inode *inode = NULL;
	struct dentry *d;
...
int fuse_rename2_backing(struct bpf_fuse_args *fa,
			 ...
			 unsigned int flags)
{
	const struct fuse_rename2_in *fri = fa->ctx.in_args[0].value;
			 ...
			 struct dentry *newent)
{
	...
	*fli = (struct fuse_link_in){
	...
	fa->ctx.opcode = FUSE_LINK;
	fa->ctx.in_numargs = 2;
	fa->ctx.in_args[0].size = sizeof(*fli);
	fa->ctx.in_args[0].value = fli;
	fa->ctx.in_args[1].size = newent->d_name.len + 1;
	fa->ctx.in_args[1].max_size = NAME_MAX + 1;
	fa->ctx.in_args[1].value = (void *) newent->d_name.name;
	fa->in_flags[1] = BPF_FUSE_VARIABLE_SIZE | BPF_FUSE_MUST_ALLOCATE;
			...
			u32 request_mask, unsigned int flags)
{
	...
	struct fuse_attr_out *fao = fa->ctx.out_args[0].value;
	struct kstat tmp;
	int err;
...
void *fuse_getattr_finalize(struct bpf_fuse_args *fa,
			...
			u32 request_mask, unsigned int flags)
{
	struct fuse_attr_out *outarg = fa->ctx.out_args[0].value;
...
{
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(fsio->fao);
	fa->ctx.out_args[0].value = &fsio->fao;
...
{
	struct fuse_conn *fc = get_fuse_conn(dentry->d_inode);
	const struct fuse_setattr_in *fsi = fa->ctx.in_args[0].value;
		...
		struct dentry *dentry, struct kstatfs *buf)
{
	*fso = (struct fuse_statfs_out) { 0 };
	...
	fa->ctx.out_numargs = 1;
	fa->ctx.out_args[0].size = sizeof(fso);
	fa->ctx.out_args[0].value = fso;
		...
		struct dentry *dentry, struct kstatfs *buf)
{
	int err = 0;
	struct path backing_path;
	struct fuse_statfs_out *fso = fa->ctx.out_args[0].value;
			    ...
			    bool *force_again, bool *allow_force, bool is_continued)
{
	...
	fa->out_flags[1] = BPF_FUSE_VARIABLE_SIZE;
			 ...
			 bool *force_again, bool *allow_force, bool is_continued)
{
	...
	struct fuse_read_out *fro = fa->ctx.out_args[0].value;
	...
	if (ec.offset == 0)
		*allow_force = false;
	fa->ctx.out_args[1].size = ec.offset;
			    ...
			    bool *force_again, bool *allow_force, bool is_continued)
{
	...
	err = parse_dirfile(fa->ctx.out_args[1].value, fa->ctx.out_args[1].size, ctx);
	*force_again = !!fro->again;
	...
	free_page((unsigned long) fa->ctx.out_args[1].value);
	return ERR_PTR(err);
}
...
int fuse_access_backing(struct bpf_fuse_args *fa, struct inode *inode, int mask)
{
	...
	const struct fuse_access_in *fai = fa->ctx.in_args[0].value;
...
ssize_t fuse_bpf_simple_request(struct fuse_mount *fm, struct bpf_fuse_args *fa)
{
	...
	uint32_t max_size;
	ssize_t res;
	struct bpf_fuse_context *ctx = &fa->ctx;
	bool is_prefilter = !!(ctx->opcode & FUSE_PREFILTER);
	bool is_postfilter = !!(ctx->opcode & FUSE_POSTFILTER);

	struct fuse_args args = {
		.nodeid = ctx->nodeid,
		.opcode = ctx->opcode,
		.error_in = ctx->error_in,
		.in_numargs = is_postfilter ? ctx->in_numargs + ctx->out_numargs : ctx->in_numargs,
		.out_numargs = is_prefilter ? ctx->in_numargs : ctx->out_numargs,
		.force = !!(ctx->flags & FUSE_BPF_FORCE),
		.out_argvar = !!(ctx->flags & FUSE_BPF_OUT_ARGVAR),
	};

	/* Set in args */
	for (i = 0; i < ctx->in_numargs; ++i)
		args.in_args[i] = (struct fuse_in_arg) {
			.size = ctx->in_args[i].size,
			.value = ctx->in_args[i].value,
		};

	if (is_postfilter)
		for (i = 0; i < ctx->out_numargs; ++i)
			args.in_args[ctx->in_numargs + i] = (struct fuse_in_arg) {
				.size = ctx->out_args[i].size,
				.value = ctx->out_args[i].value,
			};

	/* All out args must be writeable */
	if (is_prefilter)
		for (i = 0; i < ctx->in_numargs; ++i) {
			max_size = ctx->in_args[i].max_size ?: ctx->in_args[i].size;
			if (!bpf_fuse_get_writeable(ctx, false, i, max_size, true))
				return -ENOMEM;
		}

	if (is_postfilter)
		for (i = 0; i < ctx->out_numargs; ++i) {
			max_size = ctx->out_args[i].max_size ?: ctx->out_args[i].size;
			if (!bpf_fuse_get_writeable(ctx, true, i, max_size, true))
				return -ENOMEM;
		}
	...
	if (is_prefilter) {
		for (i = 0; i < ctx->in_numargs; ++i)
			if (fa->in_flags[i] & BPF_FUSE_VARIABLE_SIZE)
				ctx->in_args[i].size = args.out_args[i].size;
	} else {
		for (i = 0; i < ctx->out_numargs; ++i)
			if (fa->out_flags[i] & BPF_FUSE_VARIABLE_SIZE)
				ctx->out_args[i].size = args.out_args[i].size;
	}
