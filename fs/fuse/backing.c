				 ...
				 struct file *file_out, loff_t pos_out,size_t len, unsigned int flags)
{
	...
	/* TODO: Handle if out location has no backing. Handle changing of in/out files */
	return vfs_copy_file_range(backing_file_in, fci->off_in, backing_file_out, fci->off_out, fci->len, fci->flags);
}
