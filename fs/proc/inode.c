...
{
	__typeof__(pde->proc_ops->proc_read) read = pde->proc_ops->proc_read;
	if (read)
...
{
	__typeof__(pde->proc_ops->proc_write) write = pde->proc_ops->proc_write;
	if (write)
...
{
	__typeof__(pde->proc_ops->proc_poll) poll = pde->proc_ops->proc_poll;
	if (poll)
...
{
	__typeof__(pde->proc_ops->proc_ioctl) ioctl = pde->proc_ops->proc_ioctl;
	if (ioctl)
...
{
	__typeof__(pde->proc_ops->proc_compat_ioctl) compat_ioctl = pde->proc_ops->proc_compat_ioctl;
	if (compat_ioctl)
...
{
	__typeof__(pde->proc_ops->proc_mmap) mmap = pde->proc_ops->proc_mmap;
	if (mmap)
...
static int proc_reg_open(struct inode *inode, struct file *file)
{
	...
	__typeof__(pde->proc_ops->proc_release) release = pde->proc_ops->proc_release;
	if (release) {
