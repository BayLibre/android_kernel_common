		...
		struct mem_range sig)
{
	...
	if (signed_data.len > 0) {
		...
		sg.sg_signed_data_size = cpu_to_le32(signed_data.len);
		sg.sg_signed_data_offset = cpu_to_le64(pos);

		result = write_to_bf(bfc, signed_data.data,
			signed_data.len, pos, false);
		if (result)
			goto err;
	}

	tree_area_pos = incfs_get_end_offset(bfc->bc_file);
	if (signed_data.len > 0 && tree_size > 0) {
