...
static int update_file_header_flags(struct data_file* df, u32 bits_to_reset, u32 bits_to_set)
{
	int result;
	u32 new_flags;
	...
	/*
	 * It's fine to write out of the mutex: the written value calculation
	 * is protected, thus, any thread would write the same thing here.
	*/
	mutex_unlock(&bfc->bc_mutex);
	...
	*size_out = 0;
	...
	if (!error && in_range && arg->start_index == 0 &&
	    ...
	    *size_out == sizeof(struct incfs_filled_range)) {
		int result = update_file_header_flags(df, 0, INCFS_FILE_COMPLETE);
