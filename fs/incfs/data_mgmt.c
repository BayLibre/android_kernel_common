			   ...
			   int block_index)
{
	...
	if (log->rl_head.mode == RL_CACHE) {
		...
		do {
			...
			spin_unlock(&log->rl_lock);
			page = head.cache_offset ?
				find_lock_page(log->rl_log_inode->i_mapping,
						head.cache_index) :
				grab_cache_page(log->rl_log_inode->i_mapping,
						head.cache_index);
			if (page)
				addr = kmap_atomic(page);
		...
		if (page)
			memcpy(addr + head->cache_offset, &record, record_size);
