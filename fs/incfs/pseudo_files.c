...
static long ioctl_get_read_timeouts(struct mount_info *mi, void __user *arg)
{
	...
	if (!error &&
	    copy_to_user(u64_to_user_ptr(args.timeouts_array), buffer, size))
		error = -EFAULT;
...
static long ioctl_set_read_timeouts(struct mount_info *mi, void __user *arg)
{
	...
	int error = 0;
	int size;
	struct incfs_per_uid_read_timeouts *buffer, *tmp;
	int i;
	...
	if (size > INCFS_DATA_FILE_BLOCK_SIZE || size % sizeof(*buffer) != 0)
		return -EINVAL;
	...
	for (i = 0; i < size / sizeof(*buffer); ++i) {
		...
		if (t->min_time_ms > t->min_pending_time_ms ||
		    t->min_pending_time_ms > t->max_pending_time_ms) {
			error = -EINVAL;
			goto out;
		}
	}
