				...
				unsigned int digest_algorithm)
{
	...
	memcpy(d->digest, measurement, hash_alg->digest_size);
	...
	pr_debug("Valid signature for file measurement %s:%*phN\n",
		 hash_alg->name, hash_alg->digest_size, measurement);
	return 0;
}
			      ...
			      size_t desc_size)
{
	...
	return __fsverity_verify_signature(inode, desc->signature, sig_size,
				vi->measurement, hash_alg - fsverity_hash_algs);
}
