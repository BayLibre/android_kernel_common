...
/sys/fs/incremental-fs/features/v2
  Reads 'supported'. Present if all v2 features of incfs are supported.
...
/sys/fs/incremental-fs/instances/[name]/last_error
  Returns information about the last read error. Initially reads

  (0, null, 0, 0)

  After an error has occurred it reads

  (time, filename, index, errno)

  time is a decimal number stating time in ns since 1970 when the last error
  occurred.

  filename is the name of the file that was being read when the error occurred.
  It is expressed as a fully qualified path from the root of the mount.

  index is the index of the page that failed to read

  errno is the error number that was returned from the failed read
