...
DECLARE_HOOK(android_vh_binder_restore_priority,
	TP_PROTO(struct binder_transaction *t, struct task_struct *task),
	TP_ARGS(t, task));
DECLARE_RESTRICTED_HOOK(android_rvh_binder_wakeup_poll_threads_ilocked,
	TP_PROTO(struct task_struct *task),
	TP_ARGS(task), 1);
DECLARE_RESTRICTED_HOOK(android_rvh_binder_wakeup_thread_ilocked,
	TP_PROTO(struct task_struct *task),
	TP_ARGS(task), 1);
#else
...
#define trace_android_rvh_binder_wakeup_poll_threads_ilocked(task)
#define trace_android_rvh_binder_wakeup_thread_ilocked(task)
#endif
