...
DECLARE_HOOK(android_vh_nr_pcp_alloc,
	TP_PROTO(struct per_cpu_pages *pcp, struct zone *zone, struct per_cpu_zone_extra_data *extra_data,
		unsigned int order, int *batch),
	TP_ARGS(pcp, zone, extra_data, order, batch));
DECLARE_HOOK(android_vh_pcp_alloc_factor_adjust,
	TP_PROTO(struct zone *zone, struct per_cpu_zone_extra_data *extra_data, struct per_cpu_pages *pcp,
		struct page *page, int migratetype, unsigned int order),
	TP_ARGS(zone, extra_data, pcp, page, migratetype, order));
