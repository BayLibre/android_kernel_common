...
DECLARE_HOOK(android_vh_cma_set_owner,
	TP_PROTO(struct cma *cma, unsigned int bitmap_no,
		unsigned int bitmap_count),
	TP_ARGS(cma, bitmap_no, bitmap_count));
DECLARE_HOOK(android_vh_cma_clear_owner,
	TP_PROTO(struct cma *cma, unsigned long pfn,
		unsigned int count),
	TP_ARGS(cma, pfn, count));
DECLARE_HOOK(android_vh_cma_show_owner,
