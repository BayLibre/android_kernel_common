...
struct io_pgtable_ops {
	...
	size_t (*unmap_pages_walk)(struct io_pgtable *iop, unsigned long iova,
				   size_t pgsize, size_t pgcount,
				   struct iommu_iotlb_gather *gather,
...
static inline size_t
iopt_unmap_pages_walk(struct io_pgtable *iop, unsigned long iova, size_t pgsize,
		      size_t pgcount, struct iommu_iotlb_gather *gather,
		      struct io_pgtable_walker *walker)
{
	if (!iop->ops || !iop->ops->unmap_pages)
		return 0;
	return iop->ops->unmap_pages_walk(iop, iova, pgsize, pgcount, gather, walker);
}
