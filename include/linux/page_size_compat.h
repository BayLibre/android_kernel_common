...
static __always_inline bool __page_aligned_log(unsigned long addr)
{
	if (___PAGE_ALIGNED(addr))
		return true;
...
#define __PAGE_ALIGNED(addr)    __page_aligned_log(addr)
 ...
 *         @size is increased by PAGE_SHIFT (4KB):
 *             ((1 << (1)) - 1) << PAGE_SHIFT
 *             (2        ) - 1) << PAGE_SHIFT
 *             (1             ) << PAGE_SHIFT
...
/*
 * Conditional page-alignement based on mmap flags
 *
