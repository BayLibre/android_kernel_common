/*
 * Copyright 2021 Google LLC
 * Author: ramjiyani@google.com (Ramji Jiyani)
 */

#include <linux/bsearch.h>
#include <linux/errno.h>
#include <linux/printk.h>
#include <linux/string.h>

/*
 * Build time generated header files
 *
 * gki_module_exported.h -- Syms protected from _export_ by unsigned modules
 * gki_module_protected.h -- Syms protected from _access_ by unsigned modules
 */
#include "gki_module_protected.h"
#include "gki_module_exported.h"

/* bsearch() comparision callback */
static int cmp_name(const void *sym, const void *protected_sym)
{
	return strcmp(sym, protected_sym);
}

/**
 * is_gki_module_protected_symbol - Is a symbol protected from vendor module?
 *
 * @name:	Symbol being checked against protection from vendor module
 */
bool is_gki_module_protected_symbol(const char *name)
{
	return bsearch(name, gki_protected_symbols, NO_OF_PROTECTED_SYMBOLS,
		       MAX_SYM_NAME_LEN, cmp_name) != NULL;
}

/**
 * is_gki_module_exported_symbol - Is a symbol exported from a GKI module?
 *
 * @name:	Symbol being checked against exported symbols from GKI modules
 */
bool is_gki_module_exported_symbol(const char *name)
{
	return bsearch(name, gki_exported_symbols, NO_OF_EXPORTED_SYMBOLS,
		       MAX_EXPORT_NAME_LEN, cmp_name) != NULL;
}
