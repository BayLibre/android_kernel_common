 ...
 * scenario that complicates the sequence. This patch handles this be
 ...
 * is triggered, solving the original issue at hand. Starts filesystem sync in a
 * workqueue, which will then follow the nominal suspend sequence. In case of
 * abort during fs_sync, start the resume process in the main thread so that
 * filesystem sync is not blocking and so that the OS is responsive to user.
 */
