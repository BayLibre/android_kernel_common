...
#define DMA_POOL_FLAGS_ENCRYPTED		(uintptr_t)BIT(0)
				    ...
				    struct device *dev)
{
	...
	bool pool_encrypted = (uintptr_t)mem & DMA_POOL_FLAGS_ENCRYPTED;

	/* Set Per-device io tlb area to one */
	unsigned int nareas = 1;

	mem = (struct io_tlb_mem *)((uintptr_t)mem & ~DMA_POOL_FLAGS_ENCRYPTED);
