 ...
 * func:_trace_android_vh_pcpu_rwsem_acquired(released) for
 * both percpu-rwsem.h and percpu-rwsem.c to call.
 */
void _trace_android_vh_pcpu_rwsem_acquired(struct percpu_rw_semaphore *sem)
{
	trace_android_vh_record_pcpu_rwsem_starttime(sem, jiffies);
}
EXPORT_SYMBOL_GPL(_trace_android_vh_pcpu_rwsem_acquired);

void _trace_android_vh_pcpu_rwsem_released(struct percpu_rw_semaphore *sem)
{
	trace_android_vh_record_pcpu_rwsem_starttime(sem, 0);
}
EXPORT_SYMBOL_GPL(_trace_android_vh_pcpu_rwsem_released);
...
void __sched percpu_down_write(struct percpu_rw_semaphore *sem)
{
	...
	if (contended)
		trace_contention_end(sem, 0);
	_trace_android_vh_pcpu_rwsem_acquired(sem);
}
...
void percpu_up_write(struct percpu_rw_semaphore *sem)
{
	...
	_trace_android_vh_pcpu_rwsem_released(sem);
}
