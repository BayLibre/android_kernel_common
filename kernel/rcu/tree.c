...
int param_set_rcu_lazy(const char *val, const struct kernel_param *kp)
{
	...
	rcu_barrier();
	pr_info("android_enable_rcu_lazy=%s\n", val ? val : "Y");
	return ret;
#else
	pr_info("Ignoring android_enable_rcu_lazy=%s, CONFIG_RCU_LAZY is not set.\n", val ? val : "Y");
	return 0;
#endif
}
...
{
	return __call_rcu_common(head, func, IS_ENABLED(CONFIG_RCU_LAZY) && READ_ONCE(android_enable_rcu_lazy));
}
