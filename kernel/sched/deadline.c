...
static void update_curr_dl(struct rq *rq)
{
	...
	trace_sched_stat_runtime(donor, delta_exec, 0);
...
throttle:
	if (dl_runtime_exceeded(dl_se) || dl_se->dl_yielded) {
		...
		__dequeue_task_dl(rq, donor, 0);
		if (unlikely(is_dl_boosted(dl_se) || !start_dl_timer(donor)))
			enqueue_task_dl(rq, donor, ENQUEUE_REPLENISH);

		if (!is_leftmost(donor, &rq->dl))
			resched_curr(rq);
	}
