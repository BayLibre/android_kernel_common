...
#define PN_SCHEDSTAT(F) \
	...
	if (schedstat_enabled()) {
		...
		/* cas */
		/* select_task_rq_fair() */
		P_SCHEDSTAT(se.statistics.nr_wakeups_slow_cas_attempts);
		P_SCHEDSTAT(se.statistics.nr_wakeups_slow_cas_count);
		P_SCHEDSTAT(se.statistics.nr_wakeups_slow_prefer_idle);
