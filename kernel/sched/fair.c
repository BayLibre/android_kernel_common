...
{
	if (!sd->shared->overutilized)
		trace_sched_overutilized(sd, true);
...
{
	if (sd->shared->overutilized)
		trace_sched_overutilized(sd, false);
...
static unsigned long group_max_util(struct energy_env *eenv, int cpu_idx, int boosted)
{
	...
	for_each_cpu(cpu, sched_group_span(eenv->sg_cap)) {
		...
		if (unlikely(cpu == eenv->cpu[cpu_idx].cpu_id))
			util += boosted ? eenv->boosted_util_delta : eenv->util_delta;
...
static int find_new_capacity(struct energy_env *eenv, int cpu_idx)
{
	...
	unsigned long util = group_max_util(eenv, cpu_idx, 1);
	int idx, cap_idx;
...
{
	int strict_fbt = sched_feat(FIND_BEST_TARGET) && sched_feat(FBT_STRICT_ORDER);
	struct sched_domain *sd;
	struct sched_group *sg;
	int sd_cpu = -1;
	int cpu_idx, last_cpu_idx;
	int margin;
	...
	if (strict_fbt)
		...
		last_cpu_idx = eenv->max_cpu_count - 1;

	for(cpu_idx = EAS_CPU_NXT; cpu_idx <= last_cpu_idx; cpu_idx++) {
		if (eenv->cpu[cpu_idx].cpu_id < 0)
			continue;
		eenv->cpu[cpu_idx].nrg_delta = eenv->cpu[cpu_idx].energy -
				  ...
				  int cpu, int prev_cpu, int sd_flag)
{
	...
	if (wu) {
		schedstat_inc(p->se.statistics.nr_wakeups_slow_cas_attempts);
		schedstat_inc(this_rq()->eas_stats.slow_cas_attempts);
	}
	...
	while (sd) {
		...
		if (wu)
			schedstat_inc(sd->eas_stats.slow_cas_attempts);
	...
	if (wu && (cas_cpu >= 0)) {
		schedstat_inc(p->se.statistics.nr_wakeups_slow_cas_count);
		schedstat_inc(this_rq()->eas_stats.slow_cas_count);
	}
...
static inline void alloc_eenv(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		eenv->cpu = kmalloc(sizeof(struct _eenv_cpu) * cpu_count, GFP_KERNEL);
		eenv->cpu_array_len = cpu_count;
	}
}
...
static inline void reset_eenv(struct energy_env *eenv)
{
	...
	cpu_len = eenv->cpu_array_len;
	cpu = eenv->cpu;
	memset(eenv, 0, sizeof(struct energy_env));
	eenv->cpu = cpu;
	memset(eenv->cpu, 0, sizeof(struct _eenv_cpu)*num_possible_cpus());
	eenv->cpu_array_len = cpu_len;
...
static inline struct energy_env *get_eenv(struct task_struct *p, int prev_cpu)
{
	...
	eenv->boosted_util_delta = boosted_task_util(p);
...
static int find_energy_efficient_cpu(struct sched_domain *sd, struct task_struct *p, int cpu, int prev_cpu, int sync)
{
	...
	if(!use_fbt) {
		...
		for_each_cpu_and(cpu_iter, &p->cpus_allowed, sched_domain_span(sd)) {
			...
			if (eas_cpu_idx >= eenv->cpu_array_len)
				break;
		}
	} else {
		 ...
		 */
		prefer_idle = sched_feat(MAINLINE_PREFER_IDLE) ?
				0 : (schedtune_prefer_idle(p) > 0);
...
out:
	/* possible values for energy_cpu are:
	 * -1        : either no scenarios to compare or
	 *             no energy model data available
	 * prev_cpu  : found <= 1.56% energy saving (maybe 0)
	 * otherwise : found  > 1.56% energy saving
	 */
	if (energy_cpu == -1) {
...
static inline int wake_energy(struct task_struct *p, int prev_cpu, int sd_flag, int wake_flags)
{
	...
	if(sched_feat(MAINLINE_PREFER_IDLE)){
		...
		if (schedtune_prefer_idle(p) > 0 && !sync) {
			schedstat_inc(p->se.statistics.nr_wakeups_slow_prefer_idle);
			schedstat_inc(this_rq()->eas_stats.slow_prefer_idle);
			return false;
		}
	}
	return true;
