					    ...
					    struct task_group *parent)
{
	...
	/*
	 * Default uclamp hold is 0 ms; but we inherit the effective value from
	 * the parent so that we don't break any restrictions imposed on it and
	 * its child groups.
	 */
	tg->uclamp_hold_ms_req = 0;
	tg->uclamp_hold_jiffies = msecs_to_jiffies(parent->uclamp_hold_ms_req);
#endif
}
