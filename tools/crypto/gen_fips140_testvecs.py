...
def is_string_value(value):
    return (value.isascii() and
            all(c == '\x00' or c.isprintable() for c in str(value, 'utf-8')))
...
# Unfortunately, pycryptodome doesn't support XTS, so for it we need to use a
# different Python package (the "cryptography" package).
...
for alg in SHA_VARIANTS:
    h = hmac.new(hmac_key, message, alg)
    print_value(f'hmac_{alg}_digest', h.digest())
