...
print_help() {
    ...
    echo "It will build kernel and launch virtual device by default, please add additional options to disable them."
    ...
    echo "                        If serial is specified, Android virtual device launch will be skipped"
...
<<<<<<< HEAD   (0982bd ANDROID: ABI: update symbol list for honor)
...
=======
echo "Building KUnit tests according to device $SERIAL_NUMBER ro.product.cpu.abi $ABI ..."
>>>>>>> CHANGE (7aa292 ANDROID: Update README with CONFIG_KUNT=m approach.)
case $ABI in
	arm64*)
        TESTSDIR+="_arm64"
