...
"""This utility generates a single lcov tracefile from a gcov tar file produced from Tradefed."""
...
def generate_lcov_tracefile(
  ...
  A temporay helper script, 'llvm_gcov.sh', is created that points to a prebuilt
  copy of llvm-cov.
  ...
  logging.info(f"Running lcov on {gcov_dir}")
  lcov_cmd = (
      f"{LCOV} -q "
...
def correct_symlinks_in_directory(directory: str, prefix_mappings: {}) -> None:
  ...
  logging.info(f"Fixing up symbolic links in {directory}")
    ...
    for filename in files:
      ...
      if os.path.islink(filepath):
        if not update_symlink_from_mapping(filepath, prefix_mappings):
          logging.error(
              f"Unable to update link at {filepath} with any prefix"
              f" mappings: {prefix_mappings}",
          )
          sys.exit(-1)


def find_most_recent_tarfile(path: str, pattern: str = "*.tar.gz") -> str:
  """Finds the most recent tarfile within a directory or returns a valid tarfile path.
...
def unpack_gcov_tar(file_path: str, output_dir: str) -> str:
  ...
  logging.info(
      f"Unpacking {os.path.basename(file_path)} for test {testname}..."
  )
...
def get_kernel_repo_dir() -> str:
  ...
  # kernel_repo/common/tools/testing/android/bin/create-tracefile.py
...
def build_config() -> {}:
  ...
  config["llvm_cov_prebuilt"] = (
      f'{config["repo_dir"]}/prebuilts/clang/host/linux-x86/llvm-binutils-stable/llvm-cov'
  )
  config["llvm_gcov_sh"] = (
...
def main() -> None:
  ...
  arg_parser.add_argument(
      ...
      required=True,
      help=(
          "Either a path to a gcov tar file or a directory that contains a gcov"
          " tar file(s). The gcov tar file is expected to created from"
  ...
  arg_parser.add_argument(
      ...
      required=False,
      help=(
          "File pattern of a source file(s) to include in generated tracefile."
  ...
  if shutil.which(LCOV) is None:
    logging.error(
        f"{LCOV} is not found and is required for this script. Please"
        " install from:"
    )
  ...
  if not gcno_mappings:
    logging.error(
        "Either 'to' or 'from' fields not found in the"
        f" {config['output_dir']}.",
        file=sys.stderr,
    )
    sys.exit(-1)
  ...
  if tar_file is None:
    logging.error(
        f"Unable to find a gcov tar under {args.tar_location}",
        file=sys.stderr,
    )
    sys.exit(-1)
