...
class Config:
  ...
  def _get_repo_path(self, rel_path: str):
  ...
  @property
  def llvm_gcov_sh_path(self):
    return os.path.join(self.tmp_dir, "tmp", "llvm-gcov.sh")


def build_config(repo_dir: str, llvm_cov_path: str, tmp_dir: str) -> {}:
  """Build configuration.
  ...
  config = {}
  config["repo_dir"] = (
      os.path.abspath(repo_dir) if repo_dir else get_kernel_repo_dir()
  )
