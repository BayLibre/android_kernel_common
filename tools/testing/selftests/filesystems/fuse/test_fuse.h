...
#define TESTFUSELOOKUP(expected, prefilter)				\
	do {								\
		struct fuse_in_header *in_header =			\
				(struct fuse_in_header *)bytes_in;	\
		...
		if (in_header->opcode == FUSE_FORGET) continue;		\
		if (in_header->opcode == FUSE_BATCH_FORGET) continue;	\
		TESTGE(res, sizeof(*in_header));			\
		TESTEQUAL(in_header->opcode,				\
			FUSE_LOOKUP | (prefilter ? FUSE_PREFILTER : 0));\
		TESTEQUAL(res,						\
