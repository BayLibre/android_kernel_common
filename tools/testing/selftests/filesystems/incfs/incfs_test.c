...
static int validate_ranges(const char *mount_dir, struct test_file *file)
{
	...
	if (range_cnt > 128)
		range_cnt = 128;
	if (range_cnt != fba.range_buffer_size_out / sizeof(*ranges) / 2) {
		error = -ERANGE;
		goto out;
	}

	error = TEST_SUCCESS;
	for (i = 0; i < fba.range_buffer_size_out / sizeof(*ranges) - 2; i += 2)
		if (ranges[i] != i * 2 || ranges[i + 1] != i * 2 + 1) {
			error = -EINVAL;
			goto out;
		}

	if (ranges[i] != i * 2 ||
		    (ranges[i + 1] != i * 2 && ranges[i + 1] != i * 2 + 1)) {
		error = -EINVAL;
		goto out;
	}
...
static int emit_partial_test_file_hash(char *mount_dir, struct test_file *file)
{
	...
	fill_block_array[0] = (struct incfs_fill_block) {
		.block_index = 1,
		.data_len = INCFS_DATA_FILE_BLOCK_SIZE,
...
static int validate_hash_ranges(const char *mount_dir, struct test_file *file)
{
	...
	if (fba.range_buffer_size_out != 32) {
		error = -EINVAL;
		goto out;
	}

	if (ranges[0] != (uint64_t) -1 || ranges[1] != (uint64_t) -1 ||
