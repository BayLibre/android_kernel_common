...
static int validate_verity(const char *mount_dir, struct test_file *file)
{
	...
	int fd = -1;
	uint64_t flags;
	uint8_t buffer[sizeof(struct fsverity_digest) + INCFS_MAX_HASH_SIZE];
	struct fsverity_digest *digest = (struct fsverity_digest *) buffer;
...
static int enable_verity_test(const char *mount_dir)
{
	...
	for (i = 0; i < test.files_count; ++i) {
		...
		TESTEQUAL(enable_verity(mount_dir, file, NULL, NULL), 0);
	}
