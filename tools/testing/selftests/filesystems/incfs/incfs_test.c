...
static int sysfs_test_last_error(const char *filename, int page, int error)
{
	int result = TEST_FAILURE;
	FILE *file = NULL;
	uint64_t read_time;
	char read_filename[PATH_MAX];
	int read_page, read_error;

	TEST(file = fopen(SYSFS_DIR "last_error", "re"), file);
	TESTEQUAL(fscanf(file, "(%lu, %s %d, %d)",
			 &read_time, read_filename, &read_page, &read_error),
		  4);
	/* %s reads to next whitespace, so it reads the comma */
	TESTEQUAL(read_filename[strlen(read_filename) - 1], ',');
	read_filename[strlen(read_filename) - 1] = 0;

	if (!filename)
		TESTEQUAL(strcmp(read_filename, "null"), 0);
	else {
		TESTEQUAL(*read_filename, '/');
		TESTEQUAL(strcmp(read_filename + 1, filename), 0);
	}
	TESTEQUAL(read_page, page);
	TESTEQUAL(read_error, error);
...
static int sysfs_test(const char *mount_dir)
{
	...
	TEST(filename = concat_file_name(mount_dir, file.name), filename);
	TEST(fd = open(filename, O_RDONLY | O_CLOEXEC), fd != -1);
	TESTEQUAL(sysfs_test_last_error(NULL, 0, 0), 0);
	TESTEQUAL(sysfs_test_value("reads_failed_timed_out", 0), 0);
	TEST(read(fd, NULL, 1), -1);
	TESTEQUAL(sysfs_test_last_error(file.name, 0, -ETIME), 0);
