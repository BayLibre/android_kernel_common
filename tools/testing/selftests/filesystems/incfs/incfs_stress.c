...
void *reader(void *data)
{
	...
	for (i = 0; i < read_data->num_reads && !cancel_threads; ++i) {
		...
		size_t count = rnd(read_data->filesize - offset,
				&read_data->rng_seed);
...
int write_data(int cmd_fd, int dir_fd, const char *name, size_t size)
{
	...
	for (i = 0; i < block_count; ++i) {
		...
		size_t block_size = size > i * INCFS_DATA_FILE_BLOCK_SIZE ?
		...
		if (written != 1) {
			error = -errno;
			err_msg("Failed to write block %d in file %s",
					i, name);
			break;
		}
	}
...
int main(int argc, char * const *argv)
{
	...
	err = mkdir(src_dir, S_IRUSR | S_IWUSR | S_IXUSR);
	if (err) {
	...
	err = mkdir(dst_dir, S_IRUSR | S_IWUSR | S_IXUSR);
	if (err) {
		err_msg("Failed to make directory %s", src_dir);
		err = -errno;
		goto cleanup;
