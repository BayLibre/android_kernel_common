...
{
	size_t size = sizeof(struct signature_blob) + strlen(add_data);
	...
	*sb = (struct signature_blob) {
		...
		.hash_section = (struct hash_section) {
			.algorithm = INCFS_HASH_TREE_SHA256,
			.log2_blocksize = 12,
			.salt_size = 0,
			.hash_size = SHA256_DIGEST_SIZE,
		},
		.signing_section_size = sizeof(uint32_t)
				+ strlen(add_data) + 1,
	};

	memcpy(sb->hash_section.hash, root_hash, SHA256_DIGEST_SIZE);
	strcpy((char *)sb->signing_section, add_data);
	*buf = sb;
	return size;
}
