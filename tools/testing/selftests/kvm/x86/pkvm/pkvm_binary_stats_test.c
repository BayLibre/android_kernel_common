...
/*
 * pkvm_binary_stats_test
 *
 * Test the fd-based interface for KVM statistics on pKVM protected VMs.
 */
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
static void stats_test(int stats_fd)
{
	...
	for (i = 0; i < header.num_desc; ++i) {
		...
		size_data = max(size_data, pdesc->offset +
...
#define MAX_NUM_VM		64

/*
 * Usage: pkvm_binary_stats_test [#vm] [#vcpu]
 * The first parameter #vm sets the number of VMs being created.
 * The second parameter #vcpu sets the number of VCPUs being created.
 * By default, DEFAULT_NUM_VM VM and DEFAULT_NUM_VCPU VCPU for the VM are
 * created for testing.
 */
int main(int argc, char *argv[])
{
	...
	/* Get the number of VMs and VCPUs that will be created for testing. */
	if (argc > 1) {
		max_vm = strtol(argv[1], NULL, 0);
		if (max_vm <= 0)
			max_vm = DEFAULT_NUM_VM;
		if (max_vm > MAX_NUM_VM) {
			ksft_print_msg("Invalid VM numbers, largest number is %d\n",
				       MAX_NUM_VM);
			return 0;
		}
	}
	...
	for (i = 0; i < max_vm; ++i) {
		vms[i] = vm_create_barebones_type(KVM_X86_PKVM_PROTECTED_VM);
	...
	for (i = 0; i < max_vm; ++i) {
		...
		for (j = 0; j < max_vcpu; ++j) {
			vcpu_stats_fds[j] =
				vcpu_get_stats_fd(vcpus[i * max_vcpu + j]);
