...
int main(int argc, char *argv[])
{
	...
	/*
	 * Create and run a dummy pKVM protected VM that immediately exits to
	 * userspace via GUEST_SYNC, while concurrently migrating the process by
	...
	if (latency >= 0) {
		...
		TEST_ASSERT(fd >= 0,
			    __KVM_SYSCALL_ERROR("open() /dev/cpu_dma_latency", fd));
