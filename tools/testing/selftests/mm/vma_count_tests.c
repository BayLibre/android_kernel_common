...
/* Constants */
static const int TEST_AREA_NR_PAGES = 3;
/* 1 before test area + 1 after test area + 1 after extra mappings */
static const int NR_GUARDS = 3;
static const int TEST_AREA_PROT = PROT_NONE;
static const int EXTRA_MAP_PROT = PROT_NONE;
...
static bool test_suite_setup(void)
{
	...
	if (MAX_VMA_COUNT < 0) {
		perror("Failed to read /proc/sys/vm/max_map_count");
		return false;
	}
	...
	if (base_addr == MAP_FAILED) {
		perror("mmap initial reservation");
		return false;
	}

	if (munmap(base_addr, reservation_size) == -1) {
		perror("munmap initial reservation");
		return false;
	}
