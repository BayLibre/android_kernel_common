...
#include <errno.h>
#include <fcntl.h>
<<<<<<< TARGET BRANCH (0d65e9 UPSTREAM: mm: allow guard regions in file-backed and read-on)
...
TEST_F(guard_regions, uffd)
{
	...
	ASSERT_EQ(ioctl(uffd, UFFDIO_UNREGISTER, &range), 0);
	close(uffd);
>>>>>>> SOURCE BRANCH (458e4d UPSTREAM: selftests/mm: rename guard-pages to guard-regions)
	ASSERT_EQ(munmap(ptr, 10 * page_size), 0);
}
