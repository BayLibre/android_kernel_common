...
{
	return le16toh(q->split.used->idx);
}

static inline void virtio_add_used(struct virtio_queue *q, uint16_t used_idx,
				   uint16_t avail_idx, uint16_t len)
{
	uint16_t desc_idx = q->split.avail->ring[avail_idx & (q->num - 1)];

	used_idx = used_idx & (q->num - 1);
	q->split.used->ring[used_idx].id = desc_idx;
	q->split.used->ring[used_idx].len = htole16(len);
}
...
void virtio_req_complete(struct virtio_req *req, uint32_t len)
{
	...
	q->split.last_avail_idx = avail_idx;

	/*
	 * Triggers the irq whenever there is no available buffer.
	 */
	if (q->split.last_avail_idx == le16toh(q->split.avail->idx))
		send_irq = 1;

	/*
	 * There are two rings: q->split.avail and q->split.used for each of the rx and tx
	 ...
	 * q->split.used and appends buffer to it after consuming it from q->split.avail.
	 *
	 ...
	 * trigger the irq when this item in q->split.used ring is populated."
	 *
	 * Because driver and device are run in two different threads. When
	 * driver sets virtio_get_used_event(q), q->split.used->idx may already be
	 * increased to a larger one. So we need to trigger the irq when
	 * virtio_get_used_event(q) < q->split.used->idx.
	 *
	 * To avoid unnessary irqs for each packet after
	 * virtio_get_used_event(q) < q->split.used->idx, last_used_idx_signaled is
	 * stored and irq is only triggered if
	 * last_used_idx_signaled <= virtio_get_used_event(q) < q->split.used->idx
	 *
	...
	if (send_irq || lkl_vring_need_event(le16toh(virtio_get_used_event(q)),
					     ...
					     q->split.last_used_idx_signaled)) {
		q->split.last_used_idx_signaled = virtio_get_used_idx(q);
 ...
 */
static inline struct lkl_vring_desc *
vring_desc_at_le_idx(struct virtio_queue *q, __lkl__virtio16 le_idx)
{
	return &q->split.desc[le16toh(le_idx) & (q->num - 1)];
}

static inline struct lkl_vring_desc *
vring_desc_at_avail_idx(struct virtio_queue *q, uint16_t idx)
{
	uint16_t desc_idx = q->split.avail->ring[idx & (q->num - 1)];
					    ...
					    uint16_t *idx)
{
	...
	if (q->max_merge_len) {
		if (++(*idx) == le16toh(q->split.avail->idx))
			return NULL;
		desc_idx = q->split.avail->ring[*idx & (q->num - 1)];
		return vring_desc_at_le_idx(q, desc_idx);
	}
 ...
 *    is enabled.
 */
static int virtio_split_process_one(struct virtio_dev *dev, int qidx)
{
	struct virtio_queue *q = &dev->queue[qidx];
	uint16_t idx = q->split.last_avail_idx;
...
void virtio_process_queue(struct virtio_dev *dev, uint32_t qidx)
{
	...
	if (q->packed_ring) {
		// TODO (b/229259231): Adding full packed mode support
	} else {
		while (q->split.last_avail_idx !=
		       le16toh(q->split.avail->idx)) {
			/*
			* Make sure following loads happens after loading
			* q->split.avail->idx.
			*/
			__sync_synchronize();
			if (virtio_split_process_one(dev, qidx) < 0)
				break;
			if (q->split.last_avail_idx ==
			    le16toh(q->split.avail->idx))
				virtio_set_avail_event(q, q->split.avail->idx);
		}
	}
...
{
	virtio_dev_setup_ops(dev, queues, num_max, &virtio_ops);
}

int virtio_dev_setup_ops(struct virtio_dev *dev, int queues, int num_max,
			 const struct lkl_iomem_ops *ops)
{
	...
	dev->base = register_iomem(dev, mmio_size, ops);
	if (!dev->base) {
...
int virtio_dev_cleanup(struct virtio_dev *dev)
{
	...
	// There are cases a device with invalid device id will cause "ENODEV"
	// returned here. To avoid leaking resources, we will ignore that error
	// and continue finishing the cleanup work.
	if (ret < 0 && ret != -LKL_ENODEV)
		return ret;
	...
	ret = lkl_sys_virtio_mmio_device_remove(devname, strlen(devname));
	if (ret < 0)
		return ret;
