...
static void* get_thread_stack_info(lkl_thread_t thread, size_t* stack_size) {
	pthread_attr_t thread_attr;
	void *thread_stack = NULL;

	if (pthread_getattr_np((pthread_t)thread, &thread_attr))
		return NULL;
...
static int kasan_lkl_map_thread_shadow_stack(lkl_thread_t thread)
{
	void *thread_stack = NULL;
	size_t stack_size = 0;

	thread_stack = get_thread_stack_info(thread, &stack_size);
	if (thread_stack == NULL)
		return -1;

	if (kasan_lkl_map_shadow_memory(thread_stack, stack_size) != 0)
		return -1;
...
struct posix_thread_context {
	void (*fn)(void *);
	void *arg;
};

static void *posix_start_thread(void *thread_context)
{
	void (*fn)(void *) =
		((struct posix_thread_context *)thread_context)->fn;
	void *arg = ((struct posix_thread_context *)thread_context)->arg;
	free(thread_context);

#ifdef LKL_CONFIG_KASAN
	if (kasan_lkl_map_thread_shadow_stack(thread_self()) != 0)
		panic();
#endif
...
{
	pthread_t thread;
	// thread_context should be freed in the new thread
	struct posix_thread_context *thread_context =
		malloc(sizeof(struct posix_thread_context));
	if (thread_context == NULL)
		return 0;

	thread_context->fn = fn;
	thread_context->arg = arg;
	if (WARN_PTHREAD(pthread_create(&thread, NULL, posix_start_thread,
					thread_context))) {
		free(thread_context);
		return 0;
	}
...
#if defined(LKL_HOST_CONFIG_VFIO_PCI)
extern struct lkl_dev_pci_ops vfio_pci_ops;
#elif  defined(LKL_HOST_CONFIG_LKL_PCI)
extern struct lkl_dev_pci_ops lkl_pci_ops;
#endif // LKL_HOST_CONFIG_VFIO_PCI
...
struct lkl_host_operations lkl_host_ops = {
	...
	.print = print,
	.mem_alloc = lkl_malloc,
	.mem_free = lkl_free,
	.page_alloc = page_alloc,
	.page_free = page_free,
#ifdef LKL_CONFIG_KASAN
	.kasan_init_shadow_memory = kasan_lkl_init_shadow_memory,
	.kasan_release_shadow_memory = kasan_lkl_release_shadow_memory,
	.kasan_map_shadow_memory = kasan_lkl_map_shadow_memory,
	.kasan_map_thread_shadow_stack = kasan_lkl_map_thread_shadow_stack,
#endif
	.ioremap = lkl_ioremap,
