...
struct virtio_queue {
	...
	union {
		...
		struct {
			struct lkl_vring_packed_desc *desc;
			struct lkl_vring_packed_desc_event *driver_event;
			struct lkl_vring_packed_desc_event *device_event;
			uint16_t ring_wrap_counter;
			uint16_t last_avail_idx;
		} packed;
	};
};
...
int virtio_dev_setup_ops(struct virtio_dev *dev, int queues, int num_max,
			 const struct lkl_iomem_ops *virtio_ops);
