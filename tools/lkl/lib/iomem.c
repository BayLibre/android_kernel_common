...
#define IOMEM_ADDR_TO_INDEX(addr) (((uintptr_t)addr) >> IOMEM_OFFSET_BITS)
#define IOMEM_ADDR_TO_OFFSET(addr)                                             \
	(((uintptr_t)addr) & ((1 << IOMEM_OFFSET_BITS) - 1))
#define IOMEM_INDEX_TO_ADDR(i) (void *)(((uintptr_t)i) << IOMEM_OFFSET_BITS)
...
{
	unsigned int index = IOMEM_ADDR_TO_INDEX(addr);
...
int lkl_iomem_access(const volatile void *addr, void *res, int size, int write)
{
	unsigned int index = IOMEM_ADDR_TO_INDEX(addr);
	struct iomem_region *iomem = &iomem_regions[index];
	unsigned int offset = IOMEM_ADDR_TO_OFFSET(addr);
	int ret;

	if (index > MAX_IOMEM_REGIONS || !iomem_regions[index].ops ||
	    offset + size > iomem_regions[index].size) {
#ifndef LKL_HOST_CONFIG_LKL_FUZZING
		return -1;
#else
		lkl_printf(
			"OOB memory access in lkl_iomem at address %px, size %x, write %d\n",
			addr, size, write);
		lkl_host_ops.panic();
#endif
	}
