...
// Malloc a memory buffer of the requested size. If KASan is enabled also
// map and unpoison a corresponding shadow memory region. This routine should be
// used to allocate buffers that may be passed to LKL library and it should be
// executed strictly after lkl_start_kernel routine and before lkl_stop_kernel.
void *lkl_malloc(unsigned long size);

// Free a memory buffer. If KASan is enabled also poison the corresponding
// shadow memory region. This routine should be executed strictly after
// lkl_start_kernel and before lkl_stop_kernel.
