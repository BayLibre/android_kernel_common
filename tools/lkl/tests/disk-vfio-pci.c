...
static const char *_lkl_pci_bus = "lkl_pci";

LKL_TEST_CALL(mount_dev, lkl_mount_blkdev, 0, LKL_MKDEV(259, 0), cla.fstype, 0,
	      NULL, mnt_point, sizeof(mnt_point))
...
LKL_TEST_CALL(add_lkl_pci_bus, lkl_sys_lkl_pci_bus_add, 0, _lkl_pci_bus,
	      strlen(_lkl_pci_bus) + 1);
LKL_TEST_CALL(remove_lkl_pci_bus, lkl_sys_lkl_pci_bus_remove, 0, _lkl_pci_bus,
	      strlen(_lkl_pci_bus) + 1);
LKL_TEST_CALL(stop_kernel, lkl_sys_halt, 0);

struct lkl_test tests[] = {
	LKL_TEST(start_kernel),
	LKL_TEST(add_lkl_pci_bus),
	LKL_TEST(mount_dev),
