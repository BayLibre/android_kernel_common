...
void __init arch_mm_preinit(void)
{
	...
	if (IS_ENABLED(CONFIG_DMA_BOUNCE_UNALIGNED_KMALLOC) && !swiotlb &&
		...
		/*
		 * No 32-bit DMA bouncing is needed, either because all of DRAM
		 * is within the 32-bit limit or because all of it starts above
		 * it.  Non-coherent hardware still needs cache-line aligned
		 * bounce buffers for kmalloc(), so allocate 1MB per 1GB of RAM,
		 * and allow the buffer itself to live in high memory for the
		 * latter case.
		 */
		unsigned long size =
