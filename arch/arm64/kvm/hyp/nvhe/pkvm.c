...
static inline struct shadow_vcpu_state *get_shadow_state(struct kvm_vcpu *shadow_vcpu)
{
	return container_of(shadow_vcpu, struct shadow_vcpu_state, vcpu);
}
...
int __pkvm_teardown_shadow(int shadow_handle)
{
	...
	for (i = 0; i < hyp_nr_cpus; i++) {
		...
		if (vcpu) {
			struct shadow_vcpu_state *vcpu_state =
				get_shadow_state(vcpu);
