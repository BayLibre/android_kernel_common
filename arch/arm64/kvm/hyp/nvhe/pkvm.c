...
static bool exists_shadow(struct kvm *host_kvm)
{
	...
	for (i = 0; i < KVM_MAX_PVMS && num_checked < num_shadow_entries; i++) {
		struct kvm_shadow_vm *vm = READ_ONCE(shadow_table[i]);
...
static int generate_shadow_handle(void)
{
	...
	while (READ_ONCE(shadow_table[next_shadow_alloc]))
 ...
 */
static void publish_shadow_table(struct kvm_shadow_vm *vm)
{
