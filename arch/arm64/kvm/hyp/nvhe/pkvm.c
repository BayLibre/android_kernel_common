...
/*
 * Array of host fp state per cpu. This could include the host simd state, as
 * well as the sve and sme states if supported. Written to when the guest
 * accesses its own FPSIMD state, and read when the guest state is live and we
 * need to switch back to the host.
 *
 * Only valid when (fp_state == FP_STATE_GUEST_OWNED) in the hyp vCPU structure.
 */
struct pkvm_host_fp_state *host_fp_states;
...
{
	WARN_ON(host_fp_states);
	host_fp_states = host_fp;
}
