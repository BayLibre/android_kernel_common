...
static int __pkvm_host_use_dma_page(phys_addr_t phys_addr)
{
	 ...
	 *   refcounted, taint them by marking them as shared, and that will
	 *   prevent any future donation, this is too restrictive at the moment,
	 *   which can be improved with more infrastructure to track MMIO pages
	 *   that ideally comes with device assignment support.
	 */
	if (!addr_is_memory(phys_addr)) {
		...
		if (pte && !kvm_pte_valid(pte))
			return -EPERM;
		prot = pkvm_mkstate(KVM_HOST_S2_DEFAULT_MMIO_PTE, PKVM_PAGE_SHARED_OWNED);
