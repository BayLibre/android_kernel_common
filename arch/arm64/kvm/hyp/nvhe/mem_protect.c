...
static void *__fixmap_guest_page(phys_addr_t phys, size_t *size)
{
	void *addr;
...
static void clean_dcache_guest_page(void *va, size_t size)
{
	while (size) {
		...
		void *addr = __fixmap_guest_page(__hyp_pa(va), &__size);
...
void hyp_poison_page(phys_addr_t phys, size_t size)
{
	...
	while (size) {
		size_t __size = size == PMD_SIZE ? size : PAGE_SIZE;
		void *addr = __fixmap_guest_page(phys, &__size);
