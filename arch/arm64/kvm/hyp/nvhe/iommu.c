...
{
	return atomic_read_acquire(&iommu_finalized) != 0;
}

static bool set_iommu_finalized(void)
{
	return atomic_cmpxchg_release(&iommu_finalized, 0, 1) == 0;
}
...
{
	return set_iommu_finalized() ? 0 : -EPERM;
}
