...
static void kvm_notify_vm_availability(uint16_t vm_handle, struct kvm_ffa_buffers *ffa_buf,
...
void kvm_guest_notify_availability(struct pkvm_hyp_vm *vm, u32 availability_msg)
{
	hyp_spin_lock(&kvm_ffa_hyp_lock);
	kvm_notify_vm_availability(hyp_vcpu_to_ffa_handle(vm->vcpus[0]), &vm->ffa_buf,
				   availability_msg);
	hyp_spin_unlock(&kvm_ffa_hyp_lock);
}
