...
static int ffa_guest_share_with_cb(struct pkvm_hyp_vcpu *vcpu,
				   int (*share_cb)(struct pkvm_hyp_vcpu *, u64, u64, u64 *),
				   phys_addr_t guest_ipa, void **out_addr, u64 *exit_code)
{
	int ret = share_cb(vcpu, guest_ipa, 1, (u64 *)out_addr);
				 ...
				 u64 *exit_code)
{
	...
	return 0;
err_unshare_tx:
	WARN_ON(__pkvm_guest_unshare_hyp(vcpu, tx_ipa, 1));
	return ret;
}
			      ...
			      unsigned int vm_handle)
{
	...
	if (FIELD_GET(FFA_SRC_ENDPOINT_MASK, id) != vm_handle) {
		ret = FFA_RET_INVALID_PARAMETERS;
		goto out;
	}
	...
	if (vm_handle == HOST_FFA_ID) {
		...
		WARN_ON(__pkvm_guest_unshare_hyp(pkvm_vcpu, endp_buffers[vm_handle].tx_ipa, 1));
		WARN_ON(__pkvm_guest_unshare_hyp(pkvm_vcpu, endp_buffers[vm_handle].rx_ipa, 1));
	}
				    ...
				    struct ffa_mem_transfer *transfer)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		WARN_ON(!translation);
		WARN_ON(__pkvm_guest_unshare_ffa(vcpu, translation->ipa, 1));
				  ...
				  u64 *exit_code)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		for (j = 0; j < range->pg_cnt; j++) {
			...
			if (ret) {
				WARN_ON(__pkvm_guest_unshare_ffa(vcpu, ipa, 1));
				goto unshare;
			}
...
static int __do_ffa_mem_xfer(const u64 func_id, struct arm_smccc_res *res,
			     struct kvm_cpu_context *ctxt, unsigned int vm_handle,
			     u64 *exit_code)
{
...
static void kvm_guest_clear_transfer(struct ffa_mem_transfer *transfer, struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	list_for_each_entry_safe(translation, tmp, &transfer->translations, node) {
		WARN_ON(__pkvm_guest_unshare_ffa(hyp_vcpu, translation->ipa, 1));
		list_del(&translation->node);
		hyp_free(translation);
	}
}
...
int kvm_reclaim_ffa_guest_pages(struct pkvm_hyp_vm *vm, pkvm_handle_t handle)
{
	...
	if (endp_buffers[vm_handle].tx) {
		WARN_ON(__pkvm_guest_unshare_hyp(hyp_vcpu, endp_buffers[vm_handle].tx_ipa, 1));
		endp_buffers[vm_handle].tx = NULL;
	}

	if (endp_buffers[vm_handle].rx) {
		WARN_ON(__pkvm_guest_unshare_hyp(hyp_vcpu, endp_buffers[vm_handle].rx_ipa, 1));
		endp_buffers[vm_handle].rx = NULL;
	}
unlock:
	hyp_spin_unlock(&hyp_buffers.lock);
