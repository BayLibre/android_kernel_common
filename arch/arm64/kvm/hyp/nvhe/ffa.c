			   ...
			   struct kvm_cpu_context *ctxt)
{
	...
	if (has_version_negotiated) {
		 ...
		 * with earlier minor per DEN0077A 1.2 REL0 13.2.1 but FF-A v1.2 makes a
		 * breaking change by switching the calling convention to SMCCC 1.2.
		 *
		 ...
		 *
		 * However, the hypervisor has already negotiated a version with the firmware
		 * and cannot be changed for the lifetime of the hypervisor (DEN0077A 1.2 REL0
		 * 13.2.1).
		 *
		 * Do not let the host downgrade the hypervisor when it requires a switch
