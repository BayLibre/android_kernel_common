...
bool kvm_host_ffa_handler(struct kvm_cpu_context *ctxt, u32 func_id)
{
	...
	case FFA_FN64_RXTX_MAP:
		...
		break;
	case FFA_RXTX_UNMAP:
		do_ffa_rxtx_unmap(&res, ctxt, 0);
		break;
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, 0, NULL);
		break;
	case FFA_MEM_RECLAIM:
		do_ffa_mem_reclaim(&res, ctxt, 0);
		break;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, ctxt, 0, NULL);
		break;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&res, ctxt, 0);
		break;
	default:
