...
out_handled:
	ffa_to_smccc_res_prop(regs, ret, prop);
}
...
static void do_ffa_direct_msg2(struct arm_smccc_1_2_regs *res,
			       struct kvm_cpu_context *ctxt,
			       u64 vm_handle)
{
	DECLARE_REG(u32, endp, ctxt, 1);
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	case FFA_MSG_SEND_DIRECT_REQ2:
		if (hyp_ffa_version >= FFA_VERSION_1_2) {
			do_ffa_direct_msg2(&res, ctxt, hyp_vcpu_to_ffa_handle(hyp_vcpu));
			goto out_guest;
		}
		fallthrough;
	default:
