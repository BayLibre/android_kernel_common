...
/* Helpers*/
#define KASAN_SHADOW_ARR_SIZE		((KASAN_SHADOW_PAGES * PAGE_SIZE) / sizeof(kasan_hyp_t))
#define KASAN_VA_BYTES_PER_BYTE		(KASAN_VA_BYTES_PER_BIT << 3)
#define KASAN_SHADOW_VA_COVERED		(KASAN_SHADOW_PAGES * PAGE_SIZE * KASAN_VA_BYTES_PER_BYTE)
...
static inline bool kasan_check_range(u64 off_start, u64 off_end)
{
	...
	if (start == end)
		return (shadow_alloc[start] & GENMASK(bit_end, bit_start)) ==
			GENMASK(bit_end, bit_start);

	if ((shadow_alloc[start] & GENMASK(KASAN_SHADOW_ENTRY_LAST_BIT, bit_start))
	    != GENMASK(KASAN_SHADOW_ENTRY_LAST_BIT, bit_start))
		return false;
	start++;
