			   ...
			   unsigned long sva_desc_hva)
{
	...
	ret = kvm_iommu_ops->sva_bind_dev(iommu, domain, sid, pasid, sva_desc_hva);
	if (ret)
		domain_put(domain);
	return ret;
}
...
int kvm_iommu_page_response(pkvm_handle_t iommu_id, pkvm_handle_t domain_id, u32 endpoint_id,
			    unsigned long page_response_desc_hva)
{
	...
	kvm_iommu_ops->page_response(iommu, domain, endpoint_id, page_response_desc_hva);
	domain_put(domain);
	return 0;
