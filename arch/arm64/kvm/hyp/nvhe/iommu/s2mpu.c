...
struct s2mpu_version_ops {
	...
	u32 (*host_mmio_reg_access_mask)(struct pkvm_iommu *dev, size_t off, bool is_write);
};
...
/* check if s2mpu version compatible with the configured driver version */
static inline bool is_version_compatible(enum s2mpu_version other_version)
{
	if (this_version == other_version)
		return true;
	if (((this_version == S2MPU_VERSION_1) && (other_version == S2MPU_VERSION_2))   ||  ((other_version == S2MPU_VERSION_2) && (this_version == S2MPU_VERSION_1)))
		return true;
	return false;
}
...
static u32 host_mmio_reg_access_mask_v9(struct pkvm_iommu *dev, size_t off,
				     bool is_write)
{
	...
	/*some registers have per cache instance which can be determined at runtime*/
	ptlb_num = FIELD_GET(V9_READ_PMMU_INFO_NUM_PTLB, readl_relaxed(dev->va + REG_NS_V9_PMMU_INFO));
	if (off >= REG_NS_V9_PMMU_PTLB_INFO(0) && off < REG_NS_V9_PMMU_PTLB_INFO(ptlb_num))
		return read_only&V9_READ_PMMU_PTLB_INFO_MASK;

	stlb_num = FIELD_GET(V9_SWALKER_INFO_NUM_STLB_MASK, readl_relaxed(dev->va + REG_NS_V9_SWALKER_INFO));
	if (off >= REG_NS_V9_STLB_INFO(0) && off < REG_NS_V9_STLB_INFO(stlb_num))
		return read_only&V9_READ_SLTB_INFO_MASK;

	return no_access;
}
static u32 host_mmio_reg_access_mask_v1_v2(struct pkvm_iommu *dev, size_t off,
				     bool is_write)
{
	const u32 no_access = 0;
	const u32 read_write = (u32)(-1);
...
static u32 host_mmio_reg_access_mask(struct pkvm_iommu *dev, size_t off, bool is_write)
{
	const u32 no_access  = 0;
	const u32 read_write = (u32)(-1);
	const u32 read_only  = is_write ? no_access  : read_write;
	...
	return version_ops->host_mmio_reg_access_mask(dev, off, is_write);
	}
				    ...
				    u32 esr, size_t off)
{
	...
	mask = host_mmio_reg_access_mask(dev, off, is_write);
	if (!mask)
		return false;
...
/* operations that differ between version  */
