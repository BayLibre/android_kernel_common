...
unsigned long pkvm_priv_hcall_limit __ro_after_init = KVM_HOST_SMCCC_FUNC(__pkvm_prot_finalize);

int reset_pkvm_priv_hcall_limit(void)
{
	unsigned long *addr;

	if (pkvm_priv_hcall_limit == KVM_HOST_SMCCC_FUNC(__pkvm_prot_finalize))
		return -EACCES;
