...
static void flush_protected_debug_state(struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	if (FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_PMSVer), id_aa64dfr0_el1_sys_val) &&
	    (read_sysreg_s(SYS_PMBLIMITR_EL1) & BIT(PMBLIMITR_EL1_E_SHIFT)))
		vcpu_set_flag(vcpu, DEBUG_STATE_SAVE_SPE);
	else
		vcpu_clear_flag(vcpu, DEBUG_STATE_SAVE_SPE);

	if (FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_TraceBuffer), id_aa64dfr0_el1_sys_val) &&
