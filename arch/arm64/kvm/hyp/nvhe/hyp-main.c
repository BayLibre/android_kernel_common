...
static void handle___pkvm_toogle_smc_trapping(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(pkvm_handle_t, handle, host_ctxt, 1);
	DECLARE_REG(bool, state, host_ctxt, 2);

	cpu_reg(host_ctxt, 1) = pkvm_toogle_smc_trapping_by_handle(handle, state);
}
...
static const hcall_t host_hcall[] = {
	...
	HANDLE_FUNC(__pkvm_toogle_smc_trapping),
};
