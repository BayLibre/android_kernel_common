...
struct pkvm_device *pkvm_get_vm_device_by_addr(struct pkvm_hyp_vm *hyp_vm, u64 phys)
{
	int i;
...
int pkvm_device_register_cookie(struct pkvm_device *dev, void *cookie)
{
	if (!cookie)
		return 0;
...
static int
__pkvm_device_request_power(struct pkvm_hyp_vcpu *hyp_vcpu, u64 ipa, bool on, bool handler)
{
...
bool pkvm_device_request_power(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	struct kvm_vcpu *vcpu = &hyp_vcpu->vcpu;
	u64 ipa = smccc_get_arg1(vcpu);
	bool on = smccc_get_arg2(vcpu);
	int ret;

	if (smccc_get_arg3(vcpu))
		goto out_guest_err;

	/* When powering-on, power_handler is called after forwarding to shot  */
	ret = __pkvm_device_request_power(hyp_vcpu, ipa, on, !on);
	if (ret == -ENOENT) {
		/* pkvm_get_guest_pa_request() has generated a hyp_req */
		write_sysreg_el2(read_sysreg_el2(SYS_ELR) - 4, SYS_ELR);
		*exit_code = ARM_EXCEPTION_HYP_REQ;
		return false;
	}
...
{
	struct kvm_vcpu *vcpu = &hyp_vcpu->vcpu;
	bool on = smccc_get_arg1(vcpu);
	u64 ipa = smccc_get_arg2(vcpu);

	/* When powering-off, power_handler is called before forwarding to host */
	if (!on)
		return 0;

	return __pkvm_device_request_power(hyp_vcpu, ipa, on, true);
}
