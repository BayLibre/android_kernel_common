...
int pkvm_device_hyp_assign_mmio(u64 pfn)
{
	...
	/* Hyp have nc, while host may have issue cacheable writes.*/
	if (!ret)
		kvm_flush_dcache_to_poc(pfn >> PAGE_SHIFT, PAGE_SIZE);
...
static int pkvm_device_reset(struct pkvm_device *dev, bool host_to_guest)
{
	...
	return dev->reset_handler(dev, host_to_guest);
}
...
int pkvm_host_map_guest_mmio(struct pkvm_hyp_vcpu *hyp_vcpu, u64 pfn, u64 gfn)
{
	...
	/*
	 * We expect device MMIO to be MMIO guarded before mapped.
	 * That makes it clear that the guest knows these are MMIO pages
	 * and it's not tricked to access memory.
	 */
	ret = __pkvm_remove_ioguard_page(hyp_vcpu, gfn << PAGE_SHIFT, 1, &nr_unguarded);
	if (ret)
		goto out_ret;
...
int pkvm_devices_get_context(u64 iommu_id, u64 endpoint)
{
	struct pkvm_device *dev = pkvm_get_device_by_iommu(iommu_id, endpoint);
	int ret = 0;
...
void pkvm_devices_put_context(u64 iommu_id, u64 endpoint)
{
	struct pkvm_device *dev = pkvm_get_device_by_iommu(iommu_id, endpoint);
...
int pkvm_device_register_reset(u64 phys, int (*cb)(struct pkvm_device *, bool))
{
