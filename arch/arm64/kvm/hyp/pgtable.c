			   ...
			   enum kvm_pgtable_walk_flags visit)
{
	...
	struct snapshot_data *data = ctx->arg;

	if (kvm_pte_table(ctx->old, ctx->level)) {
		copy_table = mm_ops->zalloc_page(data->mc);
		if (!copy_table)
			return -ENOMEM;
				...
				size_t src_pgd_len)
{
	...
	if (!to_pgt->pgd || dest_pgt->pgd_pages < (src_pgd_len >> PAGE_SHIFT))
		return -EINVAL;

	memcpy(to_pgt->pgd, src_pgt->pgd, src_pgd_len);
