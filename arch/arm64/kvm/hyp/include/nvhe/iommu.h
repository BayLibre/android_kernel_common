 ...
 *  which is expected to hold all the unmapped physicall the addresses so the
 ...
 * @init: init the driver called once before the kernel de-privilige
 * @get_iommu_by_id: Return kvm_hyp_iommu from an ID passed from the kernel.
 * 		     It is a driver specific how the driver assign IDs.
 ...
 * @dabt_handler: data abort for MMIO, can be used for emulation access to IOMMU.
 ...
 * @iova_to_phys: get phy address from IOVA in a domain.
 */
