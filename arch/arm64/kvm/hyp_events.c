				...
				struct hyp_event_id *event_id, int nr_event_ids)
{
	...
	for (int i = 0; i < nr_events; event++, event_id++, i++) {
		/*
...
int hyp_trace_init_mod_events(struct hyp_event *event, int nr_events,
			      struct hyp_event_id *event_id, int nr_event_ids)
{
	struct hyp_event_table *tables;
	int ret, i;

	ret = hyp_event_table_init(event, nr_events, event_id, nr_event_ids);
	if (ret)
		return ret;
