...
static struct hyp_event *__hyp_trace_find_event_name(const char *name,
...
static struct hyp_event *hyp_trace_find_event_name(const char *name)
{
	struct hyp_table *table;
...
int hyp_trace_init_mod_events(struct pkvm_el2_module *mod)
{
	...
	for (i = 0; i < nr_fmts; i++, hyp_printk_fmt_offsets++)
		*hyp_printk_fmt_offsets = nr_fmts;
