...
static char hyp_ftrace_regex[128];

static __init int setup_hyp_ftrace_regex_early(char *str)
{
	strscpy(hyp_ftrace_regex, str, sizeof(hyp_ftrace_regex));

	return 1;
}
__setup("hyp_ftrace_filter=", setup_hyp_ftrace_regex_early);
...
static bool hyp_ftrace_func_match(unsigned long kern_addr)
{
	char sym[KSYM_SYMBOL_LEN];
	char *modname;

	if (!strlen(hyp_ftrace_regex))
		return true;

	kallsyms_lookup(kern_addr, NULL, NULL, &modname, sym);

	return glob_match(hyp_ftrace_regex, sym);
}

static int hyp_ftrace_funcs_apply_filter(void)
{
	bool match = false;
	void *func_pg;
	...
	for_each_funcs_pg(func_pg) {
		...
		for_each_func(func_pg, func) {
			if (hyp_ftrace_func_match(func_disable(*func))) {
				*func = func_enable(*func);
				match = true;
			} else {
				*func = func_disable(*func);
			}
		}
	}
...
hyp_ftrace_filter_write(struct file *filp, const char __user *ubuf, size_t cnt,
			loff_t *ppos)
{
	char *regex = hyp_ftrace_regex;
	int ret;

	if (cnt >= (sizeof(hyp_ftrace_regex) - 1))
		return -E2BIG;

	ret = strncpy_from_user(regex, ubuf, sizeof(hyp_ftrace_regex));
	if (ret < 0)
		return ret;

	regex[cnt - 1] = '\0';

	ret = hyp_ftrace_funcs_apply_filter();
	if (ret)
		return ret;
...
static int hyp_ftrace_filter_open(struct inode *node, struct file *file)
{
	int ret = single_open(file, hyp_ftrace_filter_show, NULL);
...
>>>>>>> 42d8a69fe234 (ANDROID: KVM: arm64: Add set_ftrace_filter for pKVM hyp)
				  ...
				  unsigned long hyp_kern_offset, bool clear)
{
	...
	while (func < funcs_end) {
		...
		enable = hyp_ftrace_func_match(kern_addr);
...
#ifdef CONFIG_PROTECTED_NVHE_FTRACE
	tracefs_create_file("set_ftrace_filter", 0600, parent, NULL,
