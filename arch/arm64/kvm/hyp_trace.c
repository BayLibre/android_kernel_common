...
{
	int cpu = iter->cpu == RING_BUFFER_ALL_CPUS ? iter->ent_cpu : iter->cpu;

	trace_seq_printf(&iter->seq, "[%03d]\t", cpu);
}
...
static void ht_total_entries(int cpu, unsigned long *entries,
...
static int ht_show(struct seq_file *m, void *v)
{
	...
	if (!iter->ent) {
		...
		ht_total_entries(iter->cpu, &entries, &overrun);
