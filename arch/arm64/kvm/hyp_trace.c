...
static int hyp_free_tracing(void)
{
	int ret;
			       ...
			       size_t cnt, loff_t *ppos)
{
	...
	if (hyp_trace_buffer)
		pr_info("Clear the hyp trace buffer to take into account the new size\n");
	mutex_unlock(&hyp_trace_lock);
...
static int ht_print_trace_fmt(struct ht_iterator *iter)
{
	...
	return trace_seq_has_overflowed(&iter->seq) ? -1 : 0;
};
