...
{
	unsigned long reclaimed, total = 0;
	enum hyp_alloc_mgt_id id;

	for (id = __HYP_ALLOC_MGT_HEAP_ID_START__; id < NR_ALLOC_MGT_IDS; id++) {
		reclaimed = __pkvm_reclaim_hyp_alloc_mgt_id(id, nr_pages);
		nr_pages -= reclaimed;
		total += reclaimed;
	}
