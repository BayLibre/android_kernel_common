					      ...
					      char *module_path)
{
	...
	if (module_path) {
		module_path = kstrdup(module_path, GFP_KERNEL);
		if (!module_path)
			return -ENOMEM;
	}
	...
	argv = kmalloc(sizeof(char *) * (module_name ? 7 : 5), GFP_KERNEL);
	if (!argv)
		goto err_free_strdup;
...
static int __init pkvm_request_early_module(char *module_name, char *module_path)
{
	...
	pr_info("Fallback to default pkvm module path\n");

	return __pkvm_request_early_module(module_name, NULL);
}

int __init pkvm_load_early_modules(void)
{
	char *token, *buf = early_pkvm_modules;
	char *module_path = CONFIG_MODULE_PATH;
	int err;
