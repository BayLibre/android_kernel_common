...
static int __pkvm_host_map_guest_sglist(struct kvm *kvm, struct list_head *ppages)
{
...
static int __pkvm_host_map_guest(struct kvm *kvm, struct list_head *ppages)

{
	struct kvm_pinned_page *ppage, *tmp;

	if (ppages->next != ppages->prev && kvm->arch.pkvm.enabled)
		return __pkvm_host_map_guest_sglist(kvm, ppages);
	...
	list_for_each_entry_safe(ppage, tmp, ppages, list_node) {
		...
		ret = kvm_call_hyp_nvhe(__pkvm_host_map_guest, pfn, gfn,
					1 << ppage->order, KVM_PGTABLE_PROT_R);
		/*
			  ...
			  struct kvm_memory_slot *memslot)
{
	...
	ret = __pkvm_host_map_guest(kvm, &ppages);
