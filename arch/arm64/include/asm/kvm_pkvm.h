...
static inline size_t pkvm_host_fp_state_size(unsigned int vl)
{
	if (system_supports_sve())
		return sizeof(struct kvm_host_sve_state) +
		       SVE_SIG_REGS_SIZE(sve_vq_from_vl(vl));
	else
		return sizeof(struct user_fpsimd_state);
}

static inline unsigned long hyp_host_fp_pages(unsigned long nr_cpus, unsigned int vl)
{
	return PAGE_ALIGN(nr_cpus * pkvm_host_fp_state_size(vl)) >> PAGE_SHIFT;
}
