 ...
 * 				before/after devices are assigned where direction is
 * 				passed in the callback host_to_guest. Only one callback
 * 				can be registered per device.
 * 				Devices are identified by the base address of the MMIO
 * 				as defined in the device tree.
 * 				Reset is expected to clear any state/secretes on the
