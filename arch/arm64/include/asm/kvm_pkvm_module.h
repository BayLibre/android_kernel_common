 ...
 * @register_unmask_serror:	When @unmask and @mask return True, the
 *				hypervisor unmask and catch SErrors while
 *				running. It is the role of those callbacks to
 *				handle synchronisation. Intends to help
 *				debugging when such SErrors are raised. The
 *				hypervisor has no mechanism to recover and will
 *				panic if any occurs.
...
struct pkvm_module_ops {
	...
	int (*register_unmask_serror)(bool (*unmask)(void), bool (*mask)(void));
