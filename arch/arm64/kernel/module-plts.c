...
static void extract_rht_key(const Elf64_Rela* rela, struct Elf64_Rela_rht_key* rht_key) {
...
static int update_rht_and_plt_count(const Elf64_Rela* rela, struct rhashtable *elf64_rela_rht, int *plt_count) {
	...
	if (lookup_result == NULL) {
		(*plt_count)++;
		insert_node = kzalloc(sizeof(struct Elf64_Rela_rht_node), GFP_KERNEL);
		if (!insert_node) {
			return -ENOMEM;
		}
		extract_rht_key(rela, &insert_node->key);	
		return rhashtable_insert_fast(elf64_rela_rht, &insert_node->rhnode, elf64_rela_rht_params);
	}
			       ...
			       Elf64_Word dstidx, Elf_Shdr *dstsec)
{
	...
	for (i = 0; i < num; i++) {
		...
		case R_AARCH64_CALL26:
			...
			err = update_rht_and_plt_count(rela, &elf64_rela_rht, &plt_count);
			if (err < 0)
