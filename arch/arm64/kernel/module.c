			   ...
			   struct module *mod)
{
	...
	hyp_mod->bss = (struct pkvm_module_section) {
		.start	= (void *)s->sh_addr,
		.end	= (void *)s->sh_addr + s->sh_size,
	};
	...
	hyp_mod->rodata = (struct pkvm_module_section) {
		.start	= (void *)s->sh_addr,
		.end	= (void *)s->sh_addr + s->sh_size,
	};
	...
	hyp_mod->data = (struct pkvm_module_section) {
		.start	= (void *)s->sh_addr,
		.end	= (void *)s->sh_addr + s->sh_size,
	};
	...
	hyp_mod->relocs = (void *)s->sh_addr;
	hyp_mod->nr_relocs = s->sh_size / sizeof(*hyp_mod->relocs);
