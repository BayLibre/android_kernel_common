			     ...
			     u64 *nr_done)
{
	...
	*nr_done = memshare_has_range ? res.a1 : 1;
...
static int set_memory_xcrypted(u32 func_id, unsigned long start, int numpages)
{
	...
	while (nr_granules > 0) {
		...
		ret = __invoke_memshare(start, nr_granules, func_id, &nr_done);
		if (ret)
			return ret;

		WARN_ON(nr_done > nr_granules);

		nr_granules -= nr_done;
		start += nr_done * memshare_granule_sz;
	}
