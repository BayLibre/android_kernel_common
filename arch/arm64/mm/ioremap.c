...
void kvm_init_ioremap_services(void)
{
	...
	if (granule > PAGE_SIZE) {
		pr_warn("KVM MMIO guard granule (%lu) is bigger than page (%lu)\n",
			granule, PAGE_SIZE);
		return;
	}

	if (!granule || (granule & (granule - 1))) {
		pr_warn("KVM MMIO guard granule (%lu) should be power of 2\n",
			granule);
		return;
	}
...
void ioremap_phys_range_hook(phys_addr_t phys_addr, size_t size, pgprot_t prot)
{
	...
	guard_mask = ~(guard_granule-1);
	VM_BUG_ON(phys_addr & ~guard_mask || size & ~guard_mask);
