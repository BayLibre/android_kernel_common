				      ...
				      unsigned long pfn, int *target_level)
{
	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			pkvm_switch_host_ept_ro(donation->pages[donation->nr_pages], PAGE_SIZE);
			       ...
			       unsigned long last_pfn)
{
	...
	do {
		...
		if (level < retain_level && !(start_pfn > level_pfn ||
			...
			PKVM_ASSERT(donation->nr_pages <= PKVM_MAX_IOMMU_PAGE_DONATION);
			pkvm_switch_host_ept_default(virt_to_phys(level_pte), PAGE_SIZE);
		}
next:
...
out:
	...
	PKVM_ASSERT(donation->nr_pages <= PKVM_MAX_IOMMU_PAGE_DONATION);
	pkvm_switch_host_ept_default(phys, PAGE_SIZE);
}
