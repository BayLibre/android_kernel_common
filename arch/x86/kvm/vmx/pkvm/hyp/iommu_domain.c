				      ...
				      unsigned long pfn, int *target_level)
{
	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			__pkvm_host_donate_hyp(donation->pages[donation->nr_donated], PAGE_SIZE);
			tmp_page = pkvm_phys_to_virt(donation->pages[donation->nr_donated]);
			       ...
			       unsigned long last_pfn)
{
	...
	do {
		...
		if (level < retain_level && !(start_pfn > level_pfn ||
			...
			dma_clear_pte(pte);
			domain_flush_cache(domain, pte, sizeof(*pte));
			donation->pages[donation->nr_donated++] = pkvm_virt_to_phys(level_pte);
				   ...
				   int retain_level)
{
	...
	if (start_pfn == 0 && last_pfn == DOMAIN_MAX_PFN(domain->gaw)) {
		donation->pages[donation->nr_donated++] = domain->pgd;
				 ...
				 unsigned long end_pfn, int level)
{
	...
	while (start_pfn <= end_pfn) {
		...
		if (dma_pte_present(pte)) {
			...
			// TODO: flush iotlb for the freed range.
...
domain_map(struct pkvm_iommu_domain *domain, struct pkvm_iommu_map_param *param)
{
	...
	while (nr_pages > 0) {
		...
		if (!pte) {
			...
			pte = pfn_to_dma_pte(domain, &param->donation, iov_pfn, &largepage_lvl);
			if (!pte)
				return -ENOMEM;
			first_pte = pte;
			...
			if (largepage_lvl > 1) {
				...
				switch_to_super_page(domain, &param->donation,
...
unsigned long pkvm_iommu_domain_map(struct kvm_vcpu *hvcpu, unsigned long param_gva)
{
	...
	param.donation.nr_returned = param.donation.nr_donated;
	param.donation.nr_donated = 0;
	ret = write_gva(hvcpu, param_gva, &param, sizeof(struct pkvm_iommu_map_param), &e);
	if (ret < 0) {
		pkvm_err("pkvm: %s Failed to write map_param(gva: %lx) to host!\n",
				__func__, param_gva);
		return ret;
	}
	return ret;
				    ...
				    struct pkvm_iommu_page_donation *donation)
{
	 ...
	 */
	donation->pages[donation->nr_returned++] = dma_pte_addr(pte);
			 ...
			 unsigned long last_pfn, struct pkvm_iommu_page_donation *donation)
{
	...
	if (start_pfn == 0 && last_pfn == DOMAIN_MAX_PFN(domain->gaw)) {
		...
		donation->pages[donation->nr_returned++] = domain->pgd;
		__pkvm_hyp_donate_host(domain->pgd, PAGE_SIZE);
		domain->pgd = 0ULL;
	}
}
...
unsigned long pkvm_iommu_domain_unmap(struct kvm_vcpu *hvcpu, unsigned long param_gva)
{
	...
	ret = read_gva(hvcpu, param_gva, &param, sizeof(struct pkvm_iommu_unmap_param), &e);
	if (ret < 0) {
		pkvm_err("pkvm: %s Failed to read unmap_param (gva: %lx) from host!\n",
				__func__, param_gva);
		return ret;
	}
	...
	PKVM_ASSERT(param.donation.nr_donated == 0);
	pkvm_spin_lock(&domain->lock);
	domain_unmap(domain, param.start_pfn, param.last_pfn, &param.donation);
	pkvm_spin_unlock(&domain->lock);
	pkvm_put_iommu_domain(domain);

	pkvm_dbg("pkvm: %s returning %d pages\n", __func__, param.donation.nr_returned);
	ret = write_gva(hvcpu, param_gva, &param, sizeof(struct pkvm_iommu_unmap_param), &e);
	if (ret < 0) {
		pkvm_err("pkvm: %s Failed to write unmap_param(gva: %lx) to host!\n",
				__func__, param_gva);
		return ret;
	}
	return 0;
