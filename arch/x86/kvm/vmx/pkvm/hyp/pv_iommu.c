...
int pkvm_iommu_set_sm_ce(u64 param_va)
{
	...
	ret = __pkvm_host_donate_hyp_share_ro(host_gpa2hpa(param.pasid_dir_gpa),
	...
	context->lo = host_gpa2hpa(param.pasid_dir_gpa) | context_pdts(pds);
...
int pkvm_iommu_domain_alloc(u64 param_va)
{
	...
	ret = __pkvm_host_donate_hyp_share_ro(domain->pgd, VTD_PAGE_SIZE);
	if (ret) {
	...
	pgdptr = pkvm_phys_to_virt(domain->pgd);
