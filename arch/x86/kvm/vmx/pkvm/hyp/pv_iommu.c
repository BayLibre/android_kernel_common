		...
		struct pkvm_lm_context_param *param)
{
	...
	if (iommu_find_ptdev(hyp_iommu, param->bdf, 0)) {
		pkvm_err("pkvm: %s: ptdev already setup for device[%x]\n",
				__func__, param->bdf);
		return -EBUSY;
	}
	ptdev = iommu_add_ptdev(hyp_iommu, param->bdf, 0);
	if (!ptdev) {
...
unsigned long pkvm_iommu_domain_alloc(u64 phys, u64 param_gpa)
{
	...
	domain->iommu_coherency = param->iommu_coherency = iommu_paging_structure_coherency(iommu);
...
unsigned long pkvm_iommu_domain_free(u64 phys, u64 pgd_gpa)
{
	struct pkvm_iommu *hyp_iommu = find_iommu_by_reg_phys(phys);
	struct intel_iommu *iommu;
