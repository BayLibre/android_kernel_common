...
static int alloc_qi_desc(struct pkvm_iommu *iommu)
{
	...
	if (__pkvm_host_donate_hyp_share_ro(desc_phys, (1 << order) * PAGE_SIZE)) {
		pkvm_err("pkvm: %s: failed to write protect QI desc!\n", __func__);
		return -EFAULT;
	}
	qi->desc = pkvm_phys_to_virt(desc_phys);
	qi->desc_status = pkvm_phys_to_virt(desc_phys + order * PAGE_SIZE);
...
/*
 * Copied from drivers/iommu/intel/iommu.c:iommu_flush_write_buffer()
 */
 ...
 */
void flush_dev_iotlb_pasid(struct pkvm_iommu *iommu, u16 sid, u16 pfsid,
			u16 pasid, u16 qdep, u64 addr, unsigned int size_order)
{
...
static void handle_gcmd_srtp(struct pkvm_iommu *iommu)
{
	...
	if (IS_ENABLED(CONFIG_PKVM_INTEL_PVIOMMU)) {
		...
		if (__pkvm_host_donate_hyp_share_ro(pgt->root_pa, VTD_PAGE_SIZE)) {
	...
	if (activate_iommu(iommu)) {
		pkvm_err("pkvm: %s: iommu%d failed to activate\n",
				__func__, iommu->iommu.seq_id);
		return;
	}
...
#ifdef CONFIG_PKVM_INTEL_PVIOMMU
			pkvm_err("pkvm: %s: IQT write not allowed with paravirt iommu", __func__);
			ret = -EINVAL;
#else
			if (viommu->vreg.gsts & DMA_GSTS_QIES)
...
unsigned long pkvm_iommu_submit_qi(u64 phys, u64 desc_gpa, int count)
{
	...
	desc = host_gpa2hva(desc_gpa);

	submit_qi(hyp_iommu, desc, count);
