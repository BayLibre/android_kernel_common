...
static inline bool iommu_coherency(struct pkvm_iommu *iommu)
{
	return sm_enabled(iommu) ? ecap_smpwc(iommu->iommu.ecap)
				 : ecap_coherent(iommu->iommu.ecap);
}
...
static struct pkvm_ptdev *iommu_add_ptdev(struct pkvm_iommu *iommu, u16 bdf, u32 pasid)
{
	...
	if (!ptdev) {
		ptdev = pkvm_alloc_ptdev(bdf, pasid, iommu_coherency(iommu));
		if (!ptdev)
			return NULL;
	}
...
static int activate_iommu(struct pkvm_iommu *iommu)
{
	...
	if (iommu->viommu.vreg.rta & DMA_RTADDR_SMT) {
		if (!ecap_smts(iommu->iommu.ecap)) {
			pkvm_err("pkvm: %s: iommu%d: scalable mode not supported\n",
				 __func__, iommu->iommu.seq_id);
			ret = -EINVAL;
			goto out;
		}

		/* TODO: support scalable mode without nested translation */
		if (!ecap_nest(iommu->iommu.ecap)) {
			pkvm_err("pkvm: %s: iommu%d: nested translation not supported\n",
				 __func__, iommu->iommu.seq_id);
			ret = -EINVAL;
			goto out;
		}
...
static int context_cache_invalidate(struct pkvm_iommu *iommu, struct qi_desc *desc)
{
	...
	u16 did = sm_enabled(iommu) ? 0 : QI_DESC_CC_DID(desc->qw0);
