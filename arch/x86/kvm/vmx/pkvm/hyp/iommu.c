...
static int create_qi_desc(struct pkvm_iommu *iommu)
{
...
#ifdef CONFIG_PKVM_INTEL_PVIOMMU
	{
		...
		__pkvm_host_donate_hyp((u64)qi->desc_status, PAGE_SIZE);
		qi->desc_status = pkvm_phys_to_virt((u64)qi->desc_status);
	}
#else
...
void pkvm_iommu_submit_qi(struct kvm_vcpu *hvcpu, unsigned long phys, unsigned long desc_gva, int count)
{
	struct qi_desc desc[QI_MAX_BATCHED_DESC_COUNT];
	struct pkvm_iommu *pkvm_iommu = find_iommu_by_reg_phys(phys);
	struct x86_exception e;
	unsigned long ret;

	if (count > QI_MAX_BATCHED_DESC_COUNT)
		pkvm_err("pkvm: %s Unexpeced size of qi_desc array(gva: %lx from host!\n",
				__func__, desc_gva);

	ret = read_gva(hvcpu, desc_gva, desc, sizeof(struct qi_desc) * count, &e);
	if (ret < 0) {
		pkvm_err("pkvm: %s Failed to read qi_desc(gva: %lx from host!\n",
				__func__, desc_gva);
	}
...
void pkvm_set_qi_desc_status_addr(unsigned long phys, unsigned long qi_desc_status_phys)
{
	struct pkvm_iommu *pkvm_iommu = find_iommu_by_reg_phys(phys);
	pkvm_iommu->qi.desc_status = (void*)qi_desc_status_phys;
}
