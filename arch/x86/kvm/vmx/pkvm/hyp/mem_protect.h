...
static inline void clear_hyp_donated_memory(void *va, size_t size)
{
	/*
	 * The hyp will donate PAGE_SIZE aligned memory region, so clearing
	 * should also be PAGE_SIZE aligned.
	 */
	WARN_ON_ONCE(!PAGE_ALIGNED(va) || !PAGE_ALIGNED(size));

	memset(va, 0, size);
	/*
	 * Flush CPU cache for the hyp donated memory range to make sure the
	 * cannot get the previous contents in RAM via non-coherent DMA.
	 */
