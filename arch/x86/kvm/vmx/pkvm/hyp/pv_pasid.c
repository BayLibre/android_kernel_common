...
int pkvm_pasid_free_table(struct pasid_dir_entry *dir, int max_pde)
{
	...
	for (i = 0; i < max_pde; i++) {
		...
		if (!table)
			continue;
		ret = __pkvm_hyp_donate_host_unshare_ro(pkvm_virt_to_phys(table), VTD_PAGE_SIZE);
		if (ret) {
	...
	ret = __pkvm_hyp_donate_host_unshare_ro(pkvm_virt_to_phys(dir),
...
retry:
	...
	if (!entries) {
		...
		if (__pkvm_host_donate_hyp_share_ro(*ptable_gpa, VTD_PAGE_SIZE)) {
		...
		if (!try_cmpxchg64(&dir[dir_index].val, &tmp,
			...
			if (__pkvm_hyp_donate_host_unshare_ro(*ptable_gpa, VTD_PAGE_SIZE)) {
				pkvm_err("pkvm: %s: failed to undonate unused pasid table Page!\n",
						__func__);
				return -EFAULT;
			}
			goto retry;
