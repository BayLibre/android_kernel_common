...
int pkvm_pasid_free_table(struct pasid_dir_entry *dir, int max_pde)
{
	...
	for (i = 0; i < max_pde; i++) {
		...
		if (!table)
			continue;
		ret = __pkvm_hyp_release_host(pkvm_virt_to_phys(table), VTD_PAGE_SIZE);
		if (ret) {
	...
	ret = __pkvm_hyp_release_host(pkvm_virt_to_phys(dir), ALIGN(max_pde * 8, VTD_PAGE_SIZE));
	if (ret)
...
retry:
	...
	if (!entries) {
		...
		if (__pkvm_host_donate_hyp_request_ro(pkvm_virt_to_phys(entries), VTD_PAGE_SIZE)) {
