				...
				u8 bus, struct context_entry *context, bool upper)
{
	...
	for (ce_idx = 0; ce_idx < 128; ce_idx++) {
		...
		dev->pasid_table = pde;
		dev->max_pasid = nr_pdes << PASIDDIR_SHIFT;
		pkvm_put_device(dev);
	}
...
static int pkvm_pasid_get_entry(struct intel_iommu *iommu, struct pkvm_ptdev *ptdev,
		u64 *ptable_gpa, struct pasid_entry **pte)
{
	...
	dev = ptdev->dev;
	if (!dev->pasid_table || ptdev->pasid >= dev->max_pasid) {
		pkvm_err("pkvm: %s: unexpected pasid table:  device[%x] ptable=%p, max_pasid=%u\n",
				__func__, dev->bdf, dev->pasid_table, dev->max_pasid);
		return -EINVAL;
	}

	dir = dev->pasid_table;
	dir_index = ptdev->pasid >> PASIDDIR_SHIFT;
	index = ptdev->pasid & PASID_PTE_MASK;
...
int pkvm_iommu_clear_pasid_entry(u64 phys, u64 param_gpa)
{
	...
	ptdev = iommu_find_ptdev(hyp_iommu, param->bdf, param->pasid);
	if (!ptdev) {
		pkvm_err("pkvm: %s: failed to locate ptdev for device[%x]\n",
				__func__, param->bdf);
		goto out_unlock;
	}

	ret = pkvm_pasid_get_entry(iommu, ptdev, NULL, &pte);
	if (ret) {
...
int pasid_setup_ptdev(struct pkvm_iommu *hyp_iommu,
		struct pkvm_pasid_table_param *param, struct pkvm_ptdev **ptdev)
{
	struct pkvm_device *dev;

	if (iommu_find_ptdev(hyp_iommu, param->bdf, param->pasid)) {
		pkvm_err("pkvm: %s: ptdev already setup for device[%x]\n",
				__func__, param->bdf);
		return -EBUSY;
	}

	*ptdev = iommu_add_ptdev(hyp_iommu, param->bdf, param->pasid);
	if (!*ptdev) {
	...
	dev = (*ptdev)->dev;
	if (!dev->pasid_table) {
		dev->pasid_table = host_gpa2hva(param->pasid_dir_gpa);
		dev->max_pasid = param->max_pasid;
	} else if (dev->pasid_table != host_gpa2hva(param->pasid_dir_gpa)) {
		pkvm_err("pkvm: %s: pasid table mismatch!\n", __func__);
		iommu_del_ptdev(hyp_iommu, *ptdev);
		*ptdev = NULL;
		return -EINVAL;
	}
...
int pkvm_iommu_pasid_setup_fl(u64 phys, u64 param_gpa)
{
	...
	ret = pasid_setup_ptdev(hyp_iommu, param, &ptdev);
	if (ret)
		goto out_unlock;

	ret = pkvm_pasid_get_entry(iommu, ptdev, &param->pasid_table_gpa, &pte);
	if (ret)
		goto out_unlock;
