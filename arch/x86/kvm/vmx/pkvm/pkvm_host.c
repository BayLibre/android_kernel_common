...
static struct pkvm_iommu_driver *iommu_driver = NULL;
...
static int __init pkvm_iommu_driver_init(int pkvm_init_ret)
{
	int ret;
	if (!smp_load_acquire(&iommu_driver))
		return 0;

	ret = iommu_driver->init_driver();
	if (ret && !pkvm_init_ret) {
		// IOMMU init failed. Disable pkvm
		pr_warn("IOMMU initialization failed. Disabling pkvm!\n");
		iommu_driver->fini_driver();
		pkvm_firmware_rmem_clear();
		/*
		 * TODO: make this work!
		 *      This doesn't work mainly because reprivilege hypercall
		 *      is disabled after finalize. But there may be other issues
...
static int __init __vmx_pkvm_init(void)
{
	...
	if (no_iommu || dmar_disabled) {
		pr_err("IOMMU absent, disabled or not functional, disabling pkvm!\n");
		pkvm_firmware_rmem_clear();
		return -ENODEV;
	}
...
int __init vmx_pkvm_init(void)
{
	...
	ret = __vmx_pkvm_init();
	ret = pkvm_iommu_driver_init(ret);
