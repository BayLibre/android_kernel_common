...
{
	void *apic_regs = vcpu->arch.apic->regs;
	void *apic = vcpu->arch.apic;

	kvm_unshare_hyp(vcpu, (void *)vcpu + sizeof(struct vcpu_vmx));
	kvm_unshare_hyp(apic, (void *)apic + sizeof(struct kvm_lapic));
	kvm_unshare_hyp(apic_regs, (void *)apic_regs + PAGE_SIZE);
}
...
static int pkvm_share_vcpu(struct kvm_vcpu *vcpu)
{
	...
	ret = kvm_share_hyp(apic_regs, (void *)apic_regs + PAGE_SIZE);
	if (ret)
		goto unshare_apic;
