...
static __init int pkvm_setup_host_vmcs_config(void)
{
	...
	if (boot_cpu_has(X86_FEATURE_ARCH_LBR)) {
		/*
		 * Enable the LBR vmexit/vmentry controls to guarantee the
		 * deprivileged host cannot profile the pKVM via LBR.
		 */
	...
	if (boot_cpu_has(X86_FEATURE_MPX)) {
		 ...
		 * MPX, enable the MPX vmexit/vmentry controls to guarantee the
		 * MSR_IA32_BNDCFGS setting by the host won't affect the pKVM.
		 */
...
static __init void init_guest_state_area_from_native(struct vcpu_vmx *vmx)
{
	...
	if (boot_cpu_has(X86_FEATURE_IBT) || boot_cpu_has(X86_FEATURE_SHSTK)) {
		...
		/*
		 * Supervisor shadow stack is guaranteed not enabled. See
