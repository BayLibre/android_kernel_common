...
static void pkvm_vcpu_after_set_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (pkvm_is_protected_vcpu(vcpu)) {
		 ...
		 * mode if vcpu_after_set_cpuid hypercall is successes. Align
		 * the apic_base in the host side to be x2apic mode as well to
		 * make sure the APIC ID (which will be emulated by the host
		 * for the protected apic) can be initialized properly.
		 */
...
static bool pkvm_protected_apic_has_interrupt(struct kvm_vcpu *vcpu)
{
	...
	KVM_BUG_ON(pkvm_hypercall_out(protected_apic_has_interrupt, &out), vcpu->kvm);
