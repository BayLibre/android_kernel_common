...
static int pkvm_vm_init(struct kvm *shared_kvm, unsigned long gpa, unsigned long pgd_hva)
{
	unsigned long pkvm_vm_pa;
	unsigned long pkvm_vm_pa_hva;
	...
	pgd_size = 1;
	pkvm_vm_pa_hva = host_gpa2hpa(pgd_hva);
	if (!PAGE_ALIGNED(pkvm_vm_pa_hva))
		return -EINVAL;

	if (__pkvm_host_donate_hyp(pkvm_vm_pa_hva, pgd_size))
		return -EINVAL;

	ret = pkvm_vm_mmu_init(pkvm_vm, (void *)pkvm_phys_to_virt(pkvm_vm_pa_hva));
	if (ret)
		goto vm_destroy;
...
static void pkvm_vm_destroy(int handle)
{
	...
	drain_hyp_per_vm_pool(&shared_pkvm->teardown_mc, &pkvm_vm->pool);
