...
undonate_fpu:
	__pkvm_hyp_donate_host(__pkvm_pa(fps), fps->size, true);
undonate_vcpu:
	__pkvm_hyp_donate_host(__pkvm_pa(pkvm_vcpu), pkvm_vcpu->size, true);
put_pkvm_vm:
	put_pkvm_vm(pkvm_vm);
	return ret;
}
					       ...
					       size_t size)
{
	...
	if (!kvm_set_cpuid(vcpu, new, new_nent) && (vcpu->arch.cpuid_entries == new)) {
		...
		if (old) {
			 ...
			 * host to retrieve. The size will be used by the host
			 * to determine how much memory pages to free.
			 */
			*(size_t *)old = size;
			free = old;
		} else {
					   ...
					   unsigned long gpa, size_t size)
{
	...
	vcpu = to_kvm_vcpu(pkvm_vcpu);
	/* Expect the host uses this PV interface for pVM only. */
	if (!pkvm_is_protected_vcpu(vcpu))
		return gpa;
