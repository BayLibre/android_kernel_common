...
static void teardown_donated_memory(struct pkvm_memcache *mc, void *addr, size_t size)
{
	...
	memset(addr, 0, size);
	/*
	 * The memory range [addr, addr + size) will be donated to the host. So
	 * flush CPU cache to make sure the host cannot get the previous
	 * contents in RAM via non-coherent DMA.
	 */
...
undonate:
	/*
	 * No need to clear as there is no sensitive data stored in this memory
	 * range yet.
	 */
...
static struct pkvm_vcpu *donate_pkvm_vcpu(unsigned long vcpu_gpa)
{
	...
	pkvm_vcpu = donate_host_memory(vcpu_gpa, size);
	if (!pkvm_vcpu)
		return NULL;

	memset(pkvm_vcpu, 0, size);
	pkvm_vcpu->size = size;
...
{
	struct fpstate *fps = donate_host_memory(fpu_gpa, size);
			    ...
			    unsigned long fpu_gpa)
{
	...
	/*
	 * No need to clear the fps/pkvm_vcpu memory ranges as there is no
	 * sensitive data stored in these memory ranges yet.
	 */
undonate_fpu:
