...
static int pkvm_vm_finalize(int handle)
{
	...
	for (i = 0; i < kvm->created_vcpus; i++) {
		...
		if (vcpu->vcpu_id == kvm->arch.bsp_vcpu_id) {
			/*
			 * Make sure to update pvmfw_load_addr and bsp_vcpu_id values
			 * before allowing the primary vCPU to run. Paired with smp_rmb()
			 * in pkvm_vcpu_run().
			 */
			smp_wmb();
...
static unsigned long pkvm_vcpu_run(struct pkvm_vcpu *pkvm_vcpu, bool force_immediate_exit)
{
	...
	if (pkvm_is_protected_vcpu(vcpu)) {
		...
		if (unlikely(pkvm_vcpu_is_pvmfw_bsp(vcpu) && !kvm_vcpu_has_run(vcpu)))
