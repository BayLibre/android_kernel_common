					       ...
					       unsigned long e2size)
{
	...
	if (!kvm_set_cpuid(vcpu, new, new_nent) && (vcpu->arch.cpuid_entries == new)) {
		 ...
		 */
		if (old) {
			e2size = PAGE_ALIGN(sizeof(struct kvm_cpuid_entry2) * old_nent);
			free_entries = old;
			memset(free_entries, 0, e2size);
		} else {
			e2size = 0;
			free_entries = NULL;
		}
	...
	if (e2size)
		__pkvm_hyp_donate_host(pkvm_virt_to_phys(free_entries), e2size);
free:
	for (; e2size > 0; free_entries += PAGE_SIZE, e2size -= PAGE_SIZE)
		push_pkvm_memcache(&mc, free_entries, pkvm_virt_to_host_gpa);

	if (VALID_PAGE(mc.head)) {
		/*
		 * Store the nr_page in the first page of the teardowned memory
		 * at the offset of sizeof(phys_addr_t) bytes, which is used to
		 * store the next page physical address.
		 */
					   ...
					   unsigned long fpsize)
{
	...
	/* Undonate and free the old fpstate memory pages */
	if (fpsize)
		__pkvm_hyp_donate_host(pkvm_virt_to_phys(free_fps), fpsize);
free:
	for (; fpsize > 0; free_fps += PAGE_SIZE, fpsize -= PAGE_SIZE)
		push_pkvm_memcache(&mc, free_fps, pkvm_virt_to_host_gpa);
