...
static int __vcpu_create(struct kvm *kvm, struct kvm_vcpu *vcpu, struct fpstate *fps)
{
	...
	if (pkvm_is_protected_vcpu(vcpu)) {
		...
		/*
		 * Force set the X86_FEATURE_X2APIC to enable x2apic mode by default
		 * for pVMs to let the pVM use MSR instructions to access lapic, as
		 * emulating xapic mode will require the host to decode MMIO
		 * instruction which is not supported if the guest is a pVM as the pVM's
		 * CPU and memory state will be isolated. Doing this when creating vCPU
		 * to guarantee the x2apic mode will be enabled.
		 *
		 * Setting X86_FEATURE_X2APIC without checking the pVM's CPUID is fine
		 * as the pVM's CPUID will be enforced by the pKVM to have this feature.
		 */
