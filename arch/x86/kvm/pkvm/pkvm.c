...
{
	void *apic_regs = kern_pkvm_va(to_kvm_vcpu(pkvm_vcpu)->arch.apic->regs);

	__pkvm_unpin_shared_mem(__pkvm_pa(pkvm_vcpu->shared_vcpu), kvm_vcpu_sz);
	__pkvm_unpin_shared_mem(__pkvm_pa(to_kvm_vcpu(pkvm_vcpu)->arch.apic),
...
static void pkvm_vm_destroy(int handle)
{
	...
	__pkvm_unpin_shared_mem(__pkvm_pa(pkvm_vm->shared_kvm), kvm_x86_call(vm_size));
