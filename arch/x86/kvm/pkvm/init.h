 ...
 *
 * Iterates over all host vCPUs in the pkvm_hyp->host_vcpus[] array, skipping
 * NULL entries and the ones which corresponding CPU is not initialized by
 * pkvm_init().
 */
#define for_each_pkvm_initialized_cpu(i, vcpu)					\
	for ((i) = 0; (i) < pkvm_hyp->num_cpus &&				\
		      ({ (vcpu) = pkvm_hyp->host_vcpus[(i)]; true; }); (i)++)	\
		if (!(vcpu) || !pkvm_cpu_initialized((vcpu)->cpu))		\
			continue;						\
		else
