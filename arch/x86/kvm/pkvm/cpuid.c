...
#define ENFORCE_REG(reg1, reg2, mask) {	\
	(reg1) &= ~(mask);		\
	(reg1) |= (reg2) & (mask);	\
}
	switch (entry->function) {
	case 1:
		ENFORCE_REG(tmp.ecx, entry->ecx,
			    CPUID_1_ECX_TSC_DLTIMER | CPUID_1_ECX_HYP);
		ENFORCE_REG(tmp.ebx, entry->ebx, CPUID_1_EBX_ID_MASK);
		ENFORCE_REG(tmp.edx, entry->edx, CPUID_1_EDX_HTT);
		break;
	default:
		break;
	}
...
int pkvm_enforce_cpuid(struct kvm_cpuid_entry2 *e2, int *nent, int max_nent)
{
	...
	for (i = 0; i < def_nent; i++) {
		 ...
		 * may contain more entries than host provides.
		 */
