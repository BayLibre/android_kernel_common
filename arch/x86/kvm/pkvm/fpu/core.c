...
int fpu_swap_kvm_fpstate(struct fpu_guest *guest_fpu, bool enter_guest)
{
...
#endif
	/*
	 * If enters the npVM, the FPU are already loaded with the npVM fpu
	 * state by the host. If exits from the npVM, the fpu registers will be
	 * saved by the host. So no need to save FPU for the npVM.
	 *
	 * If enters the pVM, the FPU are loaded with the host fpu state, which
	 * is already saved by the host itself before switching to the pkvm
	 * hypervisor. If exits from the pVM, then the fpu state should be saved
	 ...
	 *
	 * If enters the pVM, restore the FPU with the pVM fpu state. If exits
	 * the pVM, wipe the FPU by restoring FPU with an initial fpu state
	 */
