...
static void *admit_host_page(void *arg)
{
	 ...
	 * because pop will need to read this page to get the next head.
	 * Without donating, reading this page may leak sensitive data as this
	 * page may be a private page of a pVM or the hypervisor.
	 *
	 * The page will be cleared by zalloc_page when allocating it from
	 * the memcache, thus no need to clear it now.
	 */
	if (WARN_ON(pkvm_host_donate_hyp(pkvm_host_gpa_to_phys(host_mc->head.addr),
					 PAGE_SIZE, false)))
		return NULL;
			   ...
			   struct pkvm_memcache *host_mc)
{
	 ...
	 */
	struct pkvm_memcache tmp_mc = *host_mc;
	int ret;
