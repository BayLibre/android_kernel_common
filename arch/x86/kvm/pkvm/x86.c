...
static u64 kvm_get_arch_capabilities(void)
{
	 ...
	 * bugs which is guaranteed by the deprivilege code, so we can set
...
#else
	/*
	 * The CPU which can run the pKVM hypervisor doesn't have GDS bug, which
	 * is guaranteed by the deprivilege code.
	 */
	data |= ARCH_CAP_GDS_NO;
#endif
...
static bool pkvm_host_can_emulate_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info, bool set)
{
	...
	case MSR_IA32_TSC:
		/*
		 * According to the pKVM feature list discussion, the
		 * compromised host cannot mislead pVM that is executing
		 * time-sensitive operations, as Trusty pVM can use GSC as the
		 * secure time source. So still allow the host to access the
