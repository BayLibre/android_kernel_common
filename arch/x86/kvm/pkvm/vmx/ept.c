...
{
	guest_mmu_ops = (struct pkvm_pgtable_ops) {
	...
	guest_mmu_cap = (struct pkvm_pgtable_cap) {
	...
	if (pkvm_hyp->vmx_cap.ept & VMX_EPT_2MB_PAGE_BIT)
		guest_mmu_cap.allowed_pgsz |= 1 << PG_LEVEL_2M;
	if (pkvm_hyp->vmx_cap.ept & VMX_EPT_1GB_PAGE_BIT)
		guest_mmu_cap.allowed_pgsz |= 1 << PG_LEVEL_1G;
}
