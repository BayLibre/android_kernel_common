			    ...
			    ...)
{
	...
	int ret;
#ifdef CONFIG_KASAN
	void *shadow_mem = NULL, *lkl_image_base = NULL;
        unsigned long lkl_image_size = 0;
#endif
	...
	shadow_mem = lkl_ops->kasan_init_shadow_memory(
		(void *)KASAN_SHADOW_OFFSET, KASAN_SHADOW_SIZE);
	BUG_ON(shadow_mem != (void *)KASAN_SHADOW_OFFSET);

	// map memory region to shadow LKL image (for globals)
	lkl_image_base = (void*)&__executable_start;
	lkl_image_size =
		(unsigned long)_end - (unsigned long)&__executable_start;
	BUG_ON(lkl_ops->kasan_map_shadow_memory(
            lkl_image_base, lkl_image_size) != 0);

	// map memory region to shadow current thread stack
...
#ifdef CONFIG_KASAN
	BUG_ON(lkl_ops->kasan_release_shadow_memory((void *)KASAN_SHADOW_OFFSET,
						    KASAN_SHADOW_SIZE) != 0);
#endif
	return 0;
}
