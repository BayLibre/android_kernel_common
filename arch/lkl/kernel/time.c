...
{
#ifdef CONFIG_KASAN
        // map memory for shadow the stack of timer thread
        BUG_ON(lkl_ops->kasan_map_thread_shadow_stack(lkl_ops->thread_self()) != 0);
#endif
...
void __init time_init(void)
{
	...
	if (request_irq(timer_irq, timer_irq_handler, IRQF_NOBALANCING |
			IRQF_TIMER, "timer", &clockevent))
