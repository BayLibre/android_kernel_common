...
static int lkl_pci_remove(struct platform_device *pdev)
{
	...
	if (bus) {
		...
		// TODO(b/229790508): temp solution until the bug is resolved.
		// For some reason, by the time we reach this line of code the
		// reference for host_bridge object is equal to 1 what prevent
		// the corresponding memory from being freed. Manually, decrement the
		// reference counter to free resources.
...
static struct platform_driver lkl_pci_driver = {
	...
	.probe = lkl_pci_probe,
	.remove = lkl_pci_remove,
};

static int __init lkl_pci_init(void)
{
	/*register a platform driver*/
	return platform_driver_register(&lkl_pci_driver);
}
