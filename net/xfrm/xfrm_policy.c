...
xfrm_policy_ok(const struct xfrm_tmpl *tmpl, const struct sec_path *sp, int start,
			   unsigned short family, int first_unchecked)
{
	int idx = start;
	...
	for (; idx < sp->len; idx++) {
		if (xfrm_state_ok(tmpl, sp->xvec[idx], family))
			return ++idx;
		if (sp->xvec[idx]->props.mode != XFRM_MODE_TRANSPORT && idx >= first_unchecked) {
			if (start == -1)
				start = -2-idx;
			break;
		}
			...
			unsigned short family)
{
	...
	if (pol->action == XFRM_POLICY_ALLOW) {
		...
		for (i = xfrm_nr-1, k = 0; i >= 0; i--) {
			k = xfrm_policy_ok(tpp[i], sp, k, family, sp->first_unchecked);
			if (k < 0) {
				if (k < -1)
