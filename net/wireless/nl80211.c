			   ...
			   struct sk_buff *msg)
{
	...
	if (cap->ftm.support_6ghz &&
		...
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_TX_LTF_REP,
		       cap->ftm.max_tx_ltf_rep))
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_RX_LTF_REP,
		       cap->ftm.max_rx_ltf_rep))
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_TX_STS,
		       cap->ftm.max_tx_sts))
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_RX_STS,
		       cap->ftm.max_rx_sts))
		return -ENOBUFS;
	if (cap->ftm.max_total_ltf_tx > 0 &&
	    nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_TOTAL_LTF_TX,
		       cap->ftm.max_total_ltf_tx))
		return -ENOBUFS;
	if (cap->ftm.max_total_ltf_rx > 0 &&
	    nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_TOTAL_LTF_RX,
		       cap->ftm.max_total_ltf_rx))
		return -ENOBUFS;
	if (cap->ftm.support_rsta_edca &&
	    nla_put_flag(msg, NL80211_PMSR_FTM_CAPA_ATTR_RSTA_EDCA_SUPPORT))
		return -ENOBUFS;
	if (cap->ftm.support_rsta_ntb &&
	    nla_put_flag(msg, NL80211_PMSR_FTM_CAPA_ATTR_RSTA_NTB_SUPPORT))
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_NUM_TX_ANTENNAS,
		       cap->ftm.max_no_of_tx_antennas))
		return -ENOBUFS;
	if (nla_put_u8(msg, NL80211_PMSR_FTM_CAPA_ATTR_MAX_NUM_RX_ANTENNAS,
