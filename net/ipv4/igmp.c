			       ...
			       __be32 addr, int event)
{
	...
	if (nla_put_in_addr(skb, IFA_MULTICAST, addr)) {
...
static void inet_ifmcaddr_notify(struct net_device *dev, __be32 addr, int event)
{
	...
	pr_err("%s: %s:%d: notify event %d to group %d", __func__, __FILE__, __LINE__, event, RTNLGRP_IPV4_MADDR);
	rtnl_notify(skb, net, 0, RTNLGRP_IPV4_MADDR, NULL, GFP_ATOMIC);
	return;
error:
	rtnl_set_sk_err(net, RTNLGRP_IPV4_MADDR, err);
