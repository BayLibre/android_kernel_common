...
static inline int inet6_ifmcaddr_msgsize(void)
{
	return NLMSG_ALIGN(sizeof(struct ifaddrmsg)) + nla_total_size(16);
}
			     ...
			     const struct in6_addr *addr, int event)
{
	...
	int err = -ENOBUFS;
	skb = nlmsg_new(inet6_ifmcaddr_msgsize(), GFP_ATOMIC);
	if (!skb)
		goto error;
	...
	rtnl_notify(skb, net, 0, RTNLGRP_IPV6_MADDR, NULL, GFP_ATOMIC);
	return;
error:
	rtnl_set_sk_err(net, RTNLGRP_IPV6_MADDR, err);
}
