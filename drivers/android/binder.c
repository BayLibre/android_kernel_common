			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		if (!(tr->flags & TF_ONE_WAY) && w &&
			...
			binder_set_extended_error(ee, BINDER_EE_PARAM_ERRNO,
				BR_FAILED_REPLY, -EPROTO,
				"new transaction with todo list");
			binder_inner_proc_unlock(proc);
					...
					struct binder_transaction_log_entry *e)
{
	...
	smp_rmb();
	seq_printf(m,
		   "%d: %s from %d:%d to %d:%d context %s node %d handle %d size %d:%d error %d:%d:%d",
		   e->debug_id, (e->call_type == 2) ? "reply" :
		   ((e->call_type == 1) ? "async" : "call "), e->from_proc,
		   e->from_thread, e->to_proc, e->to_thread, e->context_name,
		   e->to_node, e->target_handle, e->data_size, e->offsets_size,
		   e->extended_error.code, e->extended_error.command,
		   e->extended_error.param);
	/*
