				...
				u32 *desc)
{
	...
	d = dbitmap_get_first_zero_bit(dmap);
	if (likely(d < dmap->nbits)) {
		*desc = d;
		return 0;
	}
	 ...
	 * The proc->outer_lock is released to safely allocate the
	 * new bitmap and reacquired to complete the expansion.
	 */
...
static void binder_cleanup_ref_olocked(struct binder_ref *ref)
{
	...
	if (dmap->map)
		clear_bit(ref->data.desc, dmap->map);
...
static void shrink_bitmap_maybe(struct binder_proc *proc)
{
	unsigned long *new;
	int nbits;
