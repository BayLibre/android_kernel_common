 ...
 * 3. A notification is pending to be sent.
 * 4. A notification was went to userspace and kernel is waiting for an ack.
 *
 * When a notification is in flight, new state change events are coalesced into
 * the existing binder_ref_freeze struct to either 1. overwrite the
 * frozen/unfrozen state to be the latest value if the notification hasn't been
 * delivered, or 2. flag for re-delivery with new state if a delivery has
 * already happened.
 */
