...
static int direct_reclaim_state_release(struct inode *inode, struct file *file)
{
...
static const struct proc_ops direct_reclaim_state_proc_ops = {
	...
	.proc_release	= direct_reclaim_state_release,
};
...
static void __exit memhealthmod_end(void)
{
	...
	if (unregister_trace_mm_vmscan_direct_reclaim_begin(mm_vmscan_direct_reclaim_begin_probe,
		NULL))
		pr_warn("memhealth failed to unhook a probe from the mm_vmscan_direct_reclaim_begin" \
				" tracepoint\n");
	if (unregister_trace_mm_vmscan_direct_reclaim_end(mm_vmscan_direct_reclaim_end_probe, NULL))
		pr_warn("memhealth failed to unhook a probe from the mm_vmscan_direct_reclaim_end" \
				" tracepoint\n");
