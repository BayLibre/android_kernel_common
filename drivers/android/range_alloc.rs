...
enum DescriptorState {
    ...
    Reserved { is_oneway: bool, pid: i32 },
    ...
    Allocated { is_oneway: bool, pid: i32 },
}
...
impl<T> RangeAllocator<T> {
    ...
    pub(crate) fn reserve_new(
        ...
        // buffer size).
        self.oneway_spam_detected = if is_oneway && new_oneway_space < self.size / 10 {
            self.low_oneway_space(pid)
        } else {
            false
        };
    ...
    fn low_oneway_space(&self, calling_pid: i32) -> bool {
        let mut cursor = self.list.cursor_front();
