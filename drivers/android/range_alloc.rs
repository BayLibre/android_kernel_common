...
impl<T> RangeAllocator<T> {
    ...
    pub(crate) fn reserve_new(
        ...
        if found_size != size {
            self.descriptors
                .as_ref()
                .set(new_desc.offset.try_into().unwrap(), new_desc)?;
    ...
    pub(crate) fn reservation_abort(&mut self, offset: usize) -> Result {
        ...
        let remove_next = self.descriptors.as_ref().get_scoped(
        ...
        if remove_next {
            ...
            self.descriptors
                .as_ref()
                .remove(next_offset.try_into().unwrap());
        }
        ...
        let original_offset = if let Some(prev_offset) = prev_offset {
            ...
            self.descriptors
                .as_ref()
                .get_scoped(prev_offset, |prev| match prev {
        ...
        if let Some(original_offset) = original_offset {
            self.descriptors
                .as_ref()
                .remove(original_offset.try_into().unwrap());
        };
