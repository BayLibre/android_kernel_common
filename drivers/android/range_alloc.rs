...
impl<T> RangeAllocator<T> {
    ...
    pub(crate) fn reservation_abort(&mut self, offset: usize) -> Result {
        ...
        let reservation = match desc.state.take() {
            Some(DescriptorState::Reserved(reservation)) => reservation,
            None => {
                pr_warn!(
                    "EINVAL from range_alloc.reservation_abort - offset: {}",
                    offset
            ...
            allocated => {
                ...
                // take() set the state to None, put it back again.
    ...
    pub(crate) fn reservation_commit(&mut self, offset: usize, data: Option<T>) -> Result {
        ...
        desc.state = Some(DescriptorState::Allocated(reservation.allocate(data)));
    ...
    pub(crate) fn reserve_existing(&mut self, offset: usize) -> Result<(usize, Option<T>)> {
        ...
        let allocation = match desc.state.take() {
            Some(DescriptorState::Allocated(allocation)) => allocation,
            None => {
                pr_warn!(
                    "ENOENT from range_alloc.reserve_existing - offset: {}",
                    offset
...
// TODO: remove once test support is available
mod tests {
