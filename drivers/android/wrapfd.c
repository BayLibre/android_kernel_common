...
#define BLOCK_MODIFICATION	(1 << 0)
#define BLOCK_MAPPING		(1 << 1)
...
{
	assert_spin_locked(&ctx->lock);
	return (ctx->block_mask & BLOCK_MAPPING) == 0;
}
...
static int block_operations(struct wrap_ctx *ctx, unsigned int mask)
{
	...
	if (WARN_ON((mask & BLOCK_MODIFICATION) == 0))
		return -EINVAL;
...
static void wrap_vm_close(struct vm_area_struct *vma)
{
	...
	if (refcount_dec_and_test(&mapping->refcnt)) {
		vma->vm_ops = mapping->content_vm_ops;
		kfree(mapping);
	}
			  ...
			  struct wrapfd_load __user *user_wrapfd_load)
{
	...
	spin_lock(&ctx->lock);
	ret = block_operations(ctx, BLOCK_MODIFICATION);
	spin_unlock(&ctx->lock);
			    ...
			    struct wrapfd_rewrap __user *user_wrapfd_rewrap)
{
	...
	spin_lock(&ctx->lock);
	ret = block_operations(ctx, BLOCK_MODIFICATION | BLOCK_MAPPING);
	if (!ret) {
		content = ctx->content;
		ctx->content = NULL;
	}
