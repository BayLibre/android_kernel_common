...
static inline unsigned int dbitmap_expand_nbits(struct dbitmap *dmap)
{
	return dmap->nbits << 1;
}
...
dbitmap_get_first_zero_bit(struct dbitmap *dmap)
{
	unsigned long bit;

	bit = find_first_zero_bit(dmap->map, dmap->nbits);
	if (bit < dmap->nbits)
		set_bit(bit, dmap->map);
...
static inline int dbitmap_init(struct dbitmap *dmap)
{
	...
	/* 0 is reserved for context manager */
	set_bit(0, dmap->map);
