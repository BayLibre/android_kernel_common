...
impl<T> TreeRangeAllocator<T> {
    ...
    pub(crate) fn reservation_abort(&mut self, offset: usize) -> Result<FreedRange> {
        ...
        let (reservation, free_node_res) = desc.try_change_state(|state| match state {
            Some((DescriptorState::Reserved(reservation), free_node_res)) => (None, Ok((reservation, free_node_res))),
            None => {
                pr_warn!(
                    "EINVAL from range_alloc.reservation_abort - offset: {}",
                    offset
    ...
    pub(crate) fn reservation_commit(&mut self, offset: usize, data: Option<T>) -> Result {
        ...
        desc.try_change_state(|state| match state {
            Some((DescriptorState::Reserved(reservation), free_node_res)) => (
                Some((DescriptorState::Allocated(reservation.allocate(data)), free_node_res)),
                Ok(()),
            ),
            other => {
                pr_warn!(
    ...
    pub(crate) fn reserve_existing(&mut self, offset: usize) -> Result<(usize, Option<T>)> {
        ...
        let data = desc.try_change_state(|state| match state {
            Some((DescriptorState::Allocated(allocation), free_node_res)) => {
                ...
                (Some((DescriptorState::Reserved(reservation), free_node_res)), Ok(data))
            }
            other => {
                pr_warn!(
                    "ENOENT from range_alloc.reserve_existing - offset: {}",
