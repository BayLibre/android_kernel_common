...
impl Process {
    ...
    fn get_thread(self: ArcBorrow<'_, Self>, id: i32) -> Result<Arc<Thread>> {
        ...
        match inner.threads.entry(id) {
            ...
            rbtree::Entry::Occupied(entry) => {
                // There were two concurrent calls to `get_thread` with the same id, and the other
                // call was first. In this case, we return the thread that the other call created.
                Ok(entry.get().clone())
            }
        }
    }
