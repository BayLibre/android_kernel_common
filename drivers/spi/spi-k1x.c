...
static int k1x_spi_transfer_one_message(struct spi_controller *ctlr,
					   struct spi_message *msg)
{
	struct spi_driver_data *drv_data = spi_controller_get_devdata(ctlr);
...
static int k1x_spi_unprepare_transfer(struct spi_controller *ctlr)
{
	struct spi_driver_data *drv_data = spi_controller_get_devdata(ctlr);
...
static int k1x_spi_probe(struct platform_device *pdev)
{
	...
	ctlr = spi_alloc_host(dev, sizeof(struct spi_driver_data));
	if (!ctlr) {
	...
	ctlr->dma_alignment = DMA_ALIGNMENT;
	ctlr->cleanup = cleanup;
	ctlr->setup = setup;
	ctlr->transfer_one_message = k1x_spi_transfer_one_message;
	ctlr->unprepare_transfer_hardware = k1x_spi_unprepare_transfer;
	...
	status = of_property_read_u32(np, "k1x,ssp-clock-rate", &ctlr->max_speed_hz);
	if (status < 0) {
		dev_err(&pdev->dev, "cannot get clock-rate from DT file\n");
		goto out_error_ctlr_alloc;
	}

	clk_set_rate(drv_data->clk, ctlr->max_speed_hz);
	ctlr->max_speed_hz = clk_get_rate(drv_data->clk);
	clk_prepare_enable(drv_data->clk);
    	reset_control_deassert(drv_data->reset);

	if ((ctlr->bus_num == 2 || ctlr->bus_num == 3) &&
	...
	ctlr->num_chipselect = platform_info->num_chipselect;
