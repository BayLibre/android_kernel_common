...
/*
 * Returns true if the given timestamp is valid (i.e.,
 * if all the following conditions are satisfied), false
 * otherwise.
...
static bool is_valid_timestamp(ktime_t timestamp)
{
	...
	if (ktime_compare(min_time, timestamp) >= 0)
		return false;

	if (ktime_compare(timestamp, current_time))
		return false;
