...
static const struct cqhci_crypto_alg_entry {
	enum cqhci_crypto_alg mmc_alg;
	enum cqhci_crypto_key_size mmc_key_size;
} cqhci_crypto_algs[BLK_ENCRYPTION_MODE_MAX] = {
	[BLK_ENCRYPTION_MODE_AES_256_XTS] = {
		.mmc_alg = CQHCI_CRYPTO_ALG_AES_XTS,
		.mmc_key_size = CQHCI_CRYPTO_KEY_SIZE_256,
	},
};
...
static void cqhci_clear_keyslot(struct cqhci_host *host, int slot)
{
	/*
...
cqhci_find_blk_crypto_mode(union cqhci_crypto_cap_entry cap)
{
	...
	for (i = 0; i < ARRAY_SIZE(cqhci_crypto_algs); i++) {
		BUILD_BUG_ON(CQHCI_CRYPTO_KEY_SIZE_INVALID != 0);
		if (cqhci_crypto_algs[i].mmc_alg == cap.algorithm_id &&
		    cqhci_crypto_algs[i].mmc_key_size == cap.key_size) {
			return i;
		}
	}
...
/**
 * cqhci_init_crypto - Read crypto capabilities, init crypto fields in host
 * @host: Per adapter instance
 *
 * Return: 0 if crypto was initialized or is not supported, else a -errno value.
 */
int cqhci_init_crypto(struct cqhci_host *host)
{
	int cap_idx = 0;
	int err = 0;
	...
	host->mmc->ksm = keyslot_manager_create(
					dev, num_keyslots,
					&cqhci_ksm_ops,
...
int cqhci_prep_crypto_desc(struct cqhci_host *host, struct mmc_request *mrq,
			   __le64 *task_desc)
{
	u64 crypto_desc = 0;
	...
	/*
	 * Set the crypto context in the upper 64 bits of the 128-bit task
	 * descriptor.
	 */
