...
static enum blk_crypto_mode_num cqhci_crypto_blk_crypto_mode_num_for_alg_dusize(
 ...
 * -ENOMEM upon OOM.
 */
static int cqhci_host_init_crypto_spec(struct cqhci_host *host,
				       const struct keyslot_mgmt_ll_ops *ksm_op)
{
	int cap_idx = 0;
	int err = 0;
	...
	if (!(cqhci_readl(host, CQHCI_CAP) & CQHCI_CAP_CS)) {
		dev_err(dev, "no crypto capability\n");
		err = -ENODEV;
		goto out;
	}
	...
	host->ksm = keyslot_manager_create(cqhci_num_keyslots(host), ksm_op,
...
static void cqhci_crypto_setup_rq_keyslot_manager_spec(struct cqhci_host *host,
...
static void cqhci_crypto_destroy_rq_keyslot_manager_spec(struct cqhci_host *host,
						  struct request_queue *q)
{
	keyslot_manager_destroy(host->ksm);
}

static int cqhci_prepare_crypto_desc_spec(struct cqhci_host *host,
				   ...
				   u64 *crypto_ctx)
{
	...
	if (crypto_ctx) {
		*crypto_ctx = CQHCI_CRYPTO_DATA_UNIT_NUM(bc->bc_dun[0]) |
			   CQHCI_CRYPTO_CONFIG_INDEX(bc->bc_keyslot) |
			   CQHCI_CRYPTO_ENABLE(true);
	}
...
int cqhci_crypto_get_ctx(struct cqhci_host *host, struct mmc_request *mrq,
			 u64 *crypto_ctx)
{
	return cqhci_prepare_crypto_desc_spec(host, mrq, crypto_ctx);
}
