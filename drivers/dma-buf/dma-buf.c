...
int dma_buf_account_task(struct dma_buf *dmabuf, struct task_struct *task, bool fd_ref)
{
	...
	if (rec) {
		...
		if (!strcmp(task->comm, "surfaceflinger"))
			pr_err("dmabuf account existing record, %s [%d %s] [%lx %zu] [%u %zu] %lu\n",
					fd_ref ? "fd" : "vma",
		...
		if (!strcmp(task->comm, "surfaceflinger"))
			pr_err("dmabuf account new record, %s [%d %s] [%lx %zu] [%u %zu] %lu\n",
					fd_ref ? "fd" : "vma",
					task_pid_nr(task), task->comm,
					(unsigned long)dmabuf, dmabuf->size,
					dmabuf_info->rss, dmabuf_info->dmabuf_count,
...
void dma_buf_unaccount_task(struct dma_buf *dmabuf, struct task_struct *task, bool fd_ref)
{
	...
	if (rec) {
		if (--rec->refcnt == 0) {
			...
			if (!strcmp(task->comm, "surfaceflinger"))
				pr_err("dmabuf unaccount and remove record, %s [%d %s] [%lx %zu] [%u %zu] %lu\n",
						fd_ref ? "fd" : "vma",
						task_pid_nr(task), task->comm,
						(unsigned long)dmabuf, dmabuf->size,
						dmabuf_info->rss, dmabuf_info->dmabuf_count,
			...
			if (!strcmp(task->comm, "surfaceflinger"))
				pr_err("dmabuf unaccount record, %s [%d %s] [%lx %zu] [%u %zu] %lu\n",
					fd_ref ? "fd" : "vma",
					task_pid_nr(task), task->comm,
					(unsigned long)dmabuf, dmabuf->size,
					dmabuf_info->rss, dmabuf_info->dmabuf_count,
		...
		if (!strcmp(task->comm, "surfaceflinger"))
			pr_err("dmabuf unaccount non-existing record, %s [%d %s] [%lx %zu] [%u %zu]\n",
