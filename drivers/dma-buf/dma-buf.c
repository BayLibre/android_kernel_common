...
static struct task_dma_buf_record *dma_buf_unaccount(struct dma_buf *dmabuf, struct task_dma_buf_info *dmabuf_info)
{
	...
	if (rec) {
		...
		pr_err("Could not find dmabuf %lu in unaccount for a task\n",
						  ...
						  bool share_vm, bool share_fs)
{
	...
	struct task_dma_buf_shared_list *new_list = NULL;
	unsigned long flags;
	unsigned int count;
	int retries = 0;
	...
	count = READ_ONCE(from->dmabuf_count); // Small BUG: This is a size_t, so probably 64bits. Read could be torn on 32bit, but very unlikely we have so many buffers
...
retry:
	...
	mm_list = from->mm_list;
	fd_list = from->fd_list;
	spin_unlock_irqrestore(&from->lock, flags);
	...
	if (share_vm) {
		if (mm_list) {
			...
			to->mm_list = mm_list;
			kfree(new_list); /* Don't need the list */
		} else {
...
int copy_dmabuf_info(u64 clone_flags, struct task_struct *task)
{
	...
	/*
	 * No or partial sharing: Both MM and FD references to dmabufs are duplicated in the child. We
