...
static int dma_buf_account_task_no_preload(struct dma_buf *dmabuf,
...
int dma_buf_begin_new_exec(struct files_struct *old_files)
{
	...
	if (my_files) {
		size_t num_records = 0, num_records_check;
		...
		rcu_read_unlock();
retry:
		if (!task_dmabuf_records_preload(num_records))
			goto err_prealloc;
		...
		if (num_records_check > num_records) {
			...
			if (retries++ > 5)
				goto err_num_records_changed;

			num_records = num_records_check;
			goto retry;
		}
		...
		for (unsigned int n = 0; n < fdt->max_fds; n++) {
			...
			err = dma_buf_account_task_no_preload(file->private_data, new_dmabuf_info);
			if (err)
