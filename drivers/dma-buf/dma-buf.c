...
static struct task_dma_buf_info *compute_dmabuf_info(struct task_struct *task)
{
	...
	rcu_read_lock();
	if (task->files) {
		count_fd = COUNT_DMABUF_FDS(files_lookup_fd_rcu);
	}
	...
	/* count cant change underneath us. See comment above. */
	...
	if (count > 0) {
		...
		rcu_read_lock();
		if (task->files) {
			for (unsigned int n = 0; count_fd && n < files_fdtable(task->files)->max_fds; ++n) {
