...
DECLARE_HASHTABLE(package_to_userid, 8);
DECLARE_HASHTABLE(ext_to_groupid, 8);
DEFINE_MUTEX(pkg_list_lock);
LIST_HEAD(pkglist_listeners);
...
kuid_t get_appid(const char *key)
{
	...
	qstr_init(&q, key);
	return make_kuid(&init_user_ns, __get_appid(&q));
}
EXPORT_SYMBOL(get_appid);

static gid_t __get_ext_gid(const struct qstr *key)
{
	struct hashtable_entry *hash_cur;
	unsigned int hash = key->hash;
	gid_t ret_id;
...
kgid_t get_ext_gid(const char *key)
{
	...
	qstr_init(&q, key);
	return make_kgid(&init_user_ns, __get_ext_gid(&q));
}
EXPORT_SYMBOL(get_ext_gid);

static bool __is_excluded(const struct qstr *app_name, unsigned int user)
{
	struct hashtable_entry *hash_cur;
...
static int insert_ext_gid_entry_locked(const struct qstr *key, gid_t value)
{
	struct hashtable_entry *hash_cur;
	struct hashtable_entry *new_entry;
	unsigned int hash = key->hash;
...
static int insert_ext_gid_entry(const struct qstr *key, gid_t value)
{
	int err;
...
static int insert_userid_exclude_entry(const struct qstr *key, uid_t value)
{
	int err;
	struct pkg_list *pkg;
...
static void remove_ext_gid_entry_locked(const struct qstr *key, gid_t group)
{
	...
	hash_for_each_possible_rcu(ext_to_groupid, hash_cur, hlist, hash) {
		if (qstr_case_eq(key, &hash_cur->key)
				&& atomic_read(&hash_cur->value) == group) {
			hash_del_rcu(&hash_cur->hlist);
			synchronize_rcu();
			free_hashtable_entry(hash_cur);
			break;
...
static void remove_userid_all_entry_locked(uid_t userid)
{
	struct hashtable_entry *hash_cur;
	struct hlist_node *h_t;
	HLIST_HEAD(free_list);
...
static void remove_userid_all_entry(uid_t userid)
{
	struct pkg_list *pkg;
...
static void remove_userid_exclude_entry(const struct qstr *key, uid_t userid)
{
	struct pkg_list *pkg;
...
static void packagelist_destroy(void)
{
	...
	hlist_for_each_entry_safe(hash_cur, h_t, &free_list, dlist)
		free_hashtable_entry(hash_cur);
	mutex_unlock(&pkg_list_lock);
	pr_info("pkglist: destroyed packagelist pkgld\n");
}
...
struct package_details_attribute package_details_attr_##_name = \
...
static void extension_details_release(struct config_item *item)
{
	struct extension_details *extension_details =
					to_extension_details(item);

	pr_info("pkglist: No longer mapping %s files to gid %d\n",
				...
				struct config_group *group, const char *name)
{
	...
	ret = insert_ext_gid_entry(&extension_details->name,
					extensions_value->num);
					...
					struct config_item *item)
{
	...
	pr_info("pkglist: No longer mapping any files to gid %d\n",
				value->num);
	kfree(value);
}
...
struct packages_attribute packages_attr_##_name = \
			__CONFIGFS_ATTR(_name, _mode, _show, _store)
...
struct packages_attribute packages_attr_packages_gid_list =
