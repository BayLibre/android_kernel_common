...
static int clk_zhihe_pllvco_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	struct clk_zhihepll *pll = to_clk_zhihepll(hw);
	const struct zhihe_pll_rate_table *rate_table = pll->rate_table;
	...
	for (i = 0; i < pll->rate_count; i++) {
		...
		if (req->rate == now) {
			req->rate = rate_table[i].vco_rate;
			return 0;
...
static int clk_zhihe_plldiv_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	struct clk_zhihepll *pll = to_clk_zhihepll(hw);
	const struct zhihe_pll_rate_table *rate_table = pll->rate_table;
	unsigned long best = 0, now = 0;
...
EXPORT_SYMBOL(zhihe_clk_fake_pll_fixed_ops);
