...
EXPORT_SYMBOL(zhihe_unregister_clocks);
...
static int clk_zhihediv_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	struct clk_zhihediv *zhihe_div = to_clk_zhihediv(hw);
...
EXPORT_SYMBOL(zhihe_parse_regbase);
...
EXPORT_SYMBOL(zhihe_register_clock);

MODULE_AUTHOR("dong.yan <yand@zhihecomputing.com>");
MODULE_DESCRIPTION("Zhihe SoC clock core (divider/gate/PLL helpers)");
