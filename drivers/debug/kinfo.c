				 ...
				 enum kmemdump_uid id, void *vaddr, size_t size)
{
	...
	pr_info("--- Kernel all info contents updated by kmemdump ---");
	...
	pr_info("--- End of kernel all info update ---");
...
static int debug_kinfo_probe(struct platform_device *pdev)
{
	...
	/* Print the initial contents of the struct for debugging. */
	pr_info("--- Kernel all info initial contents from probe ---");
	pr_info("struct kernel_all_info:");
	pr_info("  magic_number: 0x%08x", all_info->magic_number);
	pr_info("  combined_checksum: 0x%08x", all_info->combined_checksum);
	...
	pr_info("  mod_mem_offset: %u", info->mod_mem_offset);
	pr_info("  mod_kallsyms_offset: %u", info->mod_kallsyms_offset);
	pr_info("--- End of initial contents ---");
