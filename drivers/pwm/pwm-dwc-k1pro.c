...
static int dwpwm_probe(struct platform_device *pdev)
{
	...
	struct dwc_pwm *dwc;
	u32 npwm = DWC_TIMERS_TOTAL;
	int ret;

	of_property_read_u32(node, "npwm", &npwm);
	if (npwm > DWC_TIMERS_TOTAL)
		npwm = DWC_TIMERS_TOTAL;

	chip = devm_pwmchip_alloc(dev, npwm, sizeof(*dwc));
	if (IS_ERR(chip))
		return PTR_ERR(chip);
...
static void dwpwm_remove(struct platform_device *pdev)
{
