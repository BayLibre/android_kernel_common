...
bool ufshcd_crypto_enable(struct ufs_hba *hba)
{
	...
	if (priv->crypto_profile.num_slots)
...
int ufshcd_hba_init_crypto_capabilities(struct ufs_hba *hba)
{
	...
	priv->crypto_profile.features = BLK_CRYPTO_FEATURE_STANDARD_KEYS;
...
void ufshcd_init_crypto(struct ufs_hba *hba)
{
	...
	/* Clear all keyslots */
	for (slot = 0; slot < priv->crypto_profile.num_slots; slot++)
		priv->crypto_profile.ksm_ll_ops.keyslot_evict(&priv->crypto_profile, NULL, slot);
}
