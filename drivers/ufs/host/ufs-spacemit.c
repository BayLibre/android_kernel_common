...
/*
 * The K3 UniPro link does not reliably reach HS directly from PWM. Mirror
 * the vendor BSP sequence: perform an intermediate transition to FASTAUTO
 * with the target gear settings, let the link settle, then let the core
 * issue the final FAST mode change.
 */
static int ufs_spacemit_pwr_change_pre_change(struct ufs_hba *hba,
					      const struct ufs_pa_layer_attr *dev_req_params)
{
	struct ufs_pa_layer_attr fastauto_params;
	int ret;
	...
	fastauto_params = *dev_req_params;
	fastauto_params.pwr_rx = FASTAUTO_MODE;
	fastauto_params.pwr_tx = FASTAUTO_MODE;

	/*
	 * This nested power mode change re-enters
	 * pwr_change_notify(PRE_CHANGE) with FASTAUTO as the target power
	 * mode, which is filtered out above.
	 */
	ret = ufshcd_change_power_mode(hba, &fastauto_params,
				       UFSHCD_PMC_POLICY_FORCE);
	if (ret)
		dev_warn(hba->dev, "%s: FASTAUTO transition failed: %d\n",
			 __func__, ret);

	/*
	 * Settle delay between the FASTAUTO and FAST transitions, as in the
	 * vendor BSP. Proceed with the FAST change even if the FASTAUTO step
	 * failed, matching the vendor behaviour.
	 */
	msleep(100);
					     ...
					     struct ufs_pa_layer_attr *dev_req_params)
{
	...
	switch (status) {
	case PRE_CHANGE:
		ret = ufs_spacemit_pwr_change_pre_change(hba, dev_req_params);
		break;
	case POST_CHANGE:
...
/*
 * On K3 the UTRD/response UPIU updates DMA'd by the controller may be
 * observed stale when the completion handler reads them. The vendor BSP
 * places a dma_rmb() at the head of the core completion path; the
 * android_vh_ufs_compl_command hook fires in ufshcd_compl_one_cqe() right
 * before the core reads the OCS and the response, so a dma_rmb() here is
 * equivalent.
 */
static void ufs_spacemit_compl_command(void *data, struct ufs_hba *hba,
				       struct scsi_cmnd *cmd)
{
	if (hba->vops != &ufs_hba_spacemit_vops)
		return;
...
static int __init ufs_spacemit_module_init(void)
{
	int ret;

	ret = register_trace_android_vh_ufs_compl_command(ufs_spacemit_compl_command,
							  NULL);
	if (ret)
		return ret;

	ret = platform_driver_register(&ufs_spacemit_platform_driver);
	if (ret)
		unregister_trace_android_vh_ufs_compl_command(ufs_spacemit_compl_command,
							      NULL);

	return ret;
}
module_init(ufs_spacemit_module_init);

static void __exit ufs_spacemit_module_exit(void)
{
	platform_driver_unregister(&ufs_spacemit_platform_driver);
	unregister_trace_android_vh_ufs_compl_command(ufs_spacemit_compl_command,
						      NULL);
}
module_exit(ufs_spacemit_module_exit);
