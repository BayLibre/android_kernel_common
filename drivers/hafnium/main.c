...
static int hf_vcpu_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		case HF_VCPU_RUN_SLEEP:
			hrtimer_start(&vcpu->timer, ns_to_ktime(ret.sleep.ns),
				      HRTIMER_MODE_REL);
			hf_vcpu_sleep(vcpu);
...
static int hf_send_skb(struct sk_buff *skb)
{
	...
	pr_debug("hf_mailbox_send len %d\n", skb->len);
...
static int hf_sock_sendmsg(struct socket *sock, struct msghdr *m, size_t len)
{
	...
	if (copy_from_iter(skb_put(skb, len), len, &m->msg_iter) != len) {
		err = -EFAULT;
		goto err_cleanup;
	}
