...
static struct goldfish_fwdata_state state;
...
#define DT_COMPATIBLE_ATTR(_folder) \
	struct kobj_attribute _folder##_compatible_attr = { \
		.attr = { .name = "compatible", .mode = S_IRUGO, }, \
		.show = property_show, \
	}
...
static int goldfish_fwdata_probe(struct platform_device *pdev)
{
	int ret = -EIO;
...
static int goldfish_fwdata_remove(struct platform_device *pdev)
{
	clean_up();
	return 0;
}

static const struct acpi_device_id goldfish_fwdata_acpi_match[] = {
	{ "ANDR0001", 0 },
	{ },
};
MODULE_DEVICE_TABLE(acpi, goldfish_fwdata_acpi_match);

static struct platform_driver goldfish_fwdata_driver = {
	.probe = goldfish_fwdata_probe,
	.remove = goldfish_fwdata_remove,
	.driver = {
		.name = "goldfish_fwdata",
		.acpi_match_table = ACPI_PTR(goldfish_fwdata_acpi_match),
	}
};

module_platform_driver(goldfish_fwdata_driver);
