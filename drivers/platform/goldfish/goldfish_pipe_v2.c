...
static void goldfish_pipe_dma_clear_lock(struct goldfish_pipe *pipe)
{
	...
	if (!pipe->closing) {
		...
		if (dma) {
			BUG_ON(atomic_read(&dma->in_use) != GOLDFISH_DMA_REGION_USED);
			clear_bit(BIT_WAKE_ON_UNLOCK_DMA, &pipe->flags);
			atomic_set(&dma->in_use, GOLDFISH_DMA_REGION_AVAILABLE);
...
static int try_lock(atomic_t *a, int old, int new)
{
	return old == atomic_cmpxchg(a, old, new);
}
...
static int goldfish_pipe_dma_acquire_lock(struct goldfish_pipe *pipe)
{
	if (pipe->closing) {
		...
		struct goldfish_dma_context *dma = pipe->dma;
		if (dma) {
			if (try_lock(&dma->in_use,
				GOLDFISH_DMA_REGION_AVAILABLE,
				GOLDFISH_DMA_REGION_USED)) {
				return 0;
			} else {
				set_bit(BIT_WAKE_ON_UNLOCK_DMA, &pipe->flags);
				return goldfish_pipe_wait_event(
					BIT_WAKE_ON_UNLOCK_DMA, pipe);
			}
		} else {
