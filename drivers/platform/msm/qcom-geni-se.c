...
int se_geni_clks_off(struct se_geni_rsc *rsc)
{
	...
	return geni_se_rmv_ab_ib(geni_se_dev, rsc);
}
...
int se_geni_resources_off(struct se_geni_rsc *rsc)
{
	...
	return pinctrl_select_state(rsc->geni_pinctrl, rsc->geni_gpio_sleep);
}
			   ...
			   unsigned long ab, unsigned long ib)
{
	...
	if (geni_se_dev->num_paths == 2) {
		if (unlikely(!(geni_se_dev->bus_bw_noc))) {
			geni_se_dev->bus_bw_noc = msm_bus_scale_register_client(
				geni_se_dev->pdata);
			      ...
			      size_t size)
{
	...
	*iova = DMA_ERROR_CODE;
	...
	return dma_alloc_coherent(cb_dev, size, iova, GFP_KERNEL);
}
