...
// #include "mem_health_test.h" -> #include <linux/mem_health_test.h>
...
static int min_filelist_kbytes;
						...
						const char *buf, size_t count)
{
	...
	p("low_mem: setting ram weight to %u\n",
		low_mem_ram_vs_swap_weight);
	return count;
}
...
// Here we can introduce our new attribute for OOM kills ??
			...
			unsigned long cmd, void *v)
{
	...
	if (task->signal->oom_mm) {
		//  In this case what do we want to do? 
		// Send an event/signal to notify regarding this occurrance?
	}



//https://cs.android.com/android/kernel/superproject/+/common-android-mainline:common/drivers/misc/uid_sys_stats.c;l=673-704
...
{
	// Create the `mem_health` directory to store data in
	struct kobject *mem_health_kobj = kobject_create_and_add("mem_health", kernel_kobj);
	if (!mem_health_kobj)
		return -ENOMEM;
	...
	err = sysfs_create_group(mem_health_kobj, &low_mem_attr_group);
	if (err) {
		pr_err("low_mem: register sysfs failed\n");
		return err;
	}

	// Sub-directory to store different mem related nodes
	low_mem_node = sysfs_get_dirent(mem_health_kobj->sd, "android-low_mem");
	if (low_mem_node) {
...
module_init(low_mem_init)		// Should this be swapped to `early_initcall()`?
