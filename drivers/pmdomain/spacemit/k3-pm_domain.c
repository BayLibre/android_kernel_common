...
struct spacemit_pm_domain {
	...
	/* GPU switch latched on after the first power-up (see power_off) */
	bool latched_on;
...
static int spacemit_pd_power_off(struct generic_pm_domain *domain)
{
	...
	/*
	 * Never cut the GPU switch at runtime: the vendor stack runs the GPU
	 * with active PM disabled (DDK bEnableActivePM=IMG_FALSE) and never
	 * exercises a runtime OFF->ON cycle of this domain. Cutting it
	 * through the ESOS and powering it back up wedges the interconnect
	 * ~200ms after the first DRM client resumes the GPU (hard machine
	 * freeze). Keep the switch on once it has been powered.
	 */
	if (spd->pm_index == PM_GPU_SWITCH_INDEX && spd->latched_on)
		goto _disable_regulator;
...
static int spacemit_pd_power_on(struct generic_pm_domain *domain)
{
	...
	/* GPU switch stays on after the first power-up (see power_off) */
	if (spd->pm_index == PM_GPU_SWITCH_INDEX && spd->latched_on)
		return 0;
...
MODULE_DEVICE_TABLE(of, spacemit_pm_domain_dt_match);

static struct platform_driver spacemit_pm_domain_driver = {
	.probe = spacemit_pm_domain_probe,
	.driver = {
		.name   = "spacemit-k3-pm-domain",
		.of_match_table = spacemit_pm_domain_dt_match,
	},
};
module_platform_driver(spacemit_pm_domain_driver);

MODULE_DESCRIPTION("SpacemiT K3 RPMI power-domain driver");
MODULE_LICENSE("GPL");
