...
static unsigned int __maybe_unused g_acpr_per;
...
#if 0 /* TODO: port to kernel 6.19 register_syscore() API */
static int acpr_per_suspend(void *data)
{
	unsigned int apcr_per;
	unsigned int apcr_clear = 0, apcr_set = (1 << PM_QOS_PE_VOTE_AP_SLPEN_OFFSET);
...
static void acpr_per_resume(void *data)
{
	(void)data;
	unsigned int apcr_per;
...
/* syscore_ops removed — kernel 6.19 uses register_syscore() API */
#endif
...
static int spacemit_pm_domain_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < sizeof(spacemit_regmap_dt_match) / sizeof(spacemit_regmap_dt_match[0]); ++i) {
		...
		if (IS_ERR(pmu->regmap[i])) {
			if (PTR_ERR(pmu->regmap[i]) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
			pr_err("%s: failed to find %s regmap\n", __func__, spacemit_regmap_dt_match[i].compatible);
	...
	/* TODO: port syscore_ops to kernel 6.19 register_syscore API */
	return 0;
...
subsys_initcall(spacemit_pm_domain_drv_register);
