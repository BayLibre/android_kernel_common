...
static DEVICE_ATTR(erase_blksize, S_IRUGO | S_IRUSR,
		   erase_blksize_show, NULL);

static void
ufshcd_init_erase_blksize(struct ufs_hba *hba)
{
	if (device_create_file(hba->dev, &dev_attr_erase_blksize))
		dev_err(hba->dev, "Failed to create erase_blksize sysfs entry\n");
}
...
static DEVICE_ATTR(logical_blksize, S_IRUGO | S_IRUSR,
		   logical_blksize_show, NULL);

static void
ufshcd_init_logical_blksize(struct ufs_hba *hba)
{
	if (device_create_file(hba->dev, &dev_attr_logical_blksize))
		dev_err(hba->dev, "Failed to create logical_blksize sysfs entry\n");
}
