	...
	u32 *attr_val)
{
	...
	u32 retries;
	 ...
	 for (retries = QUERY_REQ_RETRIES; retries > 0; retries--) {
		ret = ufshcd_query_attr(hba, opcode, idn, index,
						selector, attr_val);
		if (ret)
			dev_dbg(hba->dev, "failed with error %d, retries %d\n",
				ret, retries);
		else
			break;
	}
	...
	if (ret)
		dev_err(hba->dev,
			"query attribute, idn %d, failed with error %d after %d retires\n",
			idn, ret, QUERY_REQ_RETRIES);
	return ret;
}
	...
	enum query_opcode opcode, enum flag_idn idn, bool *flag_res)
{
	...
	if (ret)
		dev_err(hba->dev,
			"query attribute, opcode %d, idn %d, failed with error %d after %d retries\n",
			(int)opcode, (int)idn, ret, retries);
	return ret;
}
...
static int ufshcd_reset_ctmwb(struct ufs_hba *hba, bool force)
{
	...
	if (ufshcd_is_ctmwb_off(hba_ctmwb)) {
		dev_dbg(hba->dev, "write booster already disabled. ctmwb_state = %d\n",
			hba_ctmwb.ufs_ctmwb_state);
		return 0;
	}

	if (ufshcd_is_ctmwb_err(hba_ctmwb))
		dev_err(hba->dev, "previous write booster control was failed.\n");
	...
	if (err) {
		...
		dev_err(hba->dev, "disable write booster failed. err = %d\n", err);
	} else {
		ufshcd_set_ctmwb_off(hba_ctmwb);
		dev_dbg(hba->dev, "ufs write booster disabled \n");
	}
...
static int ufshcd_ctmwb_manual_flush_ctrl(struct ufs_hba *hba, int en)
{
	...
	dev_dbg(hba->dev, "%sable write booster manual flush\n", en ? "en" : "dis");
	if (en) {
		err = ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG,
					QUERY_FLAG_IDN_WB_BUFF_FLUSH_EN, NULL);
		if (err)
			dev_err(hba->dev, "enable write booster failed. err = %d\n", err);
	} else {
		err = ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_CLEAR_FLAG,
					QUERY_FLAG_IDN_WB_BUFF_FLUSH_EN, NULL);
		if (err)
...
static int ufshcd_ctmwb_flush_ctrl(struct ufs_hba *hba)
{
	...
	if (!err && (curr_status <= UFS_WB_MANUAL_FLUSH_THRESHOLD)) {
		dev_dbg(hba->dev, "enable ctmwb manual flush, buf status : %d\n", curr_status);
		...
		if (!err) {
			...
			if (err)
				dev_err(hba->dev, "disable ctmwb manual flush failed. err = %d\n",
						err);
		} else
			dev_err(hba->dev, "enable ctmwb manual flush failed. err = %d\n",
					err);
...
static int ufshcd_ctmwb_ctrl(struct ufs_hba *hba, bool enable)
{
	...
	if (hba->pm_op_in_progress) {
		dev_err(hba->dev, "ctmwb ctrl during pm operation is not allowed.\n");
		return 0;
	}
	...
	if (enable) {
		if (ufshcd_is_ctmwb_on(hba_ctmwb)) {
			dev_err(hba->dev, "write booster already enabled. ctmwb_state = %d\n",
				hba_ctmwb.ufs_ctmwb_state);
			return 0;
		}
		...
		if (err) {
			...
			dev_dbg(hba->dev, "ufs write booster enabled \n");
		}
	} else {
		if (ufshcd_is_ctmwb_off(hba_ctmwb)) {
			dev_err(hba->dev, "write booster already disabled. ctmwb_state = %d\n",
				hba_ctmwb.ufs_ctmwb_state);
			return 0;
		}
		pm_runtime_get_sync(hba->dev);
...
static int ufshcd_get_ctmwbbuf_unit(struct ufs_hba *hba)
{
	...
	if ((ret == -EOPNOTSUPP) || scsi_is_wlun(sdev->lun)){
		...
		dev_dbg(hba->dev,"do not support WB\n");
		return 0;
	}
	...
	if (dLUNumTurboWriteBufferAllocUnits) {
		...
		dev_dbg(hba->dev, "LU %d supports ctmwb, ctmwbbuf unit : 0x%x\n",
				(int)sdev->lun, dLUNumTurboWriteBufferAllocUnits);
	} else
		hba_ctmwb->support_ctmwb_lu = false;
