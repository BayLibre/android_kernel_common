			     ...
			     const union ufs_crypto_cfg_entry *cfg, int slot)
{
	...
	if (cap.algorithm_id == UFS_CRYPTO_ALG_AES_XTS &&
		...
		dev_err_ratelimited(hba->dev,
				    "Unhandled crypto algorithm; id=%d, key_size=%d\n",
	...
	/*
	 * ICE (or maybe the SCM call?) accidentally byte-swaps the 32-bit words
	 * of the key.  So we have to do it here too in order to make the final
	 * key be correct.
	 */
