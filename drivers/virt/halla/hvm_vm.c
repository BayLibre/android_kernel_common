...
{
	uint32_t irq = irq_level->irq;
	uint32_t vcpu_idx, vcpu2_idx, irq_num;
			     ...
			     struct arm_smccc_res *res)
{
	...
	/* TODO: Fix this */
	ret = exynos_hvc(HVC_FID_HVM_ENABLE_CAP,
			 hvm->vm_id, cap->cap, cap->args[0], cap->args[1]);
...
static void hvm_destroy_vm(struct hvm *hvm)
{
	...
	ret = exynos_hvc(HVC_FID_HVM_DESTROY_VM,
			hvm->vm_id, 0, 0, 0);
	if (ret) {
		pr_err("Failed to destroy VM in Halla ret[%lx]\n", ret);
		/* Destroying VM is failed in Halla but keep destroying it */
	}
	...
	kfree(hvm->s2table_buf);

	for (unsigned int i = 0; i < hvm->pgtable_num; i++)
		kfree(hvm->vm_pgtable[i]);
...
int hvm_check_extension(struct hvm *hvm, uint64_t cap, void __user *argp)
{
	switch (cap) {
	...
	case HVM_CAP_VM_GPA_SIZE: {
		uint64_t value = CONFIG_ARM64_PA_BITS;

		if (copy_to_user(argp, &value, sizeof(uint64_t)))
			return -EFAULT;
...
int hvm_dev_ioctl_create_vm(unsigned long vm_type)
{
	...
	if (!hvm) {
		pr_err("%s: Fail to alloc hvm\n", __func__);
		return -ENOMEM;
	}

	ret = exynos_hvc(HVC_FID_HVM_CREATE_VM, vm_type, 0, 0, 0);
	if (ret == 0) {
		pr_err("Failed to create vm\n");
		kfree(hvm);
		return -ENOMEM;
	}
	...
	if (ret) {
		pr_err("Failed to initialize stage2 mmu\n");
		kfree(hvm);
		return ret;
	}
	...
	if (ret) {
		pr_err("Failed to create vm debugfs\n");
		kfree(hvm);
		return ret;
	}
