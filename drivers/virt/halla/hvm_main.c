...
static int hvm_drv_enable_vm_s2_ptdump(struct device *dev)
{
#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	unsigned long ver, base, size;
	int ret;

	ver = exynos_hvc(HVC_FID_GET_HALLA_INFO,
			HALLA_INFO_TYPE_VERSION,
			0, 0, 0);
	if (ver != HALLA_INFO_VERSION) {
		dev_err(dev, "HALLA_INFO_VERSION mismatch [%lx]\n", ver);
		return -EINVAL;
	}

	base = exynos_hvc(HVC_FID_GET_HALLA_INFO,
			HALLA_INFO_TYPE_VM_S2_PTDUMP_BASE,
			0, 0, 0);
	if (base == ERROR_INVALID_INFO_TYPE) {
		dev_err(dev, "Fail to get vm s2 ptdump base\n");
		return -EINVAL;
	}

	size = exynos_hvc(HVC_FID_GET_HALLA_INFO,
			HALLA_INFO_TYPE_VM_S2_PTDUMP_SIZE,
			0, 0, 0);
	if (size == ERROR_INVALID_INFO_TYPE) {
		dev_err(dev, "Fail to get vm s2 ptdump size\n");
		return -EINVAL;
	}

	dev_info(dev,
		"VM s2 ptdump buffer base[%#lx]/size[%#lx]\n",
		base, size);

	ret = dbg_snapshot_add_bl_item_info(VM_S2_PTDUMP_DSS_NAME,
						base, size);
	if (ret)
		dev_err(dev, "Fail to add vm s2 ptdump - ret[%d]\n", ret);
...
static int hvm_drv_probe(struct platform_device *pdev)
{
	...
	ret = hvm_drv_enable_vm_s2_ptdump(&pdev->dev);
	if (ret)
		goto err_misc_register;
