 ...
 *
 * Uses dma_alloc to allocate contiguous memory region of size len.
 *
...
static int gunyah_cma_alloc(struct gunyah_cma *cma, loff_t len)
{
	...
	cma->page = dma_alloc_contiguous(&cma->dev, len, GFP_KERNEL);
	if (!cma->page)
		return -ENOMEM;
	return 0;
}
...
static int gunyah_cma_release(struct inode *inode, struct file *file)
{
	...
	dma_free_contiguous(&cma->dev, cma->page, max_size);
	cma->page = NULL;
...
static long gunyah_cma_create_mem_fd(struct gunyah_cma *cma)
{
	...
	i_size_write(inode, cma_get_size(cma->dev.cma_area));
