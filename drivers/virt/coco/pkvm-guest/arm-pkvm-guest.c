			      ...
			      bool func_has_range)
{
	...
	if (!IS_ALIGNED(phys | (numpages * PAGE_SIZE), pkvm_granule))
		return -EINVAL;
				   ...
				   pgprot_t *prot)
{
	 ...
	 * indicate the hypervisor where are the MMIO regions and those regions
	 * have to be aligned with pkvm_granule anyway.
	 */
