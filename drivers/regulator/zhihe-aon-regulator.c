...
static int zhihe_match_regulator_name(struct aon_regu_desc *regus_set, const char *string)
{
	int index;
	const char *item;
...
static int zhihe_aon_regulator_probe(struct platform_device *pdev)
{
	...
	/*get ipc handle */
	dev_info(dev, "bisect: calling zhihe_aon_get_handle\n");
	ret = zhihe_aon_get_handle(&(zhihe_aon_pmic_info.ipc_handle), "aon0");
	dev_info(dev, "bisect: zhihe_aon_get_handle done, ret=%d\n", ret);
	if (ret) {
	...
	for_each_child_of_node(np, child) {
		...
		dev_info(dev, "bisect: calling devm_regulator_register(%s)\n", regulator_name);
		rdev = devm_regulator_register(dev, desc, &config);
		dev_info(dev, "bisect: devm_regulator_register(%s) done, err=%ld\n",
			 regulator_name, IS_ERR(rdev) ? PTR_ERR(rdev) : 0);
		if (IS_ERR(rdev)) {
	...
	dev_info(dev, "bisect: zhihe_aon_regulator_probe complete\n");
	return 0;
}
