...
#if IS_ENABLED(CONFIG_ZRAM_ANDROID_IOCTL)
static bool zram_prefetch_cache_exist(struct zram *zram, u32 index)
{
...
static int zram_prefetch_cache_store(struct zram *zram, u32 index,
				     unsigned long blk_idx)
{
	void *old_val;
	...
	if (xa_err(old_val))
		...
		pr_warn("Store an existing cache entry: %u\n", index);

	return 0;
}

/*
 * The slot was prefetched and going to writeback again. We can reuse the
 * blk_idx in prefetch_cache to reduce the extra write operations.
 */
static void zram_populate_bdev_block(struct zram *zram, u32 index,
				     unsigned long blk_idx)
{
	u32 size;
	int prio;
...
static int zram_populate_table(struct zram *zram, struct page *page, u32 index)
{
	 ...
	 */
#if IS_ENABLED(CONFIG_ZRAM_ANDROID_IOCTL)
	if (zram_prefetch_cache_store(zram, index, blk_idx)) {
		zram_slot_unlock(zram, index);
		zs_free(zram->mem_pool, handle);
		return -ENOMEM;
	}
	...
	/* zram_free_page decreased the pages_stored */
	atomic64_inc(&zram->stats.pages_stored);
	/* zram_writeback_complete decreased the compr_data_size */
	atomic64_add(size, &zram->stats.compr_data_size);
	/*
	 * Add huge_pages back because it  has been decreased twice:
	 * 1. zram_free_page from zram_writeback_complete
	 * 2. zram_free_page from zram_populate_table
	 */
	if (huge) {
			 ...
			 struct zram_wb_ctl *wb_ctl)
{
	...
	while ((pps = select_pp_slot(ctl))) {
		...
		err = zram_prefetch_cache_load(zram, index, &cached_blk_idx);
		if (!err) {
			zram_populate_bdev_block(zram, index, cached_blk_idx);
			goto next;
		}
#endif
...
static int decompress_bdev_page(struct zram *zram, struct page *page, u32 index)
{
	...
	if (!zram_test_flag(zram, index, ZRAM_WB) &&
		...
		pr_warn("Neither writeback nor prefetch entry: %u\n", index);
		zram_slot_unlock(zram, index);
		return -EINVAL;
	}
#else
				...
				unsigned long index)
{
	...
	zram_free_page(zram, index);
#if IS_ENABLED(CONFIG_ZRAM_ANDROID_IOCTL)
	if (zram_prefetch_cache_load(zram, index, &cached_blk_idx) == 0)
		zram_release_bdev_block(zram, cached_blk_idx);
#endif
	zram_slot_unlock(zram, index);
}
