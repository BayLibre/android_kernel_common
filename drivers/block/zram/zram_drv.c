			      ...
			      unsigned long blk_idx)
{
	 ...
	 * prevent sleeping under the slot lock. The IO path might takes the
...
static int zram_populate_table(struct zram *zram, struct page *page, u32 index)
{
	...
	/*
	 * We bring pages from backing device to zram mem_pool, the stats was
	 * decreased in zram_writeback_complete. Add the compr_data_size and
	 * huge_pages back.
	 */
	...
	/* zram_free_page decreased the pages_stored and huge_pages. */
	atomic64_inc(&zram->stats.pages_stored);
	if (huge)
		atomic64_inc(&zram->stats.huge_pages);
