	...
	for (addr = start; addr < end; addr += PAGE_SIZE) {
		...
		if (cmd == ZRAM_ANDROID_IOC_PROCESS_WRITEBACK) {
			...
			scan_slots_for_writeback(zram, 0, index, index+1,
						 pp_ctl);
			if (private->nr_remaining_pages != -1UL)
				   ...
				   struct zram_pp_ctl *ctl)
{
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_WRITEBACK) {
		...
		nr_remaining_pages = ioc_data_pwb->size >> PAGE_SHIFT;
	...
	if (!nr_remaining_pages)
		private.nr_remaining_pages = -1UL;
	else
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_WRITEBACK) {
		...
		ioc_data_pwb = &ioc_data->data.process_writeback;
		if (ret > 0) {
			ioc_data_pwb->next_addr = private.last_addr;
			ret = 0;
		} else {
