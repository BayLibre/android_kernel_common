...
static int zram_ioctl_process_writeback_scan(struct zram *zram,
	struct zram_android_ioc_data_process_range_writeback *ioc_data_prwb,
	struct zram_pp_ctl *ctl)
{
	struct mm_struct *mm;
	struct vm_area_struct *vma;
	struct task_struct *task;
	unsigned long start_addr = (unsigned long)ioc_data_prwb->start_addr;
	...
	struct zram_process_walk_private private = {
		.zram = zram,
		.pp_ctl = ctl,
		.nr_remaining_pages = DIV_ROUND_UP_POW2(ioc_data_prwb->size,
							PAGE_SIZE),
	};
	...
	task = pidfd_get_task(ioc_data_prwb->pidfd, &f_flags);
	if (IS_ERR(task))
		return PTR_ERR(task);
	...
	if (ret > 0) {
		ioc_data_prwb->next_addr = private.next_addr;
		ret = 0;
	} else {
		ioc_data_prwb->next_addr = 0;
	}
	...
	struct zram_android_ioc_data_process_range_writeback *ioc_data_prwb)
{
	...
	ret = zram_ioctl_process_writeback_scan(zram, ioc_data_prwb, pp_ctl);
	if (!ret)
		ret = zram_writeback_slots(zram, pp_ctl, wb_ctl);

	ioc_data_prwb->written_bytes = wb_ctl->processed_bytes;
	       ...
	       unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == ZRAM_ANDROID_IOC_PROCESS_RANGE_WRITEBACK) {
		ret = zram_ioctl_process_writeback(zram,
			&ioc_data.data.process_range_writeback);
