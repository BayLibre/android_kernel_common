...
/*
 * Helper to get the OPP device node for a CPU device.
 * Returns the operating-points-v2 node or NULL.
 */
static struct device_node *spacemit_get_opp_node(struct device *dev)
{
	return dev_pm_opp_of_get_opp_desc_node(dev);
}
                                  ...
                                  unsigned long event, void *data)
{
	...
	opp_np = spacemit_get_opp_node(cpu_dev);
	if (!opp_np)
		return 0;

	if (cci_init == 0) {
		cci_clk = of_clk_get_by_name(opp_np, "cci");
		of_property_read_u64_array(opp_np, "cci-hz", &rates, 1);
		if (!IS_ERR(cci_clk)) {
                                  ...
                                  unsigned long event, void *data)
{
	...
	tcm_clk = of_clk_get_by_name(opp_np, "tcm");
	ace0_clk = of_clk_get_by_name(opp_np, "ace0");
	ace1_clk = of_clk_get_by_name(opp_np, "ace1");
	pll_clk = of_clk_get_by_name(opp_np, "pll3");
	c0hi = of_clk_get_by_name(opp_np, "c0hi");
	c1hi = of_clk_get_by_name(opp_np, "c1hi");
...
#endif /* CONFIG_SOC_SPACEMIT_K1X - pre_early_init */
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Spacemit CPUFreq Driver");
