...
static u32 virt_cpufreq_get_perftbl_entry(struct cpufreq_policy *policy,
					  u32 idx)
{
	writel_relaxed(idx, base + policy->cpu * PER_CPU_OFFSET +
		       REG_PERFTBL_SEL_OFFSET);
	return readl_relaxed(base + policy->cpu * PER_CPU_OFFSET +
...
static int virt_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	u32 num_freqtbl_entries, idx, cur_perf_domain, perf_domain;
	...
	num_freqtbl_entries = readl_relaxed(base + policy->cpu * PER_CPU_OFFSET +
				    REG_PERFTBL_LEN_OFFSET);

	if (!num_freqtbl_entries) {
		return -ENODEV;
	} else if (num_freqtbl_entries == 1) {
		policy->cpuinfo.min_freq = 1;
		policy->cpuinfo.max_freq = virt_cpufreq_get_perftbl_entry(policy, 0);
		...
		table = kcalloc(num_freqtbl_entries + 1, sizeof(*table), GFP_KERNEL);
		if (!table)
			return -ENOMEM;

		for (idx = 0; idx < num_freqtbl_entries; idx++)
			table[idx].frequency = virt_cpufreq_get_perftbl_entry(policy, idx);
