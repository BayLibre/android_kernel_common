...
{
	struct kvm_cpufreq_drv_data *data = (struct kvm_cpufreq_drv_data *)policy->driver_data;
	struct kvm_cpufreq_offsets *offsets = (struct kvm_cpufreq_offsets *)data->info->offsets;

	writel_relaxed(freq, data->base + policy->cpu * offsets->reg_size
			+ offsets->reg_set_freq);
}
...
static u32 kvm_cpufreq_get_freq(struct cpufreq_policy *policy)
{
	...
	return readl_relaxed(data->base + policy->cpu * offsets->reg_size
			+ offsets->reg_cur_freq);
}
...
static struct kvm_cpufreq_offsets kvm_freq_offsets = {
	.reg_cur_freq = 0x0,
	.reg_set_freq = 0x4,
	.reg_size = 0x8,
};
...
static void kvm_scale_freq_tick(void)
{
	...
	struct kvm_cpufreq_drv_data *data = (struct kvm_cpufreq_drv_data *)policy->driver_data;
	struct kvm_cpufreq_ops *ops = data->info->ops;
	...
	cur_freq = (u64)ops->get_freq(policy);
