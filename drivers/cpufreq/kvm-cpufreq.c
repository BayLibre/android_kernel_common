...
static int kvm_cpufreq_get_freqtbl_num_entries(int cpu)
{
	...
	smp_call_function_single(cpu, remote_get_freqtbl_num_entries, &ret, 1);
	return ret;
}
...
static int kvm_cpufreq_populate_freqtbl(struct cpufreq_frequency_table	*table, int cpu) {
	...
	smp_call_function_single(cpu, remote_populate_freqtbl, &freq_data, 1);
...
static int kvm_cpufreq_driver_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(&pdev->dev, "KVM CPUFreq driver failed to register: %d\n", ret);
		return ret;
	} else {
