...
static void pv_pasid_tear_down_entry(struct intel_iommu *iommu, struct device *dev, u32 pasid)
{
	...
	int ret = pkvm_hc_iommu_clear_pasid_entry(iommu->reg_phys, &param);
	pr_info("%s: device[%x:%x:%x] ret=%d\n", __func__, info->bus, PCI_SLOT(info->devfn), PCI_FUNC(info->devfn), ret);
}
				  ...
				  u32 pasid, u16 did)
{
	...
	int ret = pkvm_hc_iommu_set_pasid_fl(iommu->reg_phys, &param);
	if (ret == -ENOMEM) {
		pr_info("%s: retrying SET operation with a new page!\n", __func__);
		...
		ret = pkvm_hc_iommu_set_pasid_fl(iommu->reg_phys, &param);
				  ...
				  u32 pasid, u16 did, int flags)
{
	...
	spin_lock(&iommu->lock);
	if (iommu_pv_translation(iommu)) {
		int ret = pv_pasid_setup_first_level(iommu, dev, pgd, pasid, did);
		spin_unlock(&iommu->lock);
		return ret;
	}
				  ...
				  u32 pasid, u16 did, bool dirty_tracking)
{
	...
	int ret = pkvm_hc_iommu_set_pasid_sl(iommu->reg_phys, &param);
	if (ret == -ENOMEM) {
		pr_info("%s: retrying SET operation with a new page!\n", __func__);
		void *pasid_table = iommu_alloc_page_node(iommu->node, GFP_ATOMIC);
		if (!pasid_table) {
		...
		param.pasid_table_gpa = virt_to_phys(pasid_table);
		ret = pkvm_hc_iommu_set_pasid_sl(iommu->reg_phys, &param);
		if (param.pasid_table_gpa)
			iommu_free_page(pasid_table);
	}
				   ...
				   struct device *dev, u32 pasid)
{
	...
	spin_lock(&iommu->lock);
	if (iommu_pv_translation(iommu)) {
		int ret = pv_pasid_setup_second_level(iommu, dev, pgd_val,
				  ...
				  u32 pasid, u16 did)
{
	...
	int ret = pkvm_hc_iommu_set_pasid_pt(iommu->reg_phys, &param);
	if (ret == -ENOMEM) {
		pr_info("%s: retrying SET operation with a new page!\n", __func__);
		void *pasid_table = iommu_alloc_page_node(iommu->node, GFP_ATOMIC);
		if (!pasid_table) {
				   ...
				   struct device *dev, u32 pasid)
{
	...
	spin_lock(&iommu->lock);
	if (iommu_pv_translation(iommu)) {
		int ret = pv_pasid_setup_pass_through(iommu, dev, pasid, did);
		spin_unlock(&iommu->lock);
		return ret;
	}
...
static int pv_pasid_table_teardown(struct intel_iommu *iommu, u8 bus, u8 devfn, u16 *did)
{
	...
	if (ret) {
		pr_warn("%s: pkvm failed to tear down pasid_table for device[%x:%x:%x]\n",
...
static int pv_device_pasid_table_setup(struct device_domain_info *info)
{
	...
	pr_info("%s: setting sm context entry for device: %x:%x:%x\n",
