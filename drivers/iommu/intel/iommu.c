			 ...
			 unsigned long last_pfn, struct list_head *freelist)
{
	...
	pr_debug("IOMMU: %s donated_pages: %d, returning %d pages to gatherlist\n",
			__func__, (int)donation->nr_pages,
			donation->nr_pages > max_lvl ? (int)donation->nr_pages - max_lvl : 0);

	/*
	 * Maintain upto 'max_lvl' pages in the cache for normal operation.
	 * But if we are exiting the domain, free all the pages.
	 */
	if (start_pfn == 0 && last_pfn == DOMAIN_MAX_PFN(domain->gaw)) {
		max_lvl = 0;
	}
	for (i = donation->nr_pages - 1; i >= max_lvl; donation->nr_pages--, i--) {
...
static int pv_context_clear(struct intel_iommu *iommu, u8 bus, u8 devfn, u16 *did)
{
	...
	ret = pkvm_hc_iommu_clear_ce(&param);
	if (ret) {
...
static void pv_context_flip_pri(struct device_domain_info *info, bool enable)
{
	...
	if (pkvm_hc_iommu_set_sm_ce_pre(&param)) {
...
static int pv_context_setup_pass_through(struct intel_iommu *iommu, u8 bus, u8 devfn)
{
	...
	int ret = pkvm_hc_iommu_set_lm_ce(&param);
