			     ...
			     int prot, gfp_t gfp, size_t *mapped)
{
	...
	ret = res.a0;
	pr_err("pviommu: map page pa=%llx @ iova=%lx => %d\n", paddr, iova, ret);
	if (ret) {
				  ...
				  struct iommu_iotlb_gather *gather)
{
	...
	ret = res.a0;
	pr_err("pviommu: unmap page @ iova=%lx => %d\n", iova, ret);
	if (ret) {
...
static int pviommu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	for (i = 0; i < fwspec->num_ids; i++) {
		sid = fwspec->ids[i];
		pr_err("pviommu: attach dev iommu=%ld, sid=%ld", (unsigned long)pv->id, (unsigned long)sid);
