...
static int smmu_alloc_idmap_mc(struct kvm_hyp_memcache *idmap_mc)
{
	...
	/*
	 * We  alloctage pages to cover mapping with PAGE_SIZE for all memory
	 * Then we allocate extra for 1GB of MMIO.
	 * We add 10 extra pages as we map the rest with first level blocks
	 * for PAGE_SIZE = 4KB, that should cover 5TB or address space.
	 */
