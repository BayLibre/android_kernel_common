...
static int kvm_arm_smmu_topup_memcache(struct arm_smccc_res *res, gfp_t gfp)
{
	...
	if (req.mem.dest == REQ_MEM_DEST_HYP_IOMMU) {
		return __pkvm_topup_hyp_alloc_mgt_flags(HYP_ALLOC_MGT_IOMMU_ID,
							req.mem.nr_pages,
							req.mem.sz_alloc,
							gfp);
...
({									\
	...
	} while (__res.a1 && !kvm_arm_smmu_topup_memcache(&__res, 0));\
	__res.a1;							\
})
				       ...
				       struct iommu_iotlb_gather *iotlb_gather)
{
	...
	do {
		 ...
		 */
		 ...
		 (unmapped || !kvm_arm_smmu_topup_memcache(&res, 0)));
