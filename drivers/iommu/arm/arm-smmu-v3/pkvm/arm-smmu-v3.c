...
{
	return CALL_FROM_OPS(__list_add_valid_or_report, new, prev, next);
}

bool __list_del_entry_valid_or_report(struct list_head *entry)
{
	return CALL_FROM_OPS(__list_del_entry_valid_or_report, entry);
}
#endif
...
static void *smmu_zalloc(size_t size)
{
	void *p;
			  ...
			  u32 pasid_bits, u64 *ent, bool *update_ste)
{
	...
	*update_ste = false;
	...
	if (!cd_table) {
		...
		if (!cd_table)
			return -ENOMEM;
		table_size = (1 << pasid_bits) * (CTXDESC_CD_DWORDS << 3);
			   ...
			   u32 sid, u32 pasid, u32 pasid_bits)
{
	...
	if (!smmu_existing_in_domain(smmu, smmu_domain)) {
		...
		iommu_node = smmu_zalloc(sizeof(struct domain_iommu_node));
		if (!iommu_node) {
			ret = -ENOMEM;
			goto out_unlock;
		}
...
int smmu_alloc_domain(struct kvm_hyp_iommu_domain *domain, pkvm_handle_t domain_id, u32 type)
{
	...
	smmu_domain = smmu_zalloc(sizeof(struct hyp_arm_smmu_v3_domain));
	if (!smmu_domain)
		return -ENOMEM;
