...
static void wait_for_transfers_inflight(struct uart_port *uport)
{
	...
	check_transfers_inflight(uport);
}
...
static void msm_geni_serial_abort_rx(struct uart_port *uport)
{
	...
	geni_write_reg(irq_clear, uport->membase, SE_GENI_S_IRQ_CLEAR);
...
static void msm_geni_serial_complete_rx_eot(struct uart_port *uport)
{
	...
	if (poll_done)
		geni_write_reg(RX_EOT, uport->membase, SE_DMA_RX_IRQ_CLR);
}
...
static int msm_geni_serial_prep_dma_tx(struct uart_port *uport)
{
	...
	if (!ret) {
		...
		geni_write_reg(0, uport->membase,
					SE_UART_TX_TRANS_LEN);
		geni_cancel_m_cmd(uport->membase);
		if (!msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS,
						M_CMD_CANCEL_EN, true)) {
		...
		geni_write_reg(M_CMD_CANCEL_EN, uport->membase,
							SE_GENI_M_IRQ_CLEAR);
...
static void msm_geni_serial_tx_fsm_rst(struct uart_port *uport)
{
	...
	tx_irq_en = geni_read_reg(uport->membase, SE_DMA_TX_IRQ_EN);
	geni_write_reg(0, uport->membase, SE_DMA_TX_IRQ_EN_SET);
	geni_write_reg(1, uport->membase, SE_DMA_TX_FSM_RST);
	do {
		done = msm_geni_serial_poll_bit(uport, SE_DMA_TX_IRQ_STAT,
							TX_RESET_DONE, true);
		tries++;
	} while (!done && tries < 5);
	geni_write_reg(TX_DMA_DONE | TX_RESET_DONE, uport->membase,
						     SE_DMA_TX_IRQ_CLR);
	geni_write_reg(tx_irq_en, uport->membase, SE_DMA_TX_IRQ_EN_SET);
}
...
static void start_rx_sequencer(struct uart_port *uport)
{
	...
	if (port->xfer_mode == FIFO_MODE) {
		...
		geni_write_reg(geni_s_irq_en, uport->membase,
							SE_GENI_S_IRQ_EN);
		geni_write_reg(geni_m_irq_en, uport->membase,
							SE_GENI_M_IRQ_EN);
	} else if (port->xfer_mode == SE_DMA) {
...
static void msm_geni_serial_rx_fsm_rst(struct uart_port *uport)
{
	...
	rx_irq_en = geni_read_reg(uport->membase, SE_DMA_RX_IRQ_EN);
	geni_write_reg(0, uport->membase, SE_DMA_RX_IRQ_EN_SET);
	geni_write_reg(1, uport->membase, SE_DMA_RX_FSM_RST);
	do {
		done = msm_geni_serial_poll_bit(uport, SE_DMA_RX_IRQ_STAT,
							RX_RESET_DONE, true);
		tries++;
	} while (!done && tries < 5);
	geni_write_reg(RX_DMA_DONE | RX_RESET_DONE, uport->membase,
						     SE_DMA_RX_IRQ_CLR);
	geni_write_reg(rx_irq_en, uport->membase, SE_DMA_RX_IRQ_EN_SET);
}
...
static void stop_rx_sequencer(struct uart_port *uport)
{
	...
	if (done) {
		geni_write_reg(irq_clear, uport->membase,
						SE_GENI_S_IRQ_CLEAR);
		goto exit_rx_seq;
	}
...
static int msm_geni_serial_handle_dma_rx(struct uart_port *uport, bool drop_rx)
{
	...
	tty_flip_buffer_push(tport);
exit_handle_dma_rx:
	return geni_se_rx_dma_prep(msm_port->wrapper_dev, uport->membase,
...
static irqreturn_t msm_geni_serial_isr(int isr, void *dev)
{
	...
	s_irq_status = geni_read_reg(uport->membase,
						SE_GENI_S_IRQ_STATUS);
	m_irq_en = geni_read_reg(uport->membase, SE_GENI_M_IRQ_EN);
...
static int msm_geni_serial_port_setup(struct uart_port *uport)
{
	...
	if (!uart_console(uport)) {
		...
		geni_write_reg(cfg0, uport->membase,
						SE_GENI_TX_PACKING_CFG0);
		geni_write_reg(cfg1, uport->membase,
				...
				struct ktermios *termios, struct ktermios *old)
{
	...
	/* parity */
	tx_trans_cfg = geni_read_reg(uport->membase,
							SE_UART_TX_TRANS_CFG);
	tx_parity_cfg = geni_read_reg(uport->membase,
							SE_UART_TX_PARITY_CFG);
	rx_trans_cfg = geni_read_reg(uport->membase,
							SE_UART_RX_TRANS_CFG);
	rx_parity_cfg = geni_read_reg(uport->membase,
							SE_UART_RX_PARITY_CFG);
		...
		const char *opt)
{
	...
	geni_write_reg(cfg0, uport->membase, SE_GENI_TX_PACKING_CFG0);
	geni_write_reg(cfg1, uport->membase, SE_GENI_TX_PACKING_CFG1);
	geni_write_reg(tx_trans_cfg, uport->membase,
							SE_UART_TX_TRANS_CFG);
	geni_write_reg(tx_parity_cfg, uport->membase,
							SE_UART_TX_PARITY_CFG);
	geni_write_reg(bits_per_char, uport->membase,
							SE_UART_TX_WORD_LEN);
	geni_write_reg(stop_bit, uport->membase, SE_UART_TX_STOP_BIT_LEN);
	geni_write_reg(s_clk_cfg, uport->membase, GENI_SER_M_CLK_CFG);
	geni_write_reg(s_clk_cfg, uport->membase, GENI_SER_S_CLK_CFG);
	geni_read_reg(uport->membase, GENI_SER_M_CLK_CFG);
