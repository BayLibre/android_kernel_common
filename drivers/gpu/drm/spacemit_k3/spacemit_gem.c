				...
				size_t size)
{
	...
	if (priv->contig_mem) {
		/*
		 * CMA dma_alloc_from_contiguous() is not exported to modules in
		 * the GKI kernel, so the contiguous-memory path is unsupported
		 * here.  priv->contig_mem is hardcoded false, so this is never
		 * taken at runtime; fail loudly if that ever changes.
		 */
		kfree(sgt);
		return -ENOMEM;
	} else {
...
free_pages:
	list_for_each_entry_safe(page, tmp_page, &pages, lru) {
		/* contig_mem path is unsupported (CMA not exported); see above. */
		if (!priv->contig_mem)
