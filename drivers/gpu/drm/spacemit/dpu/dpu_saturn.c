...
static int dpu_update_clocks(struct spacemit_dpu *dpu, uint64_t mclk)
{
	...
	if (hwdev->is_hdmi) {
		/* For HDMI, use hmclk as the display clock */
		dpu->cur_mclk = clk_get_rate(clk_ctx->hmclk);
		dpu->cur_bw = dpu->cur_mclk;
	} else {
...
static int dpu_enable_clocks(struct spacemit_dpu *dpu)
{
	...
	if (hwdev->is_hdmi) {
		clk_prepare_enable(clk_ctx->hmclk);
		if (!IS_ERR_OR_NULL(clk_ctx->mclk))
			clk_prepare_enable(clk_ctx->mclk);
		if (!IS_ERR_OR_NULL(clk_ctx->hclk))
			clk_prepare_enable(clk_ctx->hclk);
...
static int dpu_init(struct spacemit_dpu *dpu)
{
	...
	/* Enable HDMI power domain manually (APMU + 0x3f4)
	 * The genpd framework cannot be used because it disables unused
	 * domains before vendor modules load. Write directly to APMU.
	 */
	if (hwdev->is_hdmi) {
		void __iomem *apmu = ioremap(0xD4282800, 0x400);
		if (apmu) {
			u32 pwr;
			/* Set auto_pwr_on (bit0) + hw_mode (bit4) */
			pwr = readl(apmu + 0x3f4);
			pwr |= (1 << 0) | (1 << 4);
			writel(pwr, apmu + 0x3f4);
			usleep_range(300, 500);
			/* Check power status (bit15 of status reg 0xf0) */
			pwr = readl(apmu + 0xf0);
			if (pwr & (1 << 15))
				DRM_INFO("HDMI power domain enabled\n");
			else
				DRM_WARN("HDMI power domain enable timeout\n");
			iounmap(apmu);
		}
	}
