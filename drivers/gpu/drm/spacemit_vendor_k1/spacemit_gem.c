				...
				size_t size)
{
	...
	if (priv->contig_mem) {
		page = cma_alloc(dev_get_cma_area(drm->dev), page_nums, 0, false);
		if (!page) {
			kfree(sgt);
...
free_pages:
	list_for_each_entry_safe(page, tmp_page, &pages, lru) {
		if (priv->contig_mem)
			cma_release(dev_get_cma_area(drm->dev), page, page_nums);
		else
...
static void spacemit_gem_sysmem_free(struct spacemit_gem_object *spacemit_obj)
{
	...
	for_each_sgtable_sg(sgt, sg, i) {
		...
		if (priv->contig_mem)
			cma_release(dev_get_cma_area(drm->dev), page, sg->length >> PAGE_SHIFT);
		else
			__free_pages(page, compound_order(page));
	}
