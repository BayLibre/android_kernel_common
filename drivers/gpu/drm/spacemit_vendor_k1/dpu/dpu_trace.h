...
TRACE_EVENT(k1_drm_display_mode_info,
...
DECLARE_EVENT_CLASS(k1_dpu_status_template,
...
DEFINE_EVENT(k1_dpu_status_template, dpuctrl_scaling,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
DEFINE_EVENT(k1_dpu_status_template, dpuctrl,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
DEFINE_EVENT(k1_dpu_status_template, spacemit_plane_update_hw_channel,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
DEFINE_EVENT(k1_dpu_status_template, saturn_irq_enable,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
DEFINE_EVENT(k1_dpu_status_template, saturn_enable_vsync,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
DEFINE_EVENT(k1_dpu_status_template, dpu_isr_status,
	TP_PROTO(const char *name, int status),
	TP_ARGS(name, status)
);
...
DECLARE_EVENT_CLASS(k1_dpu_uint64_t_data_template,
...
DEFINE_EVENT(k1_dpu_uint64_t_data_template, u64_data,
        TP_PROTO(const char *name, uint64_t data),
        TP_ARGS(name, data)
);

TRACE_EVENT(k1_spacemit_plane_disable_hw_channel,
...
DECLARE_EVENT_CLASS(k1_dpu_func_template,
...
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_check_rdma,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_atomic_check,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_atomic_update,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_reset,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_atomic_duplicate_state,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_atomic_destroy_state,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_plane_init,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_enable_clocks,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_disable_clocks,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, saturn_wait_for_cfg_ready,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, saturn_wait_for_eof,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, saturn_wait_for_wb,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, saturn_run_display,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_init,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_uninit,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_isr,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_run,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, dpu_stop,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_mode_set_nofb,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_atomic_enable,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_atomic_disable,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_atomic_check,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_atomic_begin,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_atomic_flush,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_enable_vblank,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_crtc_disable_vblank,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_dpu_run,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_dpu_stop,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_dpu_init,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
DEFINE_EVENT(k1_dpu_func_template, spacemit_dpu_uninit,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);

DECLARE_EVENT_CLASS(k1_dpu_ctrl_template,
...
DEFINE_EVENT(k1_dpu_ctrl_template, saturn_ctrl_cfg_ready,
	TP_PROTO(int id, int enable),
	TP_ARGS(id, enable)
);
DEFINE_EVENT(k1_dpu_ctrl_template, saturn_ctrl_sw_start,
	TP_PROTO(int id, int enable),
	TP_ARGS(id, enable)
);
...
#define TRACE_INCLUDE_PATH ../../drivers/gpu/drm/spacemit_vendor_k1/dpu/
