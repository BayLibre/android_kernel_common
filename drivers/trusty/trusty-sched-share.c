...
trusty_sched_share_resources_allocate(struct trusty_sched_share_state *share_state)
{
	...
	dev_info(share_state->dev, "*** %s: sched_shared_vm=%llx  size=%d\n",
	...
	dev_info(share_state->dev, "*** %s: sched_shared_mem_id=0x%llx",
		 __func__, mem_id);
...
struct trusty_sched_share_state *trusty_register_sched_share(struct device *device)
{
	...
	dev_info(device, "*** %s: calling api SMC_SC_SCHED_SHARE_REGISTER...\n",
		 __func__);
	...
	dev_info(device, "*** %s: sched_share_state=%llx\n", __func__,
...
err_smc_std_call32:
	...
	if (result != 0) {
		dev_info(sched_share_state->dev,
