...
static int
trusty_sched_share_resources_init(struct trusty_sched_share_state *share_state)
{
	struct scatterlist *sg;
	struct trusty_sched_shared *shared;
	unsigned char *mem = share_state->sched_shared_vm;
	...
	if (!share_state->sg) {
		result = -ENOMEM;
		dev_err(share_state->dev, "%s: failed to alloc sg\n", __func__);
	...
	for_each_sg(share_state->sg, sg, share_state->num_pages, i) {
		...
		if (!pg) {
			result = -ENOMEM;
			dev_err(share_state->dev, "%s: failed to map page i= %d\n",
					__func__, i);
		...
		struct trusty_sched_share_state *sched_share_state)
{
	...
	result = trusty_sched_share_resources_init(sched_share_state);
	if (result)
		return;
...
void trusty_free_sched_share(struct trusty_sched_share_state *sched_share_state)
{
	...
	if (sched_share_state->sched_shared_mem_id) {
