...
#define TIPC_MEMREF_DATA_OUT		(0x1U << 0)
#define TIPC_MEMREF_DATA_IN		(0x1U << 1)
#define TIPC_MEMREF_FLAGS_MASK		(TIPC_MEMREF_DATA_IN | \
					 TIPC_MEMREF_DATA_OUT)
					      ...
					      size_t sz)
{
	...
	if (vds->free_msg_buf_cnt) {
		...
		if (sz > vds->msg_buf_sz) {
			...
			/* remove from free list and put it on stashed list */
			list_del(&mb->node);
...
#if defined(CONFIG_TRUSTY_MEMREFS)
...
static unsigned int memref_count_pages(struct tipc_shmem *shm)
{
	...
	for (i = 0; i < 3; i++)
		pg_cnt += shm->rgns[i].size / PAGE_SIZE;
	return pg_cnt;
}
...
static int memref_preflight_check(struct tipc_shmem *shm)
{
	...
	if (!(shm->flags & TIPC_MEMREF_FLAGS_MASK))
		return -EINVAL;

	for (i = 0; i < 3; i++) {
		/* size has to be page aligned */
		if (shm->rgns[i].size & (PAGE_SIZE - 1))
			return -EINVAL;

		/* base address also has not be page aligned */
		if ((uintptr_t)shm->rgns[i].base & (PAGE_SIZE - 1))
			return -EINVAL;
	}
	return 0;
}
...
static ssize_t memrefv_aux_data_len(struct tipc_shmem *shmv, uint shmv_cnt)
{
	uint i;
	int ret;
	size_t len;
			      ...
			      struct tipc_shmem *shm)
{
	...
	gup_flags = (shm->flags & TIPC_MEMREF_DATA_IN) ? FOLL_WRITE : 0;

	down_read(&current->mm->mmap_sem);

	/* for all 3 regions */
	for (i = 0; i < 3; i++) {
		pg_num = shm->rgns[i].size / PAGE_SIZE;
		pg_start = (unsigned long)shm->rgns[i].base;
	...
	hdr->flags = shm->flags & TIPC_MEMREF_FLAGS_MASK;
	hdr->size = mref->pg_pinned * PAGE_SIZE;
	hdr->offset = 0;
	hdr->pg_size = PAGE_SIZE;
...
{
	/* we should not have any */
	WARN_ON(!list_empty(&chan->mref_list));
}

#endif  /* CONFIG_TRUSTY_MEMREFS */
			  ...
			  long timeout, bool intr)
{
	...
	if (shmv_cnt) {
		aux_len = memrefv_aux_data_len(shmv, shmv_cnt);
		if (aux_len < 0)
			return (int)aux_len;
		aux_len += ALIGN(len, MEMREF_ALIGN);
	}
