...
/*
 * vds_get_list_to_unmap()
 *
 * @arg1: pointer to tipc's virtio state
 * @arg2: list of buffers to take from (input)
 * @arg3: list to populate with buffers to unmap (output)
 * @arg4: number of buffers to unmap (<=0 to unmap entire list)
 *
		...
		const int request_cnt)
{
	...
	list_for_each_safe(tmp_list, next_pos, list) {
		...
		list_move_tail(&mb->node, list_to_unmap);
		actual_cnt++;
		...
		unsigned long reduce_cnt)
{
	...
	while (list) {
		...
		/* NOTE: leave 1 per list to prevent reclaiming just to need it again */
		mutex_lock(lock);
		ret = vds_get_list_to_unmap(vds, list, &list_to_unmap, list_reduce_cnt - 1);
		if (ret > 0)
			*(list_cnt) -= ret;
		mutex_unlock(lock);
...
tipc_shrink_count(struct shrinker *shrink, struct shrink_control *sc)
{
	...
	ret = vds->free_msg_buf_cnt + vds->free_rx_cnt;
	...
	ret = (ret - 4) / 2;
	if (ret < 0)
		ret = 0; /* below shrinkable limit; shrinker should stop */

	dev_dbg(&default_vdev->dev, "%s: free: tx= %d rx= %d; reporting %d\n", __func__,
			vds->free_msg_buf_cnt, vds->free_rx_cnt, ret);
...
tipc_shrink_scan(struct shrinker *shrink, struct shrink_control *sc)
{
	...
	dev_dbg(&default_vdev->dev, "%s: asked to free %d of %d\n", __func__,
			sc->nr_to_scan,
			vds->free_msg_buf_cnt + vds->free_rx_cnt);

	return vds_reduce_buf_cnt(vds, sc->nr_to_scan);
}
