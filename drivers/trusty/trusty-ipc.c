...
static void _free_vds(struct kref *kref)
{
	...
	/*
	 * Holding references to the device should prevent this. If it happens
	 * anyways, it means we've lost shared memory regions until reboot.
	 */
					...
					*new_handle)
{
	...
	/* Don't allow us to be unbound while handles are shared. */
	get_device(dev);
	...
	while (*new) {
		...
		WARN_ON(handle->tipc.obj_id == new_handle->tipc.obj_id);
							  ...
							  obj_id)
{
	...
	if (out)
		put_device(tipc_shared_handle_dev(out));
