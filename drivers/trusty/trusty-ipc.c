...
static int tipc_shared_handle_drop(struct tipc_shared_handle *shared_handle)
{
	...
	if (shared_handle->shared) {
		ret = trusty_reclaim_memory(tipc_shared_handle_dev
						(shared_handle),
	...
	for (shm_idx = 0; shm_idx < msg->shm_cnt; shm_idx++) {
		...
		if (shared_handle) {
			if (tipc_shared_handle_drop(shared_handle))
				pr_err("Failed to drop handle found in dropped buffer"
				       );
		} else {
			pr_err("Found handle in dropped buffer which was not registered to tipc device..."
			       );
		}
	}
}
