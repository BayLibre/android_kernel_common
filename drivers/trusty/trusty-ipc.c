...
/* TODO: Switch to typedef from FF-A patches once landed */
typedef u64 shared_id;
...
static struct shared_handle *take_shared_handle_locked(shared_id obj_id)
{
...
static struct shared_handle *take_shared_handle(shared_id obj_id)
{
...
recover_mem:
	for (shm_idx--; shm_idx >= 0; shm_idx--) {
		if (drop_shared_handle(shm_handles[shm_idx]))
