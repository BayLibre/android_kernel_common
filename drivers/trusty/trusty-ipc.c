...
static bool owns_ffa_handle(struct tipc_shared_handle *h) {
	/* h->shared is true only if dma_buf did not own an FF-A handle */
	return !h->shared;
}
					...
					*new_handle)
{
	...
	while (*new) {
		...
		parent = *new;
		/*
		 * An obj_id (FF-A handle) can be registered multiple times if
		 * it's owned by a dma_buf, because in this case we use the same
		 * handle across multiple memory transfer operations.
		 */
		if (handle->tipc.obj_id == new_handle->tipc.obj_id) {
			if (owns_ffa_handle(new_handle)) {
...
static int tipc_shared_handle_drop(struct tipc_shared_handle *shared_handle)
{
	...
	if (!owns_ffa_handle(shared_handle)) {
		/*
		 ...
		 *
		 * This warning is only applicable in the case where registering
		 * a handle multiple time is not allowed, i.e. when handle
		 * dma_buf doesn't own the handle.
		 */
