...
static void tipc_shared_handle_register_locked(struct tipc_virtio_dev *vds,
					       struct tipc_shared_handle
					       *new_handle)
{
	...
	while (*new) {
		...
		if (handle->tipc.obj_id == new_handle->tipc.obj_id) {
			...
			dev_err(dev, "Tried to register same handle twice\n");
			return;
					...
					**shared_handle)
{
	...
	mutex_lock(&vds->lock);
	tipc_shared_handle_register_locked(vds, *shared_handle);
	*shared_handle = NULL;
	mutex_unlock(&vds->lock);
}

static struct tipc_shared_handle *
tipc_shared_handle_take_locked(struct tipc_virtio_dev *vds,
			       trusty_shared_mem_id_t obj_id)
{
							  ...
							  obj_id)
{
	...
	mutex_lock(&vds->lock);
	out = tipc_shared_handle_take_locked(vds, obj_id);
	mutex_unlock(&vds->lock);
	return out;
}
	...
	if (ret) {
		for (release_idx = 0; release_idx < req.shm_cnt; release_idx++)
			tipc_shared_handle_take(vds,
						shm_handles[release_idx]->tipc
						.obj_id);
		goto out;
	}

	/* Don't drop txbuf, it was sent in tipc_chan_queue_msg() */
	txbuf = NULL;
	/* Don't drop shm_handles, we sent successfully */
	shm_idx = 0;
			    ...
			    struct tipc_release_body *req, size_t len)
{
	...
	if (!handle) {
		dev_err(dev, "Received release control message for untracked handle: 0x%llx\n"
			, req->id);
		return;
	}
	...
	if (ret) {
		dev_err(dev, "Failed to release handle 0x%llx upon request: (%d)\n"
			, req->id, ret);
		/*
	...
	for (shm_idx = 0; shm_idx < msg->shm_cnt; shm_idx++) {
		...
		if (shared_handle) {
			if (tipc_shared_handle_drop(shared_handle))
				dev_err(dev, "Failed to drop handle found in dropped buffer"
					);
		} else {
			dev_err(dev, "Found handle in dropped buffer which was not registered to tipc device..."
				);
		}
	}
}
