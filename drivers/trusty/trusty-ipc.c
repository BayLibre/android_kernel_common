		       ...
		       struct tipc_shared_handle **out)
{
	...
	case TRUSTY_SEND_SECURE:
		if (ret) {
			dev_err(dev, "No shared mem ID allocated (%d)\n", ret);
			goto cleanup_handle;
		}
	...
	/* TODO: Make this an error once all clients migrate */
	WARN_ONCE(!ret, "Use TRUSTY_SEND_SECURE instead");
			    ...
			    const struct tipc_send_msg_req __user *arg)
{
	...
	for (shm_idx = 0; shm_idx < req.shm_cnt; shm_idx++) {
		transfer_kind = shm[shm_idx].transfer;
		if ((transfer_kind != TRUSTY_SHARE) &&
		    (transfer_kind != TRUSTY_LEND) &&
		    (transfer_kind != TRUSTY_SEND_SECURE)) {
			dev_err(dev, "Unknown transfer type: 0x%x\n",
				shm[shm_idx].transfer);
			goto shm_share_failed;
		}
		ret = dn_share_fd(dn, shm[shm_idx].fd, transfer_kind,
				  &shm_handles[shm_idx]);
		if (ret) {
