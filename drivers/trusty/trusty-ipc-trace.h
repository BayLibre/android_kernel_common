...
DECLARE_EVENT_CLASS(trusty_ipc_chan_class,
	TP_PROTO(u32 chan),
	TP_ARGS(chan),
	TP_STRUCT__entry(
		__field(u32, chan)
	...
	TP_printk("chan=%u", __entry->chan)
);

#define DEFINE_TRUSTY_IPC_CHAN_EVENT(name)	\
DEFINE_EVENT(trusty_ipc_chan_class, name,	\
	TP_PROTO(u32 chan), \
	TP_ARGS(chan))

DEFINE_TRUSTY_IPC_CHAN_EVENT(trusty_ipc_connected);
DEFINE_TRUSTY_IPC_CHAN_EVENT(trusty_ipc_disconnected);
DEFINE_TRUSTY_IPC_CHAN_EVENT(trusty_ipc_shutdown);
...
DECLARE_EVENT_CLASS(trusty_ipc_msg_class,
	...
	TP_printk("chan=%u, buf_id=0x%llx, size=%zu, shm_cnt=%zu", __entry->chan, __entry->buf,
		  __entry->size, __entry->shm_cnt)
);
...
TRACE_EVENT(trusty_ipc_tx_buf_done,
	...
	TP_printk("buf_id=0x%llx, free_cnt=%u", __entry->buf_id, __entry->free_cnt)
);
