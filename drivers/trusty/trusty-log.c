                         ...
                         uint32_t partial_entry_size)
{
    ...
    while (i < sizeof(s->line_buffer) - 1 &&
        ...
        /* read footer */
        offset = log_offset + s->header.entry_size - sizeof(struct log_data_footer);
        read_log_data(s, offset, sizeof(struct log_data_footer), value);
        s->footer = *(struct log_data_footer*)(value);

        /* read header */
        header_len = s->header.entry_size - sizeof(struct log_data_footer);
        read_log_data(s, log_offset, header_len, value);

        /* handle partial entry if available */
        if (partial_entry_size >
            (sizeof(struct log_data_footer) + s->footer.app_name_len)) {
            partial_log_len = partial_entry_size - sizeof(struct log_data_footer);
            if (partial_log_len > (s->footer.log_len + s->footer.app_name_len)) {
                partial_log_len = s->footer.log_len + s->footer.app_name_len;
            }
            s->footer.log_len = partial_log_len - s->footer.app_name_len;
		 ...
		 */
                if (alloc - get > log->sz) {
                    ...
                    while ((entry - entry_size) >= (alloc - log->sz)) {
                         ...
                         * then there is possibility of currupting the entries
                         * which are already traversed.
                         */
                        if (alloc != log->alloc) {
                            alloc = log->alloc;
                    ...
                    /* handle partial entry if available */
                    if (entry - (alloc - log->sz) > sizeof(struct log_data_footer)) {
                ...
                if (__ratelimit(&trusty_log_rate_limit)) {
                    if (s->footer.app_name_len > 0) {
                        char *app_name = &s->header.data[s->footer.log_len];
