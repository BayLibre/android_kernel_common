 ...
 * (currently little less than 100 lines) is stored unthrottled
 * in dmesg. Of course this cannot be guaranteed when the rate limit is reached
...
static struct ratelimit_state trusty_log_rate_limit =
        RATELIMIT_STATE_INIT("trusty_log", 1 * HZ, 250);
