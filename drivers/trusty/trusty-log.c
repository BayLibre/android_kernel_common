 ...
 *  - allocated size needs to be power-of-two number of pages, plus one extra
 *    page for the header.
 *  - Rounding up from 18 KB and adding one page gives a total of 9 pages.
 *  The log size can be adjusted by setting the "trusty_log.num_pages" parameter
 *  on the kernel command line, but must be from 2-257 pages (giving a buffer
 *  size of 4 KB to 1 MB).
 *  The actual value being used can be read at
 *  /sys/module/trusty_log/parameters/num_pages
 */

#define TRUSTY_LOG_DEFAULT_NUM_PAGES (9)
#define TRUSTY_LOG_MAX_PAGES (257)
#define TRUSTY_LINE_BUFFER_SIZE (256)

static unsigned int trusty_log_num_pages = 9;
module_param_named(num_pages, trusty_log_num_pages, uint, 0400);
MODULE_PARM_DESC(num_pages, "Size of shared buffer in pages, must be 1 plus a power of two, 2-257");
...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	/*
	 * Check that the specified number of pages is a power-of-two plus one,
	 * and falls within the allowed range.
	 */
	if (trusty_log_num_pages < 2 ||
	    trusty_log_num_pages > TRUSTY_LOG_MAX_PAGES ||
	    (trusty_log_num_pages != 2 &&
	    (trusty_log_num_pages & 1) == 0) ||
	    ((trusty_log_num_pages - 1) & (trusty_log_num_pages - 2)) != 0) {
		dev_info(s->dev, "Invalid num_pages: %d\n",
			 trusty_log_num_pages);
		trusty_log_num_pages = TRUSTY_LOG_DEFAULT_NUM_PAGES;
	}
	s->log_num_pages = trusty_log_num_pages;
	 ...
	 * because s->log_num_pages is always > 1
	 */
	result = trusty_share_memory(s->trusty_dev, &mem_id, s->sg,
