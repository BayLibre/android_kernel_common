...
struct trusty_log_sink_state {
	...
	u32 last_successful_get; /* track the last successful get in order to retry */
};
			   ...
			   struct trusty_log_sink_state *sink)
{
	...
	get = sink->get;
	put = log->put;
	trusty_panicked = trusty_get_panic_status(s->trusty_dev);
	alloc = log->alloc;
	if (put == get) {
		return 0;
	}
	...
	sink->last_successful_get = sink->get - read_chars;
	if (sink->sfile) {
...
static inline int trusty_log_logbuffer_try_register(struct trusty_log_state *s)
{
	...
	BUG_ON(!s->logbuffer.misc.name);
	dev_info(s->dev, "/dev/%s registration successful\n",
		 s->logbuffer.misc.name);
	return 0;
}
...
static void trusty_dump_logs(struct trusty_log_state *s, bool retry)
{
	void *get;
	u32 start;
	start = retry ? s->klog_sink.last_successful_get : s->klog_sink.get;
