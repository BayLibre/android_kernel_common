...
static void trusty_hafnium_data_ready(struct sock *sk)
{
	...
	pr_debug("%s: in_smc %d, trusty_dev %d\n", __func__, s->in_smc, !!s->trusty_dev);
				       ...
				       struct trusty_custom_smc *data)
{
	...
	dev_dbg(s->dev, "smc 0x%x 0x%x 0x%x 0x%x\n", r0, r1, r2, r3);
...
{
	struct trusty_hafnium_state *s = trusty_hafnium_smc_to_state(data);
	pr_err("trusty_hafnium_share_memory: vm %d paddr %pa, size %zd, flags 0x%x \n",
