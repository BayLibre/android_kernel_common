...
typedef ulong (*smc_func)(ulong r0, ulong r1, ulong r2, ulong r3,
			      struct trusty_custom_smc *dummy_smc);

static inline ulong ikgt_smc_instr(ulong r0, ulong r1, ulong r2, ulong r3,
			      struct trusty_custom_smc *dummy_smc)
{
...
static struct trusty_custom_smc x86_64_smc;
...
static void register_trusty_custom_smc(smc_func smc)
{
	if (smc) {
		x86_64_smc.smc = smc;
	} else {
		x86_64_smc.smc = ikgt_smc_instr;
	}
}
...
static int trusty_x86_64_probe(struct platform_device *pdev)
{
	...
	/* Use default iKGT smc implementation */
	register_trusty_custom_smc(NULL);
