...
{
	/* Trusty interrupt should not be received at this point */
	BUG_ON(1);
 ...
 * Trusty IRQs and vectors used here might conflict with hardware device
 * drivers.
 *
 * Linux kernel has reserved following IRQs for backward compatiblity.
 * (More details about IRQs in Intel 8259 PIC please refer:
 ...
 *
 * Following IRQs would be used as Trusty interrupt since they are
 ...
 *
 * setup_irq is used to statically setup interrupts in the early boot process,
 * this function is used here to reserve IRQs and vectors for Trusty interrupt.
 *
 * Since kernel assigns IRQ 0 and vector 0x20 to 8254 PIT timer at early boot
 * stage, when setup_irq is invoked to reserved IRQs and vectors for Trusty at
 * core init stage, vectors allocated begin from 0x21.
 *
...
int trusty_x86_64_irq_get_mapping(unsigned int vector)
{
	int idx;
	int size = sizeof(trusty_x86_64_irq)/sizeof(struct trusty_x86_64_irq_s);
...
void trusty_x86_64_irq_retrigger(unsigned int irq)
{
	int idx;
	int size = sizeof(trusty_x86_64_irq)/sizeof(struct trusty_x86_64_irq_s);
	struct trusty_x86_64_irq_s *irq_status = NULL;
