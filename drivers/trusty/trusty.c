...
static int trusty_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		/*
		 * TODO: replace kthread_create with kthread_create_on_cpu above
		 * instead of calling kthread_bind once kthread_create_on_cpu
		 * get exported. This will rebind the thread to the correct cpu
		 * every time is unparked. For now, park the thread when the cpu
		 * goes offline and hope it does not get migrated.
		 */
...
static int trusty_remove(struct platform_device *pdev)
{
	...
	ret = cpuhp_state_remove_instance(trusty_cpuhp_slot,
					  &s->cpuhp_node);
	WARN_ON(ret);
