					    ...
					    unsigned long a2)
{
	...
	while (true) {
		...
		if (s->trusty_share_state)
			trusty_set_actual_nice(smp_processor_id(),
					s->trusty_share_state, task_nice(current));
		...
		if (ret == SM_ERR_INTERRUPTED) {
			 ...
			 */
			trusty_enqueue_nop(dev, NULL, true);
		}
		local_irq_enable();
...
void trusty_enqueue_nop(struct device *dev, struct trusty_nop *nop, int escalate_prio)
{
	...
	if (escalate_prio && task_nice(current) != LINUX_NICE_FOR_TRUSTY_PRIORITY_HIGH)
