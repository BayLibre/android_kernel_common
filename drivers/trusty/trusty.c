			   ...
			   pgprot_t pgprot, u64 tag, bool lend)
{
	...
	if (!ret) {
		*id = ffa_handle;
		/* Tracepoint: capture pass case */
		trace_trusty_share_memory(len, nents, lend, ffa_handle);
...
err_encode_page_info:
	...
	/* Tracepoint: capture failure case */
	trace_trusty_share_memory(len, nents, lend, 0);
	return ret;
}
...
void trusty_enqueue_nop(struct device *dev, struct trusty_nop *nop)
{
	...
	trace_trusty_enqueue_nop(s->api_version, s->trusty_panicked);
	preempt_disable();
