...
struct pvclock_vsyscall_time_info *virtio_pvclock_page;
...
static int virtpvclock_probe(struct virtio_device *vdev)
{
	...
	vdev->priv = vp = kzalloc(sizeof(*vp), GFP_KERNEL);
	if (!vp) {
		err = -ENOMEM;
		goto out;
	}
...
static int virtpvclock_validate(struct virtio_device *vdev)
{
	if (virtio_has_feature(vdev, VIRTIO_PVCLOCK_F_CLOCKSOURCE_RATING)) {
		uint32_t rating = virtio_cread32(
			vdev, offsetof(struct virtio_pvclock_config,
				       clocksource_rating));
		if (rating > MAX_CLOCKSOURCE_RATING) {
			...
			virtio_pvclock_clocksource.rating =
				(int)MAX_CLOCKSOURCE_RATING;
		} else {
