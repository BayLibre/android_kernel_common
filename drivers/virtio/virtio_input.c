...
#define RUMBLE_EFFECT_TYPE EV_MAX + 1
...
static int virtinput_ff_upload(
  ...
  if (effect->type == FF_RUMBLE) {
    ...
    if (ret != 0) {
      dev_error(&dev->dev, "virtinput_ff_upload virtinput_send_status error: %d", ret);
    }
    return ret;
  } else {
    dev_warn(&dev->dev,
             "virtinput_ff_upload did not receive effect type FF_RUMBLE: %d",
             effect->type);
  }
  return 0;
}
