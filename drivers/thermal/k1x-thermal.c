...
static int k1x_thermal_probe(struct platform_device *pdev)
{
	...
	s->clk = devm_clk_get(dev, "thermal_core");
	if (IS_ERR(s->clk))
		return PTR_ERR(s->clk);

	clk_prepare_enable(s->clk);

	/* Enable bus clock if available (required for register access) */
	{
		struct clk *bus_clk = devm_clk_get_optional(dev, "bus");
	...
	for (i = s->sr[0]; i <= s->sr[1]; ++i) {
		...
		ret = devm_request_threaded_irq(dev, s->irq, k1x_thermal_irq,
				...
				thermal_zone_device_type(s->sdesc[i].tzd), s->sdesc + i);
		if (ret < 0) {
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Spacemit K1X Thermal Sensor Driver");
