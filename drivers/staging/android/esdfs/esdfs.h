...
static inline const struct cred *esdfs_override_creds(
		...
		struct esdfs_inode_info *info, const char* name, int *mask)
{
	...
	if (test_opt(sbi, GID_DERIVATION)) {
		creds->fsuid = esdfs_get_derived_lower_uid(sbi, info);
		creds->fsgid = esdfs_get_derived_lower_gid(sbi, info, name);
	} else {
