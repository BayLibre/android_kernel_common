...
static gid_t get_type(struct esdfs_sb_info *sbi, const char *name)
{
	...
	if (ext && ext[0]) {
		ext = &ext[1];
		id = esdfs_from_kgid(sbi, pkglist_get_ext_gid(ext));
		return id?:AID_MEDIA_RW;
	}
	return AID_MEDIA_RW;
}

kuid_t esdfs_get_derived_lower_uid(struct esdfs_sb_info *sbi,
...
kgid_t esdfs_get_derived_lower_gid(struct esdfs_sb_info *sbi,
				struct esdfs_inode_info *info, const char *name)
{
	...
	case ESDFS_TREE_ANDROID_DATA:
	case ESDFS_TREE_ANDROID_MEDIA:
		if (S_ISDIR(info->vfs_inode.i_mode) || !name)
