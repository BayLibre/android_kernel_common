...
static const struct attribute_group ion_device_group = {
	.attrs = ion_device_attrs,
};
...
static int ion_init_sysfs(void)
{
	...
	kobj = kobject_create_and_add("ion", kernel_kobj);
	if (!kobj)
		return -ENOMEM;

	ret = sysfs_create_group(kobj, &ion_device_group);
	if (ret) {
		kobject_put(kobj);
		return ret;
	}
