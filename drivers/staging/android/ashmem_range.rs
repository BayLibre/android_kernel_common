...
use crate::ashmem_mutex::{AshmemGuard, LockedByAshmem};
use core::{
    mem::MaybeUninit,
...
type NewRange = UniqueArc<MaybeUninit<Range>>;

impl Range {
    pub(crate) fn purged(&self, guard: &AshmemGuard<'_>) -> bool {
        self.inner.get_ref(guard).purged
    }

    pub(crate) fn pgstart(&self, guard: &AshmemGuard<'_>) -> usize {
        self.inner.get_ref(guard).pgstart
    }

    pub(crate) fn pgend(&self, guard: &AshmemGuard<'_>) -> usize {
        self.inner.get_ref(guard).pgend
    }

    pub(crate) fn size(&self, guard: &AshmemGuard<'_>) -> usize {
        let inner = self.inner.get_ref(guard);
        inner.pgend - inner.pgstart + 1
    }

    pub(crate) fn is_before_page(&self, page: usize, guard: &AshmemGuard<'_>) -> bool {
        let inner = self.inner.get_ref(guard);
        inner.pgend < page
    }

    pub(crate) fn contains_page(&self, page: usize, guard: &AshmemGuard<'_>) -> bool {
        let inner = self.inner.get_ref(guard);
    ...
    pub(crate) fn unpin(
        ...
        while let Some(curr) = cursor {
            ...
            if curr.current().overlaps_with_range(pgstart, pgend, guard) {
                ...
                pgend = usize::min(pgend, curr.current().pgend(guard));
        ...
        let new_range = new_range.pin_init_with(pin_init!(Range {
            lru <- ListLinks::new(),
            unpinned <- ListLinks::new(),
            inner: LockedByAshmem::new(RangeInner {
                pgstart,
                pgend,
                purged,
            }),
