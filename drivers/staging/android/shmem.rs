...
pub(crate) unsafe fn file_get_fpos(file: &LocalFile) -> i64 {
...
///
/// Caller must ensure that access to the file position is properly synchronized.
pub(crate) unsafe fn file_set_fpos(file: &LocalFile, pos: i64) {
    // SAFETY: Caller ensures that this is okay.
...
impl ShmemFile {
    ...
    pub(crate) fn vfs_llseek(&self, offset: i64, whence: c_int) -> Result<i64> {
    ...
    pub(crate) fn vfs_iter_read(&self, iov: &mut IovIterDest<'_>, pos: &mut i64) -> Result<i64> {
