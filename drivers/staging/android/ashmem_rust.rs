...
impl MiscDevice for Ashmem {
    ...
    fn mmap(me: Pin<&Ashmem>, vma: &VmAreaNew) -> Result<()> {
...
impl Ashmem {
    ...
    fn purge_all_caches(&self) -> Result<isize> {
        ...
        Ok(isize::try_from(ret).unwrap_or(isize::MAX))
    }
}
...
fn ashmem_memfd_ioctl_inner(file: &File, cmd: u32, arg: usize) -> Result<isize> {
    ...
    fn get_seals(file: &File) -> Result<u64> {
    ...
    match cmd {
        ...
        bindings::ASHMEM_SET_PROT_MASK => {
            ...
            let has_write = seals & WRITE_SEALS_MASK == 0;
            let set_write = prot & PROT_WRITE != 0;

            if !has_write && set_write {
                // Can't add PROT bits.
                return Err(EINVAL);
            }
