...
/*
 * This helper will continue to return the SVID that the port needs to send
 * DISCOVER_MODES to until the pmdata->svid_index is incremented after
 * svdm_consume_modes() in tcpm_pd_svdm().
 */
static int svdm_discover_next_mode(struct tcpm_port *port, struct pd_mode_data *pmdata) {
		...
		case CMD_DISCOVER_SVID:
			...
			if (svdm_consume_svids(port, p, cnt)) {
				...
				rlen = 1;
				   ...
				   svdm_discover_next_mode(port, modep)) {
				u16 svid = svdm_discover_next_mode(port, modep);
