...
static void cros_ucsi_add_altmodes(struct ucsi_connector *con)
{
	typec_mode_selection_start(con->partner,
			mode_selection_delay, mode_selection_timeout);
}

static void cros_ucsi_remove_altmodes(struct ucsi_connector *con)
{
	typec_mode_selection_delete(con->partner);
}

static void cros_ucsi_update_altmode(struct ucsi_connector *con,
		const u16 svid)
{
...
static const struct ucsi_operations cros_ucsi_ops = {
	...
	.add_altmodes = cros_ucsi_add_altmodes,
	.update_altmode = cros_ucsi_update_altmode,
	.remove_altmodes = cros_ucsi_remove_altmodes,
};
