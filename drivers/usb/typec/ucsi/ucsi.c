...
{
	struct ucsi_vendor *vucsi = container_of(work,
				    struct ucsi_vendor, work.work);
	int ret;

	ret = ucsi_init(&vucsi->ucsi);
	if (ret)
		dev_err(vucsi->ucsi.dev, "PPM init failed (%d)\n", ret);

	if (ret == -EPROBE_DEFER) {
		if (vucsi->work_count++ > UCSI_ROLE_SWITCH_WAIT_COUNT)
			return;

		queue_delayed_work(system_long_wq, &vucsi->work,
...
{
	struct ucsi *ucsi;
	struct ucsi_vendor *vucsi;

	if (!ops || !ops->read || !ops->sync_write || !ops->async_write)
		return ERR_PTR(-EINVAL);

	vucsi = kzalloc(sizeof(*vucsi), GFP_KERNEL);
	if (!vucsi)
		return ERR_PTR(-ENOMEM);

	ucsi = &vucsi->ucsi;
	INIT_DELAYED_WORK(&vucsi->work, ucsi_init_work);
...
{
	struct ucsi_vendor *vucsi = container_of(ucsi,
				    struct ucsi_vendor, ucsi);
	kfree(vucsi);
}
...
int ucsi_register(struct ucsi *ucsi)
{
	...
	queue_delayed_work(system_long_wq, &vucsi->work, 0);
...
void ucsi_unregister(struct ucsi *ucsi)
{
	...
	cancel_delayed_work_sync(&vucsi->work);
