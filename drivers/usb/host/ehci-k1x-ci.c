...
#define DRIVER_DESC "EHCI Spacemit K1-x driver"

static const char hcd_name[] = "ehci-k1x-ci";
...
static const struct ehci_driver_overrides mv_ehci_overrides __initconst = {
	.reset = mv_ehci_reset,
};
...
static int __init ehci_k1x_init(void)
{
	if (usb_disabled())
		return -ENODEV;

	ehci_init_driver(&mv_ehci_hc_driver, &mv_ehci_overrides);
	return platform_driver_register(&ehci_k1x_driver);
}
module_init(ehci_k1x_init);

static void __exit ehci_k1x_exit(void)
{
	platform_driver_unregister(&ehci_k1x_driver);
}
module_exit(ehci_k1x_exit);
