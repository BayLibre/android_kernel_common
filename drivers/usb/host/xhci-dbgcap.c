...
dbc_ep_do_queue(struct dbc_ep *dep, struct dbc_request *req)
{
	...
	if (dma_mapping_error(dev, req->dma)) {
		printk("failed to map buffer\n");
		return -EFAULT;
	}

	ret = xhci_dbc_queue_bulk_tx(dep, req);
	if (ret) {
		printk("failed to queue trbs\n");
		dma_unmap_single(dev,
				 req->dma,
				 req->length,
				 dbc_ep_dma_direction(dep));
...
static void dbc_handle_tx_event(struct xhci_hcd *xhci, union xhci_trb *event)
{
	...
	case COMP_SHORT_PACKET:
		printk("short packet %d detected\n", comp_code);
		status = 0;
		break;
	case COMP_TRB_ERROR:
	...
	case COMP_STALL_ERROR:
		printk("tx error %d detected\n", comp_code);
		status = -comp_code;
		break;
	default:
		printk("unknown tx error %d\n", comp_code);
		status = -comp_code;
		break;
	}
