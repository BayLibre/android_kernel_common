...
static inline uint32_t UDC_READ_REG32(uint32_t volatile *reg)
{
	return readl(reg);
}
...
static inline void UDC_MODIFY_REG32(uint32_t volatile *reg, uint32_t clear_mask, uint32_t set_mask)
{
