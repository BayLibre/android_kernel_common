...
static int mv_udc_dtd_queue_show(struct seq_file *s, void *unused)
{
...
static int mv_udc_registers_show(struct seq_file *s, void *unused)
{
	struct mv_udc *udc = s->private;
	unsigned long flags;
...
static int mv_udc_probe(struct platform_device *pdev)
{
	...
	if (!udc->extcon && of_property_read_bool(np, "monitor-vbus-with-psy")) {
		udc->vbus_psy = devm_power_supply_get_by_reference(&pdev->dev, "monitor-vbus-with-psy");
