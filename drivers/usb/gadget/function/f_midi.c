...
static inline void f_midi_unregister_card(struct f_midi *midi)
{
	...
	if (midi->card) {
		opts = container_of(midi->func.fi, struct f_midi_opts,
				func_inst);
		android_remove_midi_device(&opts->uevent_opts);
...
static int f_midi_register_card(struct f_midi *midi)
{
	...
	err = android_set_midi_device_info(&opts->uevent_opts, card->number, rmidi->device);
	if (err < 0) {
...
static struct usb_function_instance *f_midi_alloc_inst(void)
{
	...
	if (android_create_midi_device(&opts->uevent_opts)) {
		kfree(opts);
