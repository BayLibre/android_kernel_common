...
static void android_work(struct work_struct *data)
{
	...
	char *disconnected_str[2] = { "USB_STATE=DISCONNECTED", NULL };
	char *connected_str[2] = { "USB_STATE=CONNECTED", NULL };
	char *configured_str[2] = { "USB_STATE=CONFIGURED", NULL };
	unsigned long flags;
	...
	if (connected) {
		kobject_uevent_env(&opts->dev->kobj, KOBJ_CHANGE, connected_str);
		//dev_dbg(&opts->dev, "%s: sent uevent %s\n", __func__, connected_str);
		uevent_sent = true;
	}

	if (opts->configured) {
		kobject_uevent_env(&opts->dev->kobj, KOBJ_CHANGE, configured_str);
		//dev_dbg(&opts->dev, "%s: sent uevent %s\n", __func__, configured_str);
		uevent_sent = true;
	}

	if (disconnected) {
		kobject_uevent_env(&opts->dev->kobj, KOBJ_CHANGE, disconnected_str);
		//dev_dbg(&opts->dev, "%s: sent uevent %s\n", __func__, disconnected_str);
		uevent_sent = true;
	}
