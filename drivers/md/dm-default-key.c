...
static int default_key_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	err = blk_crypto_start_using_mode(dkc->dev->bdev,
	...
	ti->may_passthrough_inline_crypto = 1;
...
static int default_key_map(struct dm_target *ti, struct bio *bio)
{
	...
	/* Calculate the DUN and enforce data-unit alignment. */
	dun[0] = dkc->iv_offset + sector_in_target;
	if (dun[0] & ((dkc->sector_size >> SECTOR_SHIFT) - 1))
		return DM_MAPIO_KILL;
	dun[0] >>= dkc->sector_bits - SECTOR_SHIFT;
