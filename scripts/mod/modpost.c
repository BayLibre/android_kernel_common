...
static struct list_head *handle_white_list_exports(const char *white_list)
{
	char *buf, *p, *name;
	struct permitted_symbol *ps;
	LIST_HEAD(permitted_symbols);
...
static void write_permitted_imports_c_file(struct list_head *permitted_symbols)
{
	...
	buf_printf(&buf, "#include \"kernel/module/internal.h\"\n\n");
	buf_printf(&buf, "int _nr_permitted_symbol_imports(void) { return %d; }\n\n", nr_white_list_symbols);
...
int main(int argc, char **argv)
{
	...
	if (trim_unused_exports) {
		struct list_head *permitted_imports;
		if (unused_exports_white_list)
			permitted_imports = handle_white_list_exports(unused_exports_white_list);
		else
			INIT_LIST_HEAD(permitted_imports);
