...
generate_header() {
	...
	#define NO_OF_${symbol_type^^}_SYMBOLS \\
	$(printf '\t')(sizeof(gki_${symbol_type}_symbols) / sizeof(gki_${symbol_type}_symbols[0]))
	#define MAX_${symbol_type^^}_NAME_LEN (${max_name_len})

	static const char gki_${symbol_type}_symbols[][MAX_${symbol_type^^}_NAME_LEN] = {
	EOT
