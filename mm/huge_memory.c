...
static int alloc_dst_pages(struct folio *src)
{
	int i;
	int nr_pages = folio_nr_pages(src);
	int nid = folio_nid(src);
	gfp_t gfp = (GFP_HIGHUSER_MOVABLE & ~__GFP_FS) |
		    __GFP_THISNODE | __GFP_HIGH | __GFP_NOWARN;
...
static bool lru_add_dst(struct lruvec *lruvec, struct folio *src, struct folio *dst)
{
	...
	if (!lru_gen_add_dst(lruvec, dst)) {
		...
		int nr_pages = folio_nr_pages(dst);

		if (folio_test_unevictable(dst))
			dst->mlock_count = 0;
		else
			list_add_tail(&dst->lru, &src->lru);
		update_lru_size(lruvec, lru, zone, nr_pages);
	}
		...
		pgoff_t end)
{
	...
	int i, nr_dropped = 0;
	bool inplace = folio_can_split(folio);
	...
	for (i = nr - 1; i >= inplace; i--) {
		struct page *dst = folio_dst_page(folio, i);

		if (!dst) {
			clear_src_swpent(folio, i);
			continue;
		}
