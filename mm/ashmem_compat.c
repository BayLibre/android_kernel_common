...
enum ashmem_compat_mask_bitpos {
	COMPAT_ENABLED,
	MAPPED_ONCE,
};
...
static void set_ashmem_compat_mask_bit(struct file *file, enum ashmem_compat_mask_bitpos pos)
{
	unsigned long attr_mask = get_ashmem_compat_mask(file);
...
{
	return get_ashmem_compat_mask(file) & (1 << COMPAT_ENABLED);
}

static bool is_ashmem_buffer_mapped(struct file *file)
{
	return get_ashmem_compat_mask(file) & (1 << MAPPED_ONCE);
}
...
static int ashmem_compat_set_size(struct file* file, size_t size)
{
	...
	/*
	 * Synchronize against updates to the file seals from mmap(). mmap() will seal the file's
	 * size.
	 *
...
/* Assumes the file's inode lock is held in exclusive mode. */
...
static int ashmem_compat_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (!ret)
		set_ashmem_compat_mask_bit(file, MAPPED_ONCE);
...
void install_ashmem_compat_fops(struct file *file, bool ashmem_compatible)
{
	...
	if (ashmem_compatible)
		set_ashmem_compat_mask_bit(file, COMPAT_ENABLED);
