...
static inline bool wait_for_vma_users(struct vm_area_struct *vma)
{
	 ...
	 */
	if (likely(atomic_cmpxchg(&vma->file_ref_count, 1, -1) == 1))
		return true;
	...
	result = !wait_event_interruptible(vma_users_wait,
			atomic_cmpxchg(&vma->file_ref_count, 1, -1) == 1);
...
{
	return atomic_inc_unless_negative(&vma->file_ref_count);
}
...
bool get_vma(struct vm_area_struct *vma) { return true; }
void put_vma(struct vm_area_struct *vma) {}
