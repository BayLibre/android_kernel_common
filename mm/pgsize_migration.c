		       ...
		       unsigned long nr_pages)
{
	...
	vm_flags_set(vma, vma->vm_flags & ~VM_PAD_MASK);
...
struct vm_area_struct *get_pad_vma(struct vm_area_struct *vma)
{
	...
	vm_flags_set(pad, pad->vm_flags & ~(VM_READ|VM_WRITE|VM_EXEC));

	/* Remove padding bits */
	vm_flags_set(pad, pad->vm_flags & ~VM_PAD_MASK);
		   ...
		   unsigned long addr, int new_below)
{
	...
	if (nr_vma2_pages >= nr_pad_pages) { 			/* Case 1 & 3*/
		vm_flags_set(first, first->vm_flags & ~VM_PAD_MASK);
