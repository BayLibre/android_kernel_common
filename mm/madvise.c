				...
				struct mm_walk *walk)
{
	...
	trace_android_vh_madvise_pageout_bypass(mm, pageout, &bypass);
	if (bypass)
		return -EBUSY;
...
static int madvise_pageout(struct vm_area_struct *vma,
			struct vm_area_struct **prev,
			unsigned long start_addr, unsigned long end_addr)
{
	struct mm_struct *mm = vma->vm_mm;
	...
	*prev = vma;
	...
	trace_android_vh_madvise_pageout_return_error(&return_error);
	if (return_error)
		return ret;
		...
		size_t, vlen, int, behavior, unsigned int, flags)
{
	...
	trace_android_vh_process_madvise_return_error(&return_error);
	if (return_error)
		goto release_mm;
