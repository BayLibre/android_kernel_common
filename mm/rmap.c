	...
	struct vm_area_struct *vma, unsigned long address, int flags)
{
	...
	if (compound) {
		...
		trace_android_vh_update_page_mapcount(page, true, &first, &success);
		if (unlikely(!success))
...
{
	int i, nr = 1;
	bool first_mapping, success = false;
	...
	if (compound && PageTransHuge(page)) {
		for (i = 0, nr = 0; i < thp_nr_pages(page); i++) {
			...
			if (success && first_mapping)
				nr++;
			else if (atomic_inc_and_test(&page[i]._mapcount))
				nr++;
		}
		...
		if (success && !first_mapping)
			goto out;
		else if (!atomic_inc_and_test(&page->_mapcount))
			goto out;
	}
...
static void page_remove_file_rmap(struct page *page, bool compound)
{
	...
	if (compound && PageTransHuge(page)) {
		for (i = 0, nr = 0; i < thp_nr_pages(page); i++) {
			...
			if (success && first_mapping)
				nr++;
			else if (atomic_add_negative(-1, &page[i]._mapcount))
				nr++;
		}
...
void page_remove_rmap(struct page *page, bool compound)
{
	...
	trace_android_vh_update_page_mapcount(page, false,
						&first_mapping, &success);
	if (success && !first_mapping)
		goto out; /* page still mapped by someone else? */
	else if (!atomic_add_negative(-1, &page->_mapcount))
		goto out; /* page still mapped by someone else? */
