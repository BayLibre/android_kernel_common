			    ...
			    unsigned int nr_entries)
{
	...
	trace_android_vh_account_swap_pages(si, nr_entries, false, &skip);
	if (!skip)
...
swp_entry_t get_swap_page_of_type(int type)
{
	...
	if (si->flags & SWP_WRITEOK) {
		...
		if (offset) {
			trace_android_vh_account_swap_pages(si, -1, false, &skip);
			if (!skip)
...
static void _enable_swap_info(struct swap_info_struct *p)
{
	...
	trace_android_vh_account_swap_pages(p, p->pages, true, &skip);
	if (!skip) {
