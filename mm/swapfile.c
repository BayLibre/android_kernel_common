				  ...
				  struct swap_cluster_info *ci, int nr_pages)
{
	...
	if (!(ci->flags & CLUSTER_FLAG_NONFULL)) {
		VM_BUG_ON(ci->flags & CLUSTER_FLAG_FREE);
		if (ci->flags & CLUSTER_FLAG_FRAG)
			SIE(p)->frag_cluster_nr[ci->order]--;
...
new_cluster:
	...
	if (order < PMD_ORDER) {
		...
		while (!list_empty(&si->nonfull_clusters[order])) {
			...
			list_move_tail(&ci->list, &SIE(si)->frag_clusters[order]);
			ci->flags = CLUSTER_FLAG_FRAG;
			SIE(si)->frag_cluster_nr[order]++;
		...
		if (!found) {
			 ...
			 */
			while (frags < SIE(si)->frag_cluster_nr[order]) {
				ci = list_first_entry(&SIE(si)->frag_clusters[order],
						      struct swap_cluster_info, list);
				/*
	...
	for (int o = 1; o < SWAP_NR_ORDERS; o++) {
		 ...
		 */
		while (!list_empty(&SIE(si)->frag_clusters[o])) {
			ci = list_first_entry(&SIE(si)->frag_clusters[o],
					      struct swap_cluster_info, list);
			offset = alloc_swap_scan_cluster(si, cluster_offset(si, ci),
							 &found, 0, usage);
			if (found)
					...
					sector_t *span)
{
	...
	for (i = 0; i < SWAP_NR_ORDERS; i++) {
		...
		INIT_LIST_HEAD(&SIE(p)->frag_clusters[i]);
		SIE(p)->frag_cluster_nr[i] = 0;
	}
