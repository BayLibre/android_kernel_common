...
static long get_name(struct file *file, void __user *name)
{
	 ...
	 * is larger than the maximum size of a name for a memfd buffer, so the name should always
	 * fit within the given buffer.
	 *
	 * Add 1 to the copy size to account for the NUL terminator.
	 */
	return copy_to_user(name, file_name, strlen(file_name) + 1) ? -EFAULT : 0;
}
...
long memfd_ashmem_shim_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	/*
	 * Older applications won't create memfds.
	 *
	 * Instead, they will receive a memfd that was created by a process, and that process
	 * already gave the memfd an immutable name when it created the memfd.
	 *
	 * It is still possible (and unlikely) that when ashmem was in use, one application created
	 * an ashmem buffer, and then passed it to another application and that application named
	 * the buffer.
	 *
	 * To not break that usecase, handle ASHMEM_SET_NAME as a nop. This shouldn't have negative
	 * consequences given that ashmem buffer names were meant to be used for debugging purposes,
	...
	case ASHMEM_GET_NAME:
		...
		break;
	/*
	 * When processes create memfds that are going to be shared with other processes in Android,
	 * they also specify the size of the memory region and seal the file against any size
	 * changes. Therefore, ASHMEM_SET_SIZE should not be supported in that case.
	 *
	 * If this does need to be implemented to handle cases where the memfd is not sealed against
	 * size changes, then do_ftruncate() can be invoked.
	 */
