...
static struct file *do_sync_mmap_readahead(struct vm_fault *vmf)
{
	...
	if (vm_flags & VM_SEQ_READ) {
		fpin = maybe_unlock_mmap_for_io(vmf, fpin);
		trace_android_vh_file_read_start(file, vmf->pgoff,
				ra->ra_pages, true);
		page_cache_sync_ra(&ractl, ra->ra_pages);
		trace_android_vh_file_read_end(file, vmf->pgoff);
		return fpin;
	}
	...
	trace_android_vh_file_read_start(file, vmf->pgoff, ra->size, true);
...
page_not_uptodate:
	 ...
	 */
	fpin = maybe_unlock_mmap_for_io(vmf, fpin);
	trace_android_vh_file_read_start(file, vmf->pgoff, 1, true);
	error = filemap_read_folio(file, mapping->a_ops->read_folio, folio);
	trace_android_vh_file_read_end(file, vmf->pgoff);
	if (fpin)
		goto out_retry;
