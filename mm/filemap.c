				     ...
				     struct file **fpin)
{
	...
	*fpin = maybe_unlock_mmap_for_io(vmf, *fpin);
	trace_android_vh_lock_folio_drop(1, &tsk, vmf, folio, *fpin);
	if (vmf->flags & FAULT_FLAG_KILLABLE) {
		if (__folio_lock_killable(folio)) {
			...
			trace_android_vh_lock_folio_drop(2, &tsk, vmf, folio, *fpin);
			return 0;
		}
	} else
		__folio_lock(folio);

	trace_android_vh_lock_folio_drop(3, &tsk, vmf, folio, *fpin);
	return 1;
}
