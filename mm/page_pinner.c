...
static ssize_t
read_page_pinner(struct file *file, char __user *buf, size_t count,
		 loff_t *ppos)
{
	unsigned long pfn;
	struct page *page;
	...
	for (; pfn < max_pfn; pfn++) {
		...
		page_pinner = get_page_pinner(page_ext);
		if (!atomic_read(&page_pinner->count))
			continue;
		...
		*ppos = (pfn - min_low_pfn) + 1;
		return print_page_pinner(buf, count, pfn,
					 get_pageblock_migratetype(page),
					 page->flags,
					 page_pinner->ts_usec, handle,
					 atomic_read(&page_pinner->count));
	}

	return 0;
}

static const struct file_operations proc_pinner_operations = {
	.read		= read_page_pinner,
};
...
static const struct file_operations proc_longterm_operations = {
...
static int __init page_pinner_init(void)
{
	...
	debugfs_create_file("pinner", 0400, pp_debugfs_root, NULL,
			    &proc_pinner_operations);

	debugfs_create_file("longterm_pinner", 0400, pp_debugfs_root, NULL,
			    &proc_longterm_operations);
