...
static int __kmalloc_min_size = KMALLOC_MIN_SIZE;
static int __kmalloc_shift_low = KMALLOC_SHIFT_LOW;

int kmalloc_min_size(void)
{
	return __kmalloc_min_size;
}
EXPORT_SYMBOL_GPL(kmalloc_min_size);

int kmalloc_shift_low(void)
{
	return __kmalloc_shift_low;
}
...
void __init init_kmalloc_cache_parameters(void)
{
	if (arch_kmalloc_minalign() != kmalloc_minalign) {
		...
		__kmalloc_shift_low = ilog2(kmalloc_minalign);
		__kmalloc_min_size = 1 << __kmalloc_shift_low;
	}
}
...
void __init setup_kmalloc_cache_index_table(void)
{
	...
	for (i = 8; i < __kmalloc_min_size; i += 8) {
		unsigned int elem = size_index_elem(i);

		if (elem >= ARRAY_SIZE(size_index))
			break;
		size_index[elem] = __kmalloc_shift_low;
	}

	if (__kmalloc_min_size >= 64) {
		/*
...
void __init create_kmalloc_caches(slab_flags_t flags)
{
	...
	for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
		for (i = __kmalloc_shift_low; i <= KMALLOC_SHIFT_HIGH; i++) {
			 ...
			 */
			if (__kmalloc_min_size <= 32 && i == 6 &&
					!kmalloc_caches[type][1])
				new_kmalloc_cache(1, type, flags);
			if (__kmalloc_min_size <= 64 && i == 7 &&
