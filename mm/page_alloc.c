	...
	*did_some_progress = 0;
	...
	if (!mutex_trylock(&oom_lock)) {
		*did_some_progress = 1;
		...
		trace_android_vh_mm_oom_exit(&oc, *did_some_progress);
		return NULL;
	}
		...
		enum compact_priority prio, enum compact_result *compact_result)
{
	...
	*compact_result = try_to_compact_pages(gfp_mask, order, alloc_flags, ac,
	...
	trace_android_vh_mm_exit(__func__, *compact_result, 0);
		...
		unsigned long *did_some_progress)
{
	...
	trace_android_vh_mm_enter(__func__);
	psi_memstall_enter(&pflags);
...
out:
	psi_memstall_leave(&pflags);
	trace_android_vh_mm_exit(__func__, *did_some_progress, retry_times);
	return page;
}
