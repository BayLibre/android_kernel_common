...
static void drain_pages_zone(unsigned int cpu, struct zone *zone)
{
	...
	pcp_ext = per_cpu_ptr((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset, cpu);
	pcp = &pcp_ext->pcp;
	if (pcp->count) {
		unsigned long flags;
...
static void __drain_all_pages(struct zone *zone, bool force_all_cpus)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (force_all_cpus) {
			...
			pcp = &per_cpu_ptr((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset, cpu)->pcp;
			if (pcp->count)
				has_pcps = true;
		} else {
			for_each_populated_zone(z) {
				pcp = &per_cpu_ptr((struct per_cpu_pages_ext __percpu *)z->per_cpu_pageset, cpu)->pcp;
				if (pcp->count) {
					has_pcps = true;
					break;
				}
...
void free_unref_page(struct page *page, unsigned int order)
{
	...
	pcp = pcp_spin_trylock_irqsave((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset, flags);
	if (pcp) {
...
void free_unref_page_list(struct list_head *list)
{
	...
	list_for_each_entry_safe(page, next, list, lru) {
		...
		if (zone != locked_zone) {
			...
			locked_zone = zone;
			pcp = pcp_spin_lock_irqsave((struct per_cpu_pages_ext __percpu *)locked_zone->per_cpu_pageset, flags);
		}
...
void show_free_areas(unsigned int filter, nodemask_t *nodemask)
{
	...
	for_each_populated_zone(zone) {
		...
		for_each_online_cpu(cpu)
			free_pcp += per_cpu_ptr((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset, cpu)->pcp.count;
	}
	...
	for_each_populated_zone(zone) {
		...
		printk(KERN_CONT
			...
			K(free_pcp),
			K(this_cpu_read(((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset)->pcp.count)),
...
void __meminit setup_zone_pageset(struct zone *zone)
{
	...
	zone->per_cpu_pageset = (struct per_cpu_pages __percpu *)alloc_percpu(struct per_cpu_pages_ext);
	for_each_possible_cpu(cpu) {
		struct per_cpu_pages *pcp;
...
void zone_pcp_reset(struct zone *zone)
{
	...
	if ((struct per_cpu_pages_ext __percpu *)zone->per_cpu_pageset != &boot_pageset) {
		for_each_online_cpu(cpu) {
			pzstats = per_cpu_ptr(zone->per_cpu_zonestats, cpu);
