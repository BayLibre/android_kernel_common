...
{
	return container_of(pcp, struct per_cpu_pageset_ext, pageset);
}
...
static bool free_unref_page_commit(struct page *page, int migratetype,
				   bool locked, unsigned int order)
{
	struct zone *zone = page_zone(page);
	struct per_cpu_pages *pcp;
	struct per_cpu_pageset_ext *ps_ext;
...
void free_unref_page(struct page *page, unsigned int order)
{
	...
	freed_pcp = free_unref_page_commit(page, migratetype, false, order);
...
+		 * then an expensive operation is already taking place.
		 */
		if (!free_unref_page_commit(page, migratetype, true, 0))
			...
			unsigned int alloc_flags)
{
	...
	pcp = this_cpu_ptr(zone->per_cpu_pageset);
	pcp->free_factor >>= 1;
	page = __rmqueue_pcplist(zone, order, migratetype, alloc_flags, pcp, gfp_flags, false);
	local_unlock_irqrestore(&pagesets.lock, flags);
	if (page) {
...
build_all_zonelists_init(void)
{
	...
	for_each_possible_cpu(cpu)
		per_cpu_pages_init(&per_cpu(boot_pageset, cpu).pageset, &per_cpu(boot_zonestats, cpu));
...
void __meminit setup_zone_pageset(struct zone *zone)
{
	...
	zone->per_cpu_pageset = &ps_ext->pageset;
	for_each_possible_cpu(cpu) {
		struct per_cpu_pages *pcp;
...
static __meminit void zone_pcp_init(struct zone *zone)
{
	 ...
	 */
	zone->per_cpu_pageset = &boot_pageset.pageset;
...
void zone_pcp_reset(struct zone *zone)
{
	...
	if (zone->per_cpu_pageset != &boot_pageset.pageset) {
		for_each_online_cpu(cpu) {
			pzstats = per_cpu_ptr(zone->per_cpu_zonestats, cpu);
