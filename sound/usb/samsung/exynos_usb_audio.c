...
static int exynos_usb_audio_map_buf(struct usb_device *udev)
{
	...
	dev_dbg(dev, "pcm in data buffer pa addr : %#08x %08x\n",
	...
	dev_dbg(dev, "pcm out data buffer pa addr : %#08x %08x\n",
...
static int exynos_usb_audio_pcmbuf(struct usb_device *udev)
{
	ABOX_IPC_MSG msg;
	struct platform_device *pdev = usb_audio->abox;
	struct device *dev = &pdev->dev;
	...
	if (!usb_audio->is_audio || !otg_connection) {
		dev_err(dev, "USB_AUDIO_IPC : is_audio is 0. return!\n");
		return -ENODEV;
	}
	...
	dev_dbg(dev, "pcm in data buffer pa addr : %#08x %08x\n",
			upper_32_bits(le64_to_cpu(hwinfo->in_dma)),
			lower_32_bits(le64_to_cpu(hwinfo->in_dma)));
	dev_dbg(dev, "erap param2 : %#08x param1 : %08x\n",
			erap_usb->param2, erap_usb->param1);
	dev_dbg(dev, "erap param4 : %#08x param3 : %08x\n",
...
static int exynos_usb_audio_setintf(struct usb_device *udev, int iface, int alt, int direction)
{
	...
	if (!usb_audio->pcm_open_done) {
		dev_err(dev, "USB_AUDIO_IPC : pcm node was not opeded!\n");
		return -EPERM;
	}

	dev_dbg(&udev->dev, "USB_AUDIO_IPC : %s, alt = %d\n",
				__func__, alt);
	...
	if (alt == 0 && direction == SNDRV_PCM_STREAM_PLAYBACK) {
		if (usb_audio->pcm_open_done &&
			...
			dev_dbg(dev, "out_deq wait required\n");
		...
		if (usb_audio->pcm_open_done &&
			...
			dev_dbg(dev, "in_deq wait required\n");
	...
	if (!usb_audio->is_audio || !otg_connection) {
		dev_err(dev, "USB_AUDIO_IPC : is_audio = 0. return!\n");
		return -ENODEV;
	}
	...
	dev_dbg(&udev->dev, "Alt#%d / Intf#%d / Direction %s / EP DEQ : %#08x %08x\n",
...
static int exynos_usb_audio_hcd(struct usb_device *udev)
{
	...
	dev_dbg(&udev->dev, "USB_AUDIO_IPC : %s udev->devnum %d\n", __func__, udev->devnum);
	dev_dbg(&udev->dev, "=======[Check HW INFO] ========\n");
	dev_dbg(&udev->dev, "slot_id : %d\n", hwinfo->slot_id);
	dev_dbg(&udev->dev, "dcbaa : %#08llx\n", hwinfo->dcbaa_dma);
	dev_dbg(&udev->dev, "save : %#08llx\n", hwinfo->save_dma);
	dev_dbg(&udev->dev, "in_ctx : %#08llx\n", hwinfo->in_ctx);
	dev_dbg(&udev->dev, "out_ctx : %#08llx\n", hwinfo->out_ctx);
	dev_dbg(&udev->dev, "erst : %#08x %08x\n",
			upper_32_bits(le64_to_cpu(hwinfo->erst_addr)),
			lower_32_bits(le64_to_cpu(hwinfo->erst_addr)));
	dev_dbg(&udev->dev, "use_uram : %d\n", hwinfo->use_uram);
	dev_dbg(&udev->dev, "===============================\n");
...
static int exynos_usb_audio_desc(struct usb_device *udev)
{
	...
	dev_dbg(&udev->dev, "paddr : %#08llx / offset : %#08llx / len : %d / speed : %d\n",
...
static int exynos_usb_audio_conn(int is_conn)
{
	ABOX_IPC_MSG msg;
	struct platform_device *pdev = usb_audio->abox;
	struct device *dev = &pdev->dev;
...
static int exynos_usb_audio_l2(int is_l2)
{
	...
	dev_dbg(dev, " USB Audio  %s the L2 power mode\n", is_l2 ? "ENTER" : "EXIT");
...
static irqreturn_t exynos_usb_audio_irq_handler(int irq, void *dev_id, ABOX_IPC_MSG *msg)
{
	struct IPC_ERAP_MSG *erap_msg = &msg->msg.erap;

	pr_debug("%s: IRQ = %d, type=%d, par1 = %d\n", __func__,
		...
		if (erap_msg->msgtype == REALTIME_USB) {
			...
			case IPC_USB_TASK:
				if (erap_msg->param.usbaudio.param1) {
					pr_debug("irq : %d /* param1 : 1 , IN EP task done */\n", irq);
				} else {
					pr_debug("irq : %d /* param0 : 0 , OUT EP task done */\n", irq);
...
int exynos_usb_audio_init(struct device *dev, struct platform_device *pdev)
{
	...
	usb_audio = devm_kmalloc(dev, sizeof(struct exynos_usb_audio), GFP_KERNEL);
	if (!usb_audio)
		return -ENOMEM;
...
static int exynos_usb_audio_unmap_all(void)
{
	...
	pr_debug("PCM IN BUFFER FREE: paddr = %#08llx\n", addr);
	...
	pr_debug("PCM OUT BUFFER FREE: paddr = %#08llx\n", addr);
	...
	pr_debug("DESC BUFFER: paddr : %#08llx / offset : %#08llx\n",
			addr, offset);
...
/* card */
static void exynos_usb_audio_connect(struct usb_interface *intf, struct usb_device *dev)
{
	...
	if (((altsd->bInterfaceClass == USB_CLASS_AUDIO) ||
		...
		pr_debug("USB_AUDIO_IPC : %s - MIDI device detected!\n", __func__);
	} else {
		pr_debug("USB_AUDIO_IPC : %s - No MIDI device detected!\n", __func__);
		if (!usb_audio->is_audio) {
			pr_debug("USB_AUDIO_IPC : %s - USB Audio set!\n", __func__);
...
static void exynos_usb_audio_disconn(void)
{
	exynos_usb_audio_conn(0);
}

/* clock */
static int exynos_usb_audio_set_inferface(struct usb_device *udev, struct usb_host_interface *alts, int iface, int alt)
{
	unsigned char ep;
...
static int exynos_usb_audio_set_rate(int iface, int rate, int alt)
{
	...
	if (ret < 0)
		pr_err("can not transfer sample rate to abox\n");

	return ret;
}

static int exynos_usb_audio_set_pcmbuf(struct usb_device *dev)
{
	int ret = 0;
...
static int exynos_usb_audio_set_pcm_intf(struct usb_device *udev, int iface, int alt, int direction)
{
	exynos_usb_audio_setintf(udev, iface, alt, direction);
	dev_dbg(&udev->dev, "setting usb interface %d:%d, Direction: %d\n",
			iface, 0, direction);
	return 0;
}

static void exynos_usb_audio_pcm_control(bool onoff, bool direction)
{
	if (onoff == 1) {
...
MODULE_DESCRIPTION("Exynos USB Audio driver");
