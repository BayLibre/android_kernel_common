...
int exynos_usb_audio_init(struct device *dev, struct platform_device *pdev)
{
	...
	usb_audio = devm_kmalloc(&pdev->dev, sizeof(struct exynos_usb_audio), GFP_KERNEL);
	if (!usb_audio)
		return -ENOMEM;
...
/* card */
static void exynos_usb_audio_connect(struct usb_interface *intf)
{
...
static int exynos_usb_audio_set_pcmbuf(struct usb_device *dev, int iface)
{
	...
	if (ret < 0)
		pr_err("pcm buf transfer failed\n");
...
static void exynos_usb_audio_pcm_control( struct usb_device *udev,
					bool onoff, bool direction)
{
	if (onoff == 1) {
		...
		unsigned long private_value)
{
	...
	if ((chip != NULL) && (private_value == 0)) {
		exynos_usb_add_ctls(chip, 0);
		return 0;
	} else if (private_value == 1) {
		ret = usb_audio->user_scenario;
	} else {
		pr_err("No case on %s: private_value: %d\n");
		ret = 0;
	}
...
static struct snd_usb_audio_vendor_ops exynos_usb_audio_ops = {
	...
	.vendor_usb_add_ctls = exynos_usb_audio_add_control,
};
