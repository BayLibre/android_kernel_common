...
/*
 * FIPS 140 prefers the use of HMAC with a public key over a plain hash.
 */
...
static void __init unregister_existing_fips140_algos(void)
{
	 ...
	 * so technically, we may be racing with a concurrent invocation of
	 * crypto_unregister_alg() here. However, that concurrent invocation
	 * will take the algorithm off of whichever list it is linked into, so
	 * this happens to be safe. We do need to hold on to the lock, as the
	 * actual list operations are not permitted to overlap.
	 */
...
/*
 * This routine iterates over the contents of the initcall section, which
...
int __init __attribute__((__no_sanitize__("cfi"))) fips140_init(void)
{
	...
	pr_info("Loading FIPS 140-2 module\n");
	...
	pr_info("FIPS 140-2 integrity check successful\n");
	pr_info("FIPS 140-2 module successfully loaded\n");
	return 0;
