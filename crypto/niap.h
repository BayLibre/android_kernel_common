...
static const u8 niap_aes_encrypt_key[] __initconst = {
	0x6f, 0x1e, 0x97, 0x12, 0xcf, 0x82, 0x40, 0x79,
	0x2c, 0x4f, 0x8a, 0x82, 0x78, 0x2a, 0x8c, 0xdd
};

static const u8 niap_aes_decrypt_key[] __initconst = {
	0x02, 0x52, 0x38, 0x55, 0xa6, 0x88, 0x51, 0xe2,
	0xd0, 0x16, 0xaa, 0x56, 0x95, 0x64, 0x33, 0x2d
};

static const u8 niap_ecb_aes_encrypt_output[] __initconst = {
	0xc0, 0x85, 0x2a, 0xc4, 0x01, 0x25, 0x1e, 0x15,
	0xfd, 0x53, 0x10, 0x84, 0x42, 0x96, 0xa9, 0x0c
};

static const u8 niap_ecb_aes_decrypt_output[] __initconst = {
	0x55, 0x9e, 0x10, 0x65, 0x28, 0x9c, 0x03, 0x94,
	0xef, 0xfb, 0xe1, 0x21, 0x18, 0x68, 0xc6, 0x2b
};
...
static const u8 niap_cbc_aes_encrypt_output[] __initconst = {
	0xd1, 0x4f, 0xe0, 0x79, 0xa1, 0x68, 0xe4, 0xa4,
	0x51, 0x96, 0xf6, 0x10, 0x5d, 0x6a, 0x63, 0xa0
};

static const u8 niap_cbc_aes_decrypt_output[] __initconst = {
...
static const struct niap_test niap_tests[] __initconst = {
	{
		...
		.cipher		= {
			.key		= niap_aes_encrypt_key,
			.key_length	= sizeof(niap_aes_encrypt_key),
			.input		= niap_message,
			.in_length	= sizeof(niap_message),
			.iv		= niap_cbc_aes_iv,
			.iv_length	= sizeof(niap_cbc_aes_iv),
			.output		= niap_cbc_aes_encrypt_output,
			.out_length	= sizeof(niap_cbc_aes_encrypt_output)
		}
	}, {
		.alg		= "cbc(aes)",
		.encrypt	= false,
		.func		= niap_test_cipher,
		.cipher		= {
			.key		= niap_aes_decrypt_key,
			.key_length	= sizeof(niap_aes_decrypt_key),
			.input		= niap_message,
			.in_length	= sizeof(niap_message),
			.iv		= niap_cbc_aes_iv,
			.iv_length	= sizeof(niap_cbc_aes_iv),
			.output		= niap_cbc_aes_decrypt_output,
			.out_length	= sizeof(niap_cbc_aes_decrypt_output)
		}
		...
		.cipher		= {
			...
			.input		= niap_message,
			.in_length	= sizeof(niap_message),
			.output		= niap_ecb_aes_encrypt_output,
			.out_length	= sizeof(niap_ecb_aes_encrypt_output)
		}
	}, {
		.alg		= "ecb(aes)",
		.encrypt	= false,
		...
		.cipher		= {
			...
			.input		= niap_message,
			.in_length	= sizeof(niap_message),
			.output		= niap_ecb_aes_decrypt_output,
			.out_length	= sizeof(niap_ecb_aes_decrypt_output)
		}
	}, {
		.alg		= "gcm(aes)",
		.encrypt	= true,
