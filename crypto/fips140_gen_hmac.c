...
static int update_rela_ref(const char *name, const char *var_name)
{
	/*
	...
	int sym_idx = get_sym_idx(name);
	char sec_name[32];
	int sec_idx;

	sprintf(sec_name, ".init%s", name);
	sec_idx = get_section_idx(sec_name);
...
int main(int argc, char **argv)
{
	...
	if (!update_rela_ref(".rela.text", "fips140_rela_text") ||
	    !update_rela_ref(".rela.rodata", "fips140_rela_rodata"))
		exit(EXIT_FAILURE);
